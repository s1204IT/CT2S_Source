...
**
**  Schedule an event to unmap user memory.
**
**  INPUT:
**
**      gckEVENT Event
...
#if !gcdNULL_DRIVER
        /* FIXED: add FromPower flag to resolve a deadlock.
               This function calling may be happening at power status
               switching which needs to acquire a power semaphore, in
               this case, invoking _TryToIdleGPU will try to acquire
               the same semaphore again, thus, it causes a dead-lock.
         */
        if(!FromPower && Event->kernel->bTryIdleGPUEnable)
        {
    ...
    )
{
    ...
    for (;;)
    {
        gcsEVENT_PTR record;
#if gcdMULTI_GPU
        gctUINT32 pend[gcdMULTI_GPU] = {0};
        gctUINT32 pendMask[gcdMULTI_GPU] = {0};
#endif
