...
static int __PLAT_APINAME(gc3d_pwr_ops)(struct gc_iface *iface, unsigned int enabled)
{
    ...
    if(has_feat_power_domain())
    {
        ...
        if(enabled)
        {
            trace_printk("GC3d usage_count: before get %d\n", atomic_read(&dev_GC3d->power.usage_count));
            retval = pm_runtime_get_sync((&(((struct platform_device*)(iface->pdev))->dev)));
            trace_printk("GC3d usage_count: after get %d, retval %d\n", atomic_read(&dev_GC3d->power.usage_count), retval);
            BUG_ON(atomic_read(&dev_GC3d->power.usage_count) <= 0);
        }
        else
        {
            trace_printk("GC3d usage_count: before put %d\n", atomic_read(&dev_GC3d->power.usage_count));
            BUG_ON(atomic_read(&dev_GC3d->power.usage_count) <= 0);
            retval = pm_runtime_put_sync((&(((struct platform_device*)(iface->pdev))->dev)));
            trace_printk("GC3d usage_count: after put %d, retval %d\n", atomic_read(&dev_GC3d->power.usage_count), retval);
        }
    }
    else
    {
