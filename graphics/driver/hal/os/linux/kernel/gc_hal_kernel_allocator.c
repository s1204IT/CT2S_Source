    ...
    )
{
    ...
    if (contiguous)
    {
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 27)
        addr =
            alloc_pages_exact(bytes, (GFP_KERNEL & ~__GFP_WAIT) | gcdNOWARN | __GFP_NORETRY
                              | __GFP_NO_KSWAPD | GFP_NOWAIT);
        ...
        Mdl->u.contiguousPages =
            alloc_pages((GFP_KERNEL & ~__GFP_WAIT) | gcdNOWARN | __GFP_NORETRY
                         | __GFP_NO_KSWAPD | GFP_NOWAIT, order);
#endif

        if (Mdl->u.contiguousPages == gcvNULL)
        {
            Mdl->u.contiguousPages =
                alloc_pages((GFP_KERNEL & ~__GFP_WAIT) | __GFP_HIGHMEM | gcdNOWARN | __GFP_NORETRY
                            | __GFP_NO_KSWAPD | GFP_NOWAIT, order);
