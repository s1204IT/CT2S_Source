...
static int address_valid(int if_index, union mysockaddr *ifaddr,
                         union mysockaddr *maddr)
{
  int fd;
  struct ifreq ifr;
  char debug_buff0[MAXDNAME], debug_buff1[MAXDNAME];
  ...
  /* check whether given IP address is still valid */
  prettyprint_addr(ifaddr, debug_buff0);
  prettyprint_addr(maddr, debug_buff1);
  my_syslog(LOG_DEBUG, _("interface: %s addr: %s, given addr: %s"),
            ifr.ifr_name, debug_buff0, debug_buff1);
  if (sockaddr_isequal(ifaddr, maddr))
      return 0;
...
static int address_valid_v6(struct in6_addr *local,
                            int scope, int if_index, union mysockaddr *maddr)
{
  union mysockaddr addr;
  struct in_addr netmask; /* dummy */
  ...
  return address_valid(if_index, &addr, maddr);
}
#endif
...
static int address_valid_v4(struct in_addr local, int if_index,
                            union mysockaddr *maddr, struct in_addr broadcast)
{
  union mysockaddr addr;
 ...
 */
int create_bound_listener(struct listener **listeners, struct irec *iface)
{
  int rc, opt = 1, ret = 0;
#ifdef HAVE_IPV6
  ...
  if (daemon->port != 0)
  {
    ...
    while(1)
    {
...
#ifdef HAVE_IPV6
        if (!rc && iface->addr.sa.sa_family == AF_INET6)
            dad_count = 0;
#endif
        break;
      }
          ...
          dad_count++ < DAD_WAIT)
      {
          ...
          my_syslog(LOG_DEBUG, _("SOCK_DGRAM bind listening socket %s failed: %s (%d) rc=%d dad_count=%d"),
                    daemon->namebuff, strerror(errno), errno, rc, dad_count);
          if (errno == ENODEV) {
              my_syslog(LOG_DEBUG, _("reenumerate interfaces"));
              dad_count = 0;
              return errno;
	  }
        sleep(1);
    ...
    {
      prettyprint_addr(&iface->addr, daemon->namebuff);
      my_syslog(LOG_DEBUG, _("SOCK_STREAM bind listening socket %s failed: %s (%d) rc=%d dad_count=%d"),
                daemon->namebuff, strerror(errno), errno, rc, dad_count);
      if (errno == ENODEV) {
		  my_syslog(LOG_DEBUG, _("reenumerate interfaces"));
		  dad_count = 0;
      ...
      if (iface->addr.sa.sa_family == AF_INET &&
          iface_address_valid(&iface->addr, address_valid_v4, NULL) == 1) {
		  my_syslog(LOG_DEBUG, "Invalid address: %s, matches no interface address", daemon->namebuff);
		  return EINVAL;
	  }
...
#ifdef __ANDROID__
static struct listener * check_created_listeners(struct irec * interface)
{
    struct listener *l = NULL;
    struct irec * listener_iface = NULL;

	if (daemon->listeners == NULL || interface == NULL) return NULL;

	for (l = daemon->listeners; l; l = l->next) {
        listener_iface = l->iface;
        if (listener_iface == NULL) continue; // no wild card listeners should exist
        if (sockaddr_isequal(&listener_iface->addr, &interface->addr))
            return l;
    }
...
static struct irec * update_bound_listener(struct irec *new_ifc)
{
	struct listener *l = NULL;
	struct irec *old_iface = NULL;
	struct irec *next_irec = NULL;

	if (daemon->listeners == NULL || new_ifc == NULL) return NULL;

	for (l = daemon->listeners; l; l = l->next) {
		old_iface = l->iface;
		if (old_iface == NULL) continue;
		if (sockaddr_isequal(&old_iface->addr, &new_ifc->addr)) break;
	}

	if (l == NULL) return NULL;
	l->iface = new_ifc;
	next_irec = new_ifc->next;

	memcpy(l->iface, old_iface, sizeof(struct irec)); //restore irec contents
	l->iface->next = next_irec;                      //reset next to new
...
#ifdef __ANDROID__
/* Timing issue casued by android tethering architecture
 * workaround to prevent tethering enabling fail
 * interface may be brought down from android UI, do not let dnsmasq die unexpectedly
 */
	struct iname **i = &(daemon->if_names);
	while(*i) {
		if ((*i)->name && !((*i)->used)) {
			if_tmp = *i;
			*i = if_tmp->next;
    ...
    for (old_iface = prev_interfaces; old_iface; old_iface=old_iface->next) {
      ...
      if (found) {
        update_bound_listener(new_iface);
      } else {
#ifdef __ANDROID_DEBUG__
    ...
    for (new_iface = daemon->interfaces; new_iface; new_iface = new_iface->next) {
      ...
      for (old_iface = prev_interfaces; old_iface; old_iface = old_iface->next) {
        if(sockaddr_isequal(&old_iface->addr, &new_iface->addr) &&
           check_created_listeners(new_iface)) {
          found = -1;
          break;
        }
      }
...
#endif
        if (create_bound_listener(&(daemon->listeners), new_iface)) {
#ifdef HAVE_IPV6
            if (errno == ENODEV && new_iface->addr.sa.sa_family == AF_INET6)
                goto reenumerate;
#endif
		}
      }
  ...
  while ((saddr = strsep(&next, ";"))) {
      ...
      else if (inet_pton(AF_INET6, saddr, &addr.in6.sin6_addr) > 0)
	{
	  ...
	  //source_addr.in6.sin6_len = addr.in6.sin6_len = sizeof(source_addr.in6);
          addr.in6.sin6_len = sizeof(source_addr.in6);
          source_addr.in.sin_len = sizeof(source_addr.in);
#endif
