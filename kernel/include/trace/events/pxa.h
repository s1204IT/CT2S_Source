...
#define DVC_CHG_ENTRY	1
#define DVC_CHG_EXIT	2
...
TRACE_EVENT(pxa_dvc_d0_chg,
	...
	TP_printk("state: %u dvc level: %u -> %u",
		__entry->state, __entry->val1, __entry->val2)
);

TRACE_EVENT(pxa_dvc_d0cg_chg,
	TP_PROTO(u32 state, u32 val1, u32 val2),
...
TRACE_EVENT(pxa_thermal_trip,
	TP_PROTO(char *thermal_name, int temp, unsigned long trip_state, unsigned long freq_state),

	TP_ARGS(thermal_name, temp, trip_state, freq_state),

	TP_STRUCT__entry(
		__field(char*, thermal_name)
		__field(int, temp)
		__field(unsigned long, trip_state)
		__field(unsigned long, freq_state)
	),

	TP_fast_assign(
		__entry->thermal_name = thermal_name;
		__entry->temp = temp;
		__entry->trip_state = trip_state;
		__entry->freq_state = freq_state;
	),

	TP_printk("%s; temp:%d; trip_state:%lu; freq_state:%lu\n",
		__entry->thermal_name, __entry->temp,
		__entry->trip_state, __entry->freq_state)
);
