...
struct isp_block_ops {
	...
	int	(*set_power)(void *block, int level);
...
/* Helper functions */
static inline u32 isp_read(struct isp_block *block, const u32 addr)
{
	return readl(block->reg_base + addr);
}

static inline void isp_write(struct isp_block *block,
				const u32 addr, const u32 value)
{
	writel(value, block->reg_base + addr);
}

static inline void isp_set(struct isp_block *block,
...
static inline void isp_clr(struct isp_block *block,
				const u32 addr, const u32 mask)
{
	u32 val = readl(block->reg_base + addr);
	val &= ~mask;
