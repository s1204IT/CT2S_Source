...
static void __iomem *APMU_MC_CTRL1;
static void __iomem *ICU_GBL_IRQ_MSK[4];
static void __iomem *ICU_GBL_FIQ_MSK[4];
static u32 dvc_lpm[AP_COMP_MAX] = {POWER_MODE_MAX, POWER_MODE_APPS_IDLE};
...
void pxa1928_enable_lpm_ddrdll_recalibration(void)
{
	...
	if (pxa1928_is_a0()) {
		...
		/*
		 * d2 entry regtable index 31
		 * d2 exit regtable index 30
		 */
		writel(0xde1f, APMU_MC_CTRL1);
	}
}
...
#define ENABLE_AP_WAKEUP_SOURCES	\
	(PMUA_CP_IPC | PMUA_CP_TIMER1 | PMUA_PMIC | PMUA_AUDIO | PMUA_RTC_ALARM |\
	 PMUA_KEYPAD | PMUA_GPIO | PMUA_TIMER_3_1 | PMUA_TIMER_3_2 |\
	 PMUA_TIMER_3_3 | PMUA_SDH1)
...
static void pxa1928_release(u32 cpu)
{
	...
	dsb(ish);
	writel(PMUA_CORE_RSTCTRL_CPU_ETM_NRESET	|
	       PMUA_CORE_RSTCTRL_CPU_DBG_NRESET |
	       PMUA_CORE_RSTCTRL_CPU_NRESET |
	       PMUA_CORE_RSTCTRL_CPU_NPORESET,
...
static void __init pxa1928_pmu_mapping(void)
{
	...
	APMU_MC_CTRL1 = apmu_virt_addr + MC_CTRL1;
}
...
static void __init pxa1928_icu_mapping(void)
{
	...
	node = of_find_compatible_node(NULL, NULL, "mrvl,pxa1928-intc");
	BUG_ON(!node);
...
static int __init pxa1928_lowpower_init(void)
{
	...
	wk_a0 = cpu_is_pxa1928_a0();

	pr_info("Initialize pxa1928 low power controller.\n");
	/* Deliver hwdvc lpms and max lpm supported to hwdvc statistics */
	hwdvc_get_platlpm(dvc_lpm);
	max_lpm = POWER_MODE_MAX;
	...
	*/
	...
	if (!pxa1928_is_a0()) {
		u32 cpmu_dbg1;
		cpmu_dbg1 = readl(mpmu_virt_addr + CPMU_DBG1);
		cpmu_dbg1 |= 0xc0000000;
		writel(cpmu_dbg1, mpmu_virt_addr + CPMU_DBG1);
	}

	np = of_find_compatible_node(NULL, NULL, "marvell,mmp-map");
	if (!np)
		return 0;
	of_property_read_u32(np, "marvell,apll", &vcxo_d1);
	return 0;
}
