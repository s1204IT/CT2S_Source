...
#define IRQ_PXA1928_CP_TIMER1		(IRQ_PXA1928_START + 39)
...
static void pxa1928_set_wake(int irq, unsigned int on)
{
	...
	switch (irq) {
	case IRQ_PXA1928_CP_TIMER1:
		wkup_mask = PMUA_CP_TIMER1;
		break;
...
static int pxa1928_pm_check_constraint(void)
{
	...
	list_for_each_entry(node, list, node_list) {
		 ...
		 */
		if (node->prio == POWER_MODE_UDR) {
			pr_info("****pm_check_constraint PM_QOS_CPUIDLE_BLOCK_UDR %s\n",
				req->name);
		} else if (node->prio == POWER_MODE_UDR_VCTCXO) {
			pr_info("****pm_check_constraint PM_QOS_CPUIDLE_BLOCK_UDR_VCXO %s\n",
				req->name);
...
static u32 wakeup_source_check(void)
{
	...
	int len_s = 0, len = 0;
	void __iomem *gic_dist_base, *gic_cpu_base;
	int i;
	u32 reg;
	...
	if (wkup_stat & PMUA_GPIO) {
		...
		ret = check_mfp_wakeup_stat(buf, len, size);
		if (ret < 0)
	...
	if (len_s != len) {
		...
		gic_cpu_base = gic_get_cpu_base();
		gic_dist_base = gic_get_dist_base();
		reg = readl_relaxed(gic_cpu_base + 0x18);
		pr_info("GIC_CPU_HPPIR Reg 0x%x\n", reg);
		for (i = 0; i < 6; i++) {
			reg = readl_relaxed(gic_dist_base + GIC_DIST_ENABLE_SET + (i * 4));
			pr_info("GIC_DIST_ENABLE_SET Reg %d value: 0x%x\n", i, reg);
			reg = readl_relaxed(gic_dist_base + GIC_DIST_PENDING_SET + (i * 4));
			pr_info("GIC_DIST_PENDING_SET Reg %d value: 0x%x\n", i, reg);
		}
	}
...
EXPORT_SYMBOL(clear_keypad_wakeup);
