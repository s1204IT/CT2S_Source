...
#define SPSR_AIF_BITS (COMPAT_PSR_A_BIT | COMPAT_PSR_I_BIT | COMPAT_PSR_F_BIT)

struct stepping_kernel {
	int	state;		/* stepping state */
	u32	spsr_org;	/* original spsr changed for stepping */
};
...
static DEFINE_PER_CPU(struct stepping_kernel, stepping_kernel_bp);
...
{
	int i, step = 0;
	struct stepping_kernel *kernel_step;
	u32 ctrl_reg;
	u64 addr, val;
	...
	for (i = 0; i < core_num_brps; ++i) {
		 ...
		 * The breakpoint handler (overflow_handler called above)
		 * is unlikely willing to implement the stepping logic,
		 * so the only thing is could do to prevent re-entry after
		 * return from the debug exception is disabling the breakpoint.
		 */
		 if (!bp->overflow_handler
		     || (read_wb_reg(AARCH64_DBG_REG_BCR, i) & 1))
			step = 1;
unlock:
		rcu_read_unlock();
	}
	...
	if (user_mode(regs)) {
		...
		if (kernel_step->state != ARM_KERNEL_STEP_NONE)
			return 0;

		if (kernel_active_single_step()) {
			kernel_step->state = ARM_KERNEL_STEP_SUSPEND;
		} else {
			kernel_step->state = ARM_KERNEL_STEP_ACTIVE;
			kernel_enable_single_step(regs);
			kernel_step->spsr_org = (u32)regs->pstate;
			regs->pstate |= SPSR_AIF_BITS;
		}
	}
			      ...
			      struct pt_regs *regs)
{
	...
	for (i = 0; i < core_num_wrps; ++i) {
		...
		/*
		 * Do we need to handle the stepping?
		 * The watchpoint handler (overflow_handler called above)
		 * is unlikely willing to implement the stepping logic,
		 * so the only thing is could do to prevent re-entry after
		 * return from the debug exception is disabling the watchpoint.
		 */
		 if (!wp->overflow_handler
		     || (read_wb_reg(AARCH64_DBG_REG_WCR, i) & 1))
			step = 1;
...
int reinstall_suspended_bps(struct pt_regs *regs)
{
	...
	if (user_mode(regs)) {
	...
	} else if (kernel_step->state != ARM_KERNEL_STEP_NONE) {
		...
		if (kernel_step->state != ARM_KERNEL_STEP_SUSPEND) {
			regs->pstate = (regs->pstate & ~SPSR_AIF_BITS)
				     | (kernel_step->spsr_org & SPSR_AIF_BITS);
		...
		kernel_step->state = ARM_KERNEL_STEP_NONE;
	}
