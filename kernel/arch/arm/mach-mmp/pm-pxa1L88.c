...
static void pxa1L88_set_wake(int irq, unsigned int on)
{
	...
	case IRQ_PXA1L88_AP0_TIMER1:
		if (cpu_is_pxa1U88()) {
			apbc_timer0 = __raw_readl(APBC_MMPX_TIMER0);
			__raw_writel(0x1 << 7 | apbc_timer0, APBC_MMPX_TIMER0);
		}
	...
	if (on) {
		if (awucrm) {
			awucrm |= __raw_readl(get_mpmu_base_va() + AWUCRM);
			__raw_writel(awucrm, get_mpmu_base_va() + AWUCRM);
		}
	} else {
		if (awucrm) {
			awucrm = ~awucrm & __raw_readl(get_mpmu_base_va() + AWUCRM);
			__raw_writel(awucrm, get_mpmu_base_va() + AWUCRM);
		}
	}
}
...
{
	u32 ret, reg = __raw_readl(ICU_INT_CONF(IRQ_PXA1L88_PMIC - IRQ_PXA1L88_START));
	...
	pr_info("BEFORE SUSPEND AWUCRS:0x%x\n", __raw_readl(get_mpmu_base_va() + AWUCRS));
...
#define ICU_INT_STATUS_BASE	0x200
	int i, bit, len = 0;
	void __iomem *irq_stat_base = icu_get_base_addr() + ICU_INT_STATUS_BASE;
...
static u32 wakeup_source_check(void)
{
	...
	size_t size = PAGE_SIZE - 1;
	int len_s = 0, len = 0;
	u32 awucrm = __raw_readl(get_mpmu_base_va() + AWUCRM);
	u32 cwucrm = __raw_readl(get_mpmu_base_va() + CWUCRM);

	u32 awucrs = __raw_readl(get_mpmu_base_va() + AWUCRS);
	u32 cwucrs = __raw_readl(get_mpmu_base_va() + CWUCRS);

	u32 apcr = __raw_readl(get_mpmu_base_va() + APCR);
	u32 cpcr = __raw_readl(get_mpmu_base_va() + CPCR);
	...
	if (len_s != len) {
		...
		if (real_wus & PMUM_AP_ASYNC_INT) {
			...
			void __iomem *gic_dist_base = gic_get_dist_base();
