...
#define PXA1928_DVC_LEVEL_NUM	4
...
enum {
	...
	PXA1928_DVC_D0_FACTOR_NUM,
};

static char *pxa1928_dvfs_d0_clk_name[] = {
	[CORE]	= "cpu",
	[GC3D]	= "GC3D_CLK1X",
	[GC2D]	= "GC2D_CLK",
	[DDR]	= "ddr",
...
static const int pxa1928_dvfs_d0_threshold[][PXA1928_DVC_LEVEL_NUM + 1] = {
...
static const int pxa1928_dvfs_d0_threshold_z2[][PXA1928_DVC_LEVEL_NUM + 1] = {
...
static int pxa1928_dvfs_d0_init(void)
{
	struct dvfs *vd = NULL;
	struct vol_table *vt = NULL;
	struct clk *c;
	unsigned long rate;
	const int (*thr_tbl)[PXA1928_DVC_D0_FACTOR_NUM][
						PXA1928_DVC_LEVEL_NUM + 1];
	int i, j;
	int ret = 0;

	if (cpu_is_pxa1928_z1())
		thr_tbl = &pxa1928_dvfs_d0_threshold;
	else
		thr_tbl = &pxa1928_dvfs_d0_threshold_z2;

	for (i = 0; i < PXA1928_DVC_D0_FACTOR_NUM; i++) {
		...
		vd->clk_name = pxa1928_dvfs_d0_clk_name[i];
		vd->dvfs_rail = &pxa1928_dvfs_rail_d0;
...
enum {
	...
	PXA1928_DVC_D0CG_FACTOR_NUM,
};

static char *pxa1928_dvfs_d0cg_clk_name[] = {
...
static const int pxa1928_dvfs_d0cg_threshold[][PXA1928_DVC_LEVEL_NUM + 1] = {
...
static int pxa1928_dvfs_d0cg_init(void)
{
	struct dvfs *vd = NULL;
	struct vol_table *vt = NULL;
	struct clk *c;
	...
	for (i = 0; i < PXA1928_DVC_D0CG_FACTOR_NUM; i++) {
		/* factor is not enabled */
		if (!pxa1928_dvfs_d0cg_threshold[i][0])
			continue;
		...
		for (j = 1; j < PXA1928_DVC_LEVEL_NUM + 1; j++) {
			vt[j - 1].freq = pxa1928_dvfs_d0cg_threshold[i][j];
			vt[j - 1].millivolts = j - 1;
		}
		vd->vol_freq_table = vt;
		vd->clk_name = pxa1928_dvfs_d0cg_clk_name[i];
		vd->dvfs_rail = &pxa1928_dvfs_rail_d0cg;
...
static unsigned int pxa1928_dvfs_map_table_z2[][PXA1928_DVC_LEVEL_NUM] = {
	/* profile 0 */
	[0] = {1225000, 1225000, 1225000, 1225000},
	/* profile 1 */
	[1] = {887500, 887500, 987500, 1162500},
...
static void pmic_init_setup(void)
{
	...
	if (cpu_is_pxa1928_z1())
		dvc_map = &pxa1928_dvfs_map_table[mv_profile];
	else
		dvc_map = &pxa1928_dvfs_map_table_z2[mv_profile];

	for (i = 0; i < PXA1928_DVC_LEVEL_NUM; i++)
		pg870_dvc_setvolt(i, (*dvc_map)[i]);
}

static void pxa1928_dvfs_setup(void)
{
...
int __init pxa1928_init_dvfs(void)
{
	read_mv_profile();
	pxa1928_dvfs_setup();
	pmic_init_setup();
	...
	pxa1928_dvfs_d0_init();
	pxa1928_dvfs_d0cg_init();

	return 0;
}
late_initcall(pxa1928_init_dvfs);
