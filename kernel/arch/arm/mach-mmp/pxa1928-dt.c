...
static void __init pxa1928_reserve_obmmem(void)
{
...
static void __init pxa1928_dt_irq_init(void)
{
	irqchip_init();
	/* only for wake up */
...
static const struct of_dev_auxdata pxa1928_auxdata_lookup[] __initconst = {
	...
	OF_DEV_AUXDATA("marvell,pxa1928-gpio", 0xd4019000, "pxa1928-gpio",
			NULL),
	OF_DEV_AUXDATA("mrvl,mmp-edge-wakeup", 0xd4019800,
			"mmp-edge-wakeup", NULL),
	...
	OF_DEV_AUXDATA("marvell,act_monitor", 0xd0000140, "act_monitor", NULL),
#endif
...
static void __init pxa1928_dt_init_machine(void)
{
	...
	of_platform_populate(NULL, of_default_bus_match_table,
			     pxa1928_auxdata_lookup, &platform_bus);
	/* put gc in reset state */
	gc_reset();

	/* raise lcd/vdma port priority */
	writel(0xff003030, get_mccu_base_va() + (0x84));

	pxa1928_register_table_lpddr2_dll_calibration(
					(u32 *)get_mccu_base_va());
}

static const char *pxa1928_dt_board_compat[] __initdata = {
	"mrvl,pxa1928-concord",
	NULL,
};
...
DT_MACHINE_START(PXA1928_DT, "PXA1928")
	...
	.map_io		= mmp_map_io,
	.init_irq	= pxa1928_dt_irq_init,
	.init_time	= pxa1928_timer_init,
	.init_machine	= pxa1928_dt_init_machine,
