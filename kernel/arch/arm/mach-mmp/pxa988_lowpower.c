...
/* Used for dvc status statistics */
static u32 dvc_lpm[AP_COMP_MAX] = {POWER_MODE_MAX, POWER_MODE_CORE_POWERDOWN,
...
static void edge_wakeup_port_enable(void)
{
	...
	if (readl_relaxed(get_mpmu_base_va() + APCR) & PMUM_SLPWP2) {
		pr_err("Port2 is NOT enabled in APCR!\n");
		pr_err("APCR: 0x%x\n",
			readl_relaxed(get_mpmu_base_va() + APCR));
		BUG();
	}
...
static void sdh_wakeup_icu_enable(void)
{
	if (cpu_is_pxa1U88())
		writel_relaxed(ICU_IRQ_ENABLE,
			icu_virt_addr + (SDH_WAKEUP_ICU << 2));
}
...
{
	if (cpu_is_pxa1U88())
		__raw_writel(apbc_timer0, APBC_MMPX_TIMER0);
