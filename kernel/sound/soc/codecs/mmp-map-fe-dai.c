...
/* i2s3 clk is enabled by widget or not */
static bool i2s3_clk_widget_en;

static void map_i2s3_clk_en(struct map_private *map_priv, bool widget_en)
{
	u32 reg, val;

	if (widget_en) {
		spin_lock(&map_priv->map_lock);
		map_priv->i2s3_use_count++;
		spin_unlock(&map_priv->map_lock);
		if (map_priv->i2s3_use_count > 1)
			return;

		/* set port frame clock frequence */
		map_set_port_freq(map_priv, I2S3, 48000);

		/* set bclk divider */
		reg = MAP_I2S3_BCLK_DIV;
		val = 0x10010030; /* bclk=48000*64, fvco=589.824MHz */
		...
		spin_lock(&map_priv->map_lock);
		if (map_priv->i2s3_use_count > 0)
			map_priv->i2s3_use_count--;
		spin_unlock(&map_priv->map_lock);
		if (map_priv->i2s3_use_count)
			return;
		      ...
		      struct snd_ctl_elem_value *ucontrol)
{
	...
	else if (params->base == MAP_I2S3_CLK_EN)
		*(u32 *)(&ucontrol->value.bytes.data) = i2s3_clk_widget_en;
		      ...
		      struct snd_ctl_elem_value *ucontrol)
{
	...
	if ((params->base == MAP_BT_WORK_MODE) && (!!val)) {
		...
		goto out;
	} else if ((params->base == MAP_I2S3_CLK_EN) && (!!val)) {
		if (map_priv->user_count) {
			if (!i2s3_clk_widget_en) {
				i2s3_clk_widget_en = true;
				map_i2s3_clk_en(map_priv, true);
			} else
				pr_info("widget is open already!\n");
		} else
			pr_info("MAP is reset, will not enable I2S3 clk.\n");
		goto out;
	} else if ((params->base == MAP_I2S3_CLK_EN) && (!val)) {
		if (map_priv->user_count) {
			if (i2s3_clk_widget_en) {
				i2s3_clk_widget_en = false;
				map_i2s3_clk_en(map_priv, false);
			} else
				pr_info("widget is closed already!\n");
		} else
			pr_info("MAP is reset, will not disable I2S3 clk.\n");
		goto out;
	}
...
static const struct snd_kcontrol_new map_snd_controls[] = {
	...
	SND_SOC_BYTES_INFO("MAP_I2S3_CLK_EN", MAP_I2S3_CLK_EN, 1,
				map_bytes_get, map_bytes_put),
	/*
...
static const struct snd_kcontrol_new dsp1_en_control =
	SOC_DAPM_SINGLE_VIRT("Switch", 0, 0, 1, 0);
...
/* ADC output ain3 -> ain3_spkr_en -> I2S_SPKR_DL enable */
static const struct snd_kcontrol_new ain3_spkr_en_control =
	SOC_DAPM_SINGLE_VIRT("Switch", 0, 0, 1, 0);

/* ADC output ain3 -> i2s3_bt_ul -> BT_UL enable */
static const struct snd_kcontrol_new i2s3_bt_ul_control =
	SOC_DAPM_SINGLE_VIRT("Switch", 0, 0, 1, 0);

static const struct snd_soc_dapm_widget map_dapm_widgets[] = {
	SND_SOC_DAPM_VIRT_SWITCH("i2s3_bt_ul", SND_SOC_NOPM, 0, 0,
				&i2s3_bt_ul_control),

	SND_SOC_DAPM_VIRT_SWITCH("ain3_spkr_en", SND_SOC_NOPM, 0, 0,
				&ain3_spkr_en_control),

	SND_SOC_DAPM_VIRT_MUX("i2s3_func_sel", SND_SOC_NOPM, 0, 0,
				&i2s3_func_sel_mux),
...
static const struct snd_soc_dapm_route map_intercon[] = {
	...
	{"ain3_spkr_en", "Switch", "ADC output ain3"},
	...
	{"BT_UL", NULL, "i2s3_bt_ul"},
	{"i2s3_bt_ul", "Switch", "ADC output ain3"},
		...
		int clk_id, unsigned int freq, int dir)
{
	...
	/* check whether 32bit is used for i2s1/i2s4 */
	if ((codec_dai->id == 1) || (codec_dai->id == 4)) {
		reg_pcm = MAP_I2S1_CTRL_REG + ((codec_dai->id - 1) << 2);

		val_pcm = map_raw_read(map_priv, reg_pcm);
		if (val_pcm & MAP_I2S_LRCLK_WIDTH_32BCLK)
			val = (val & ~0xffff) | (((val & 0xffff) << 1) & 0xffff);
	}
	map_raw_write(map_priv, reg, val);
		...
		unsigned int fmt)
{
	struct map_fe_dai_private *map_fe_dai_priv;
	struct map_private *map_priv;
	unsigned int inf = 0, addr, inf_pcm;
	...
	case SND_SOC_DAIFMT_DSP_A:
		...
		inf |= MAP_BCLK_POL;
		/*
		 * fixup for i2s1/4 pcm mode:
		 * need to use 32bit width(64bit causes swap)
		 */
		if ((codec_dai->id == 1) || (codec_dai->id == 4))
			inf |= MAP_I2S_LRCLK_WIDTH_32BCLK;

		/* i2s3's pcm width bit will affect pcm protocol of all i2s port */
		inf_pcm = map_raw_read(map_priv, MAP_I2S3_CTRL_REG);
		inf_pcm |= MAP_PCM_WIDTH_SEL;
		map_raw_write(map_priv, MAP_I2S3_CTRL_REG, inf_pcm);
		break;
