...
/* Fix me: we need manual open S1 to enalbe hook detection on A0 PMIC */
static void pm860_config_hook_detect(struct pm800_hs_info *info,
	int headset_status)
{
	if  ((headset_status & 0x30) == 0x20) {
		regmap_update_bits(info->map, 0x1f, 0x1, 0x1);
		regmap_update_bits(info->map_test, 0x22, 0x1, 0x1);
		regmap_update_bits(info->map_test, 0x23, 0xff, 0x29);
		regmap_update_bits(info->map, 0x1f, 0x1, 0x0);
	} else if ((headset_status & 0x30) == 0x10) {
		regmap_update_bits(info->map, 0x1f, 0x1, 0x1);
		regmap_update_bits(info->map_test, 0x22, 0x1, 0x1);
		regmap_update_bits(info->map_test, 0x23, 0xff, 0x16);
		regmap_update_bits(info->map, 0x1f, 0x1, 0x0);
	} else if (!headset_status) {
		regmap_update_bits(info->map, 0x1f, 0x1, 0x1);
		regmap_update_bits(info->map_test, 0x22, 0x1, 0x0);
		regmap_update_bits(info->map_test, 0x23, 0xff, 0x0);
		regmap_update_bits(info->map, 0x1f, 0x1, 0x0);
		/* disalbe auto mode ground detect */
		regmap_update_bits(info->map, 0x3A, 0xFF, 0x0);
	}
}

/*
 * in case ground detection get the wrong GND,
 * we need get rid of hook irq storm
 */
static void pm860_check_ground_error(struct pm800_hs_info *info, int voltage)
{
	/* according our test, the error value is ~500 mV */
	if (voltage > 500 && voltage < 600) {
		pm860_config_hook_detect(info, 0);
...
static void pm800_hook_work(struct pm800_hs_info *info)
{
	...
	if (info->ground_detect)
		pm860_check_ground_error(info, voltage);
	/*
	...
	if (voltage < info->press_release_th) {
		if (info->hk_avg
			...
			&& voltage < info->hook_press_th)
			goto FAKE_HOOK;
		else if (!info->hk_avg)
			if (voltage > 10 && voltage < info->hook_press_th - 20)
				goto FAKE_HOOK;
		/* get rid of fake volume down event when votage is ~377 */
		if (voltage > (377 - 10) && voltage < (377 + 10))
			goto FAKE_HOOK;
	}
...
static void pm860_config_ground_detection(struct pm800_hs_info *info)
{
	/* set GPIO 6 and GPIO 7 as GND_S1 mode */
	regmap_update_bits(info->map, 0x33, 0xcc, 0xcc);
	/* GPADC non stop mode */
	regmap_update_bits(info->map_gpadc, 0x06, 0x03, 0x03);
	/* set ground detect voltage threshold */
	regmap_update_bits(info->map_gpadc, 0x25, 0x12, 0x12);
	regmap_update_bits(info->map_gpadc, 0x26, 0x24, 0x24);
	/* enalbe auto mode ground detect */
	regmap_update_bits(info->map, 0x3A, 0xFE, 0xFE);
}
...
static void pm800_headset_work(struct pm800_hs_info *info)
{
	...
	if (value) {
		...
		if (info->ground_detect) {
			pm860_config_ground_detection(info);

			/* experience value, 7 times is enough */
			i = 7;
			/* should not be inited as 0, value is default 0 */
			old[0] = old[1] = old[2] = 1;
			while (i--) {
				/* trigger detection */
				regmap_update_bits(info->map, 0x3B, 0x40, 0x40);
				msleep(150);
				ret = regmap_read(info->map, 0x3B, &value);
				if (ret < 0) {
					dev_err(info->dev,
					"ground register can not be read\n");
					return;
				}
				...
				old[1] = old[2];
				old[2] = value;
				/* when we get 3 times same value,
				we think it is valid */
				if (old[0] == old[1] && old[1] == old[2])
					break;
			}

			pm860_config_hook_detect(info, value);
			if ((value & 0x30) == 0x30)
				report = SND_JACK_HEADPHONE;
			else
				gpadc_set_threshold(info,
					info->press_release_th, 0);
			/* close GPADC non stop mode, after detect */
			regmap_update_bits(info->map_gpadc, 0x06, 0x02, 0x0);
					...
					pm_message_t state)
{
	...
	/* enalbe sleep detect mode, z3's chip do not support */
...
static int pm800_headset_resume(struct platform_device *pdev)
{
	...
	/* disalbe sleep detect mode, z3's chip do not support */
	if (info->chip_id != CHIP_PM86X_ID_Z3)
		regmap_update_bits(hs_info->map, PM800_HEADSET_CNTRL,
				PM800_HSDET_SLP,
				0);
...
static int pm800_headset_probe(struct platform_device *pdev)
{
	...
	if (IS_ENABLED(CONFIG_OF)) {
		...
		if (of_property_read_u32(pdev->dev.of_node,
			"marvell,ground-detect", &hs_info->ground_detect))
			dev_dbg(&pdev->dev, "Do not get ground detect\n");
		if (of_property_read_u32(pdev->dev.of_node,
	...
	hs_info->map_test = chip->subchip->regmap_test;
	...
	if (hs_info->ground_detect)
		regmap_update_bits(hs_info->map, PM800_MIC_CNTRL, HS_POL,
			   HS_POL);
