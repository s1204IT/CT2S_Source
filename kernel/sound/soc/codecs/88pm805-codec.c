...
static void pm805_codec_shutdown(struct platform_device *pdev)
{
	struct pm805_priv *pm805 = platform_get_drvdata(pdev);

	devm_free_irq(&pdev->dev, pm805->irq_audio_short1, pm805);
	devm_free_irq(&pdev->dev, pm805->irq_audio_short2, pm805);
