				   ...
				   struct snd_kcontrol *kcontrol, int connect)
{
	...
	if (widget->id != snd_soc_dapm_mixer &&
	    ...
	    widget->id != snd_soc_dapm_virt_switch)
		return -ENODEV;
	...
	struct snd_ctl_elem_value *ucontrol)
{
	...
	if (change) {
		for (wi = 0; wi < wlist->num_widgets; wi++) {
			...
			ret = soc_dapm_mixer_update_power(widget,
							kcontrol, connect);
	...
	struct snd_ctl_elem_value *ucontrol)
{
	...
	if (change) {
		for (wi = 0; wi < wlist->num_widgets; wi++) {
			...
			ret = soc_dapm_mux_update_power(widget,
							kcontrol, mux, e);
			widget->dapm->update = NULL;
		}
	}
