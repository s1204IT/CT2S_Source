...
#define MAPASOC_SAMPLE_RATES_HIFI SNDRV_PCM_RATE_44100
#define MAPASOC_SAMPLE_RATES_FM SNDRV_PCM_RATE_48000
/* I2S2/I2S3 use 8K or 16K sample rate */
#define MAPASOC_SAMPLE_RATES_LOFI (SNDRV_PCM_RATE_8000 | \
...
static int map_startup_FM(struct snd_pcm_substream *substream)
{
	...
	cpu_dai->driver->playback.rates = MAPASOC_SAMPLE_RATES_FM;
	cpu_dai->driver->capture.rates = MAPASOC_SAMPLE_RATES_FM;
...
static int map_startup_hifi(struct snd_pcm_substream *substream)
{
	...
	cpu_dai->driver->playback.formats = SNDRV_PCM_FMTBIT_S16_LE;
	cpu_dai->driver->capture.formats = SNDRV_PCM_FMTBIT_S16_LE;
	cpu_dai->driver->playback.rates = MAPASOC_SAMPLE_RATES_HIFI;
	cpu_dai->driver->capture.rates = MAPASOC_SAMPLE_RATES_HIFI;
...
static int map_startup_lofi(struct snd_pcm_substream *substream)
{
	...
	cpu_dai->driver->playback.formats = SNDRV_PCM_FMTBIT_S16_LE;
	cpu_dai->driver->capture.formats = SNDRV_PCM_FMTBIT_S16_LE;
	cpu_dai->driver->playback.rates = MAPASOC_SAMPLE_RATES_LOFI;
	cpu_dai->driver->capture.rates = MAPASOC_SAMPLE_RATES_LOFI;
			      ...
			      struct snd_pcm_hw_params *params)
{
	...
	/* For i2s3, the dai format is i2s; others are pcm(mode A) */
	if (codec_dai->id == 3) {
		snd_soc_dai_set_fmt(codec_dai, SND_SOC_DAIFMT_I2S |
