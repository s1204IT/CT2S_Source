			     ...
			     struct snd_soc_dai *dai)
{
	...
	case SNDRV_PCM_TRIGGER_PAUSE_RELEASE:
		 ...
		 */
#ifndef CONFIG_SND_SOC_ADIR_DKB
		if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK)
...
static int asoc_mmp_sspa_probe(struct platform_device *pdev)
{
	...
	priv->sspa->clk = devm_clk_get(&pdev->dev, pdev->name);
	if (IS_ERR(priv->sspa->clk))
		return PTR_ERR(priv->sspa->clk);
	/*
	 * Fixme: After we switch to use common clock driver,
	 * We shouldn't touch audio pll. For sysclk, Maybe we
	 * need to got its name from DT. Need to align with
	 * other platform, such as PXA1928 Zx and Adir.
	 */
#ifndef CONFIG_SND_MMP_MAP
	...
	ret = snd_soc_register_component(&pdev->dev, &mmp_sspa_component,
					  &mmp_sspa_dai, 1);
	if (ret != 0) {
