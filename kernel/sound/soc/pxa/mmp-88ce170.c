...
#define PXA1928ASOC_SAMPLE_RATES SNDRV_PCM_RATE_44100
...
static int mmp_88ce170_ce170_startup(struct snd_pcm_substream *substream)
{
	...
	cpu_dai->driver->playback.rates = PXA1928ASOC_SAMPLE_RATES;
	cpu_dai->driver->capture.rates = PXA1928ASOC_SAMPLE_RATES;
...
static int pxa1928asoc_suspend(struct device *dev)
{
	snd_soc_suspend(dev);
...
static int pxa1928asoc_resume(struct device *dev)
{
	snd_soc_resume(dev);

	return 0;
}
static UNIVERSAL_DEV_PM_OPS(pxa1928asoc_pm_ops, pxa1928asoc_suspend,
			 pxa1928asoc_resume, NULL);
#endif
...
/* Module information */
MODULE_DESCRIPTION("ALSA SoC CE170 PXA1928");
MODULE_LICENSE("GPL");
