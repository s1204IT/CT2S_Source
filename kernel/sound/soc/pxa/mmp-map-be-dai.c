				 ...
				 unsigned int fmt)
{
	struct map_be_dai_private *map_be_dai_priv;
	struct map_private *map_priv;
	unsigned int inf = 0, mask = 0, addr, val;
	...
	case SND_SOC_DAIFMT_CBM_CFM:
		if (cpu_dai->id == 1 || cpu_dai->id == 4)
			mask |= MAP_I2S_MASTER;
		else
			mask |= MAP_DEI2S_MASTER;
		break;
	...
	if (cpu_dai->id == 4) {
		mask |= MAP_PCM_WIDTH_SEL;
		inf &= ~MAP_I2S_MODE_MASK;
		inf &= ~MAP_PCM_WIDTH_SEL;
		inf |= MAP_I2S_MODE_I2S_FORMAT;
	}

	val = map_raw_read(map_priv, addr);
	val &= ~mask;
	val |= inf;
...
struct snd_soc_dai_driver mmp_map_be_dais[] = {
	...
	{
		.name = "map-i2s-spkr-dai",
		.id = 4,
		.playback = {
			.stream_name  = "I2S_SPKR_DL",
			.channels_min = 1,
			.channels_max = 128,
			.rates = MMP_MAP_RATES,
			.formats = MMP_MAP_FORMATS,
