...
static int pxa_88pm805_hifi_prepare(struct snd_pcm_substream *substream)
{
	...
	if (has_feat_hifi_gssp_record()) {
		if (ssp_master)
			sscr1_hifi = (sscr1_hifi & (~0x03000000)) | 0x00B01DC0;
		else
			sscr1_hifi |= 0x03B01DC0;

		sscr0_hifi |= 0x41D0003F;
		__raw_writel(sscr0_hifi, ssp->mmio_base + SSCR0);
		__raw_writel(sscr1_hifi, ssp->mmio_base + SSCR1);
