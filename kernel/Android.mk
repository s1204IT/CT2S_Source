...
$(PRODUCT_OUT)/kernel: $(KERNEL_IMAGE)
	$(hide) md5sum $(PRODUCT_OUT)/$^ | cut -b 1-32 | tr -d '\n' > $(PRODUCT_OUT)/$^.md5
	$(hide) openssl rsautl -sign -inkey $(dir $(DEFAULT_SYSTEM_DEV_CERTIFICATE))uboot_private.pem -in $(PRODUCT_OUT)/$^.md5 -out $(PRODUCT_OUT)/$^.rsa
	$(hide) cat $(PRODUCT_OUT)/$^.rsa >> $(PRODUCT_OUT)/$^
	ln -sf $^ $@
...
ifeq ($(ARCH),arm64)
	$(UBOOT_OUTPUT)/tools/mkimage -A arm64 -O linux -C gzip -a 0x01080000 -e 0x01080000 -n "$(TARGET_DEVICE) linux" -d $(KERNEL_OUTPUT)/arch/$(ARCH)/boot/Image.gz $(KERNEL_OUTPUT)/arch/$(ARCH)/boot/$(KERNEL_IMAGE)
else
