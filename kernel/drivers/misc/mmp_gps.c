			   ...
			   struct mmp_gps_info *info)
{
	 ...
	 * platform, like PXA1928. Egnore it if not set.
	 */
	info->ldo_en_gpio =
	...
	if (of_get_property(np, "marvell,use-vcxo-out", NULL)) {
		info->vcxo_out = clk_get(NULL, "vcxo_out");
		if (info->vcxo_out == NULL) {
			dev_err(dev, "could not get vcxo_out clock.\n");
			goto out;
		}
		clk_prepare_enable(info->vcxo_out);
	}
...
static void gps_ldo_control(struct mmp_gps_info *info, int enable)
{
	...
	if (enable) {
		...
		if (regulator_is_enabled(info->gps_regulator)) {
			ret = regulator_disable(info->gps_regulator);
			if (ret)
				dev_err(info->dev, "disable gps ldo fail!\n");
		} else {
			dev_info(info->dev, "ldo already disabled\n");
		}
	}
