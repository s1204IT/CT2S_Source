...
void __iomem *get_ciu_base_va(void)
{
	...
	if (unlikely(!ciu_virt_addr))
		ciu_virt_addr = iomap_register("mrvl,mmp-ciu");
	return ciu_virt_addr;
}
EXPORT_SYMBOL(get_ciu_base_va);

void __iomem *get_audio_base_va(void)
{
	static void __iomem *aduio_virt_addr;
	if (unlikely(!aduio_virt_addr))
		aduio_virt_addr = iomap_register("mrvl,mmp-aud-gp");
	return aduio_virt_addr;
}
EXPORT_SYMBOL(get_audio_base_va);

void __iomem *get_apbsp_base_va(void)
{
	static void __iomem *apbsp_virt_addr;
	if (unlikely(!apbsp_virt_addr))
		apbsp_virt_addr = iomap_register("mrvl,mmp-apb-spare");
	return apbsp_virt_addr;
}
EXPORT_SYMBOL(get_apbsp_base_va);

void __iomem *get_squ_base_va(void)
{
	static void __iomem *squ_virt_addr;
	if (unlikely(!squ_virt_addr))
		squ_virt_addr = iomap_register("mrvl,mmp-squ");
	return squ_virt_addr;
}
EXPORT_SYMBOL(get_squ_base_va);
