...
static void pxa_uart_receive_dma_cb(void *data)
{
	...
	count = DMA_BLOCK - dma_state.residue;
	if (up->port.sysrq) {
		if (up->break_abnormal && count > DMA_BURST_SIZE) {
			/*
			 * A burst size zeros will be received before the real
			 * sysrq character on pxa1928 B0.
			 * These redundant zeros are useless and need to be removed.
			 */
			count -= DMA_BURST_SIZE;
			tmp += DMA_BURST_SIZE;
		}
		while (count > 0) {
			if (!uart_handle_sysrq_char(&up->port, *tmp)) {
			       ...
			       struct uart_pxa_port *sport)
{
	...
	of_property_read_u32(np, "break-abnormal", &sport->break_abnormal);
	return 0;
}
