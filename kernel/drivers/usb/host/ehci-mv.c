...
static int mv_hsic_private_init(struct ehci_hcd_mv *ehci_mv)
{
#ifdef CONFIG_USB_EHCI_MV_U2H_HSIC
#define HSIC_INT 0x28
#define HSIC_CONNECT_INT 1
#define HSIC_INT_HS_READY 2
	struct ehci_regs *ehci_regs;
	u32 hsic_int;
	u32 status;
	int count;

	void __iomem *phy_base = ehci_mv->phy->io_priv;

	/*disable connect irq*/
	/*hsic_int = __raw_readl(phy_base + HSIC_INT);
	hsic_int &= ~HSIC_INT_CONNECT_INT_EN;
	__raw_writel(hsic_int, phy_base + HSIC_INT);*/

	ehci_regs = (struct ehci_regs *)ehci_mv->op_regs;
	...
	do {
		...
		&& !(hsic_int & (HSIC_INT_HS_READY | HSIC_CONNECT_INT)));
	if (count <= 0) {
...
static int mv_ehci_private_init(struct platform_device *pdev,
			struct ehci_hcd_mv *ehci_mv)
{
	const struct of_device_id *match;

	match = of_match_device(mv_ehci_dt_match, &pdev->dev);
	if (!match->data)
		return 0;

	if (PXA_HSIC == (unsigned long)match->data)
		return mv_hsic_private_init(ehci_mv);
...
static int mv_ehci_probe(struct platform_device *pdev)
{
	...
	retval = mv_ehci_private_init(pdev, ehci_mv);
	if (retval) {
		dev_err(&pdev->dev,
			"HSIC private init failed with err %d\n", retval);
		goto err_set_vbus;
	}
...
static const struct of_device_id mv_ehci_dt_match[] = {
	...
	{.compatible = "marvell,pxa-u2hhsic", .data = (void *)PXA_HSIC },
	{},
};
