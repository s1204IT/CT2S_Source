...
#define PHY_28NM_HSIC_CONNECT_INT 1
...
static int _mv_usb2_phy_hsic_init(struct mv_usb2_phy *mv_phy)
{
	...
	dev_info(&pdev->dev, "hsic_phy_init !!!\n");
	...
	loops = 0;
	while (!(readl(base + PHY_28NM_HSIC_PLL_CTRL2) & (0x1 << PHY_28NM_HSIC_H2S_PLL_LOCK_SHIFT))) {
		mdelay(1);
		loops++;
		if (loops > 100) {
	...
	*/
	...
	/* Make sure PHY Calibration is ready */
	loops = 0;
	while (!(readl(base + PHY_28NM_HSIC_IMPCAL_CAL) & (0x1 << PHY_28NM_HSIC_H2S_IMPCAL_DONE_SHIFT))) {
		mdelay(1);
		loops++;
		if (loops > 100) {
	...
	loops = 0;
	while (!(readl(base + PHY_28NM_HSIC_INT) & (0x1 << PHY_28NM_HSIC_CONNECT_INT))) {
		mdelay(1);
		loops++;
		if (loops > 100) {
...
static int _mv_usb2_phy_28nm_charger_detect(struct mv_usb2_phy *mv_phy)
{
	...
	/* Clear charger detection interrupt */
	reg32 = (1 << PHY_28NM_CTRL1_CHRG_DTC_OUT_SHIFT_28);
	writel(reg32, base + PHY_28NM_CTRL_REG1);
