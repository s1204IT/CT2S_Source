...
static void handle_charger_detection(struct mv_udc *udc)
{
	...
	if (work_pending(&udc->delayed_charger_work.work)) {
		...
		flush_delayed_work(&udc->delayed_charger_work);
	}

	/* SDP and NONE_STANDARD chargers need some delay to confirm */
		dev_info(&udc->dev->dev, "charger type detection: %s\n",
	...
	schedule_delayed_work(&udc->delayed_charger_work, enum_delay);
}

#ifdef CONFIG_USB_GADGET_CHARGE_ONLY
static int mv_double_check_charger(struct mv_udc *udc)
{
#define PHY_DIG_REG1               0x20
#define PHY_NOVBUS_DPDM00_MASK (0x1 << 10)
#define OTGSC_DP_MASK (0x1 << 4)
#define PORTSC_LINE_STATUS_MASK (0x3 << 10)

	struct mv_usb2_phy *phy;
	void __iomem *base;
	unsigned int val_dig;
	unsigned int val_otgsc;
	unsigned int val;

	phy = container_of(udc->phy, struct mv_usb2_phy, phy);
	base = phy->base;

	val_dig = readl(base + PHY_DIG_REG1);
	writel((val_dig & (~PHY_NOVBUS_DPDM00_MASK)), base + PHY_DIG_REG1);

	val_otgsc = readl(&udc->op_regs->otgsc);
	writel((val_otgsc | OTGSC_DP_MASK), &udc->op_regs->otgsc);

	usleep_range(100, 200);

	val = readl(&udc->op_regs->portsc[0]);
	if (PORTSC_LINE_STATUS_MASK != (PORTSC_LINE_STATUS_MASK & val))
		udc->charger_type = DEFAULT_CHARGER;

	writel(val_otgsc, &udc->op_regs->otgsc);
	writel(val_dig, base + PHY_DIG_REG1);
...
static int mv_udc_vbus_session(struct usb_gadget *gadget, int is_active)
{
	...
	if (udc->driver && udc->softconnect && udc->vbus_active) {
		...
		if (!udc->active && udc->charger_type != DCP_CHARGER) {
			spin_unlock_irqrestore(&udc->lock, flags);
			goto out;
		}
...
#ifdef CONFIG_USB_GADGET_CHARGE_ONLY
	if (is_charge_only_mode() && udc->charger_type == DCP_CHARGER)
		mv_double_check_charger(udc);
#endif /* CONFIG_USB_GADGET_CHARGE_ONLY */

	/* SDP or CDP need transfer data, hold wake lock */
	if (udc->charger_type == SDP_CHARGER) {
		dev_info(&udc->dev->dev, "update charger type: %s\n",
				charger_type(udc->charger_type));
		call_charger_notifier(udc);
	} else if (udc->charger_type == DCP_CHARGER) {
		pm_qos_update_request(&udc->qos_idle,
				PM_QOS_CPUIDLE_BLOCK_DEFAULT_VALUE);
		/* leave some delay for charger driver to do something */
		pm_wakeup_event(&udc->dev->dev, 1000);
...
#ifdef CONFIG_USB_GADGET_CHARGE_ONLY
	if (is_charge_only_mode())
		usb_phy_force_dp_dm(udc->phy, true);
#endif /* CONFIG_USB_GADGET_CHARGE_ONLY */
...
#ifdef CONFIG_USB_GADGET_CHARGE_ONLY
	if (is_charge_only_mode())
		usb_phy_force_dp_dm(udc->phy, false);
#endif /* CONFIG_USB_GADGET_CHARGE_ONLY */
