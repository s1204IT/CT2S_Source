			       ...
			       struct device_node *parent)
{
	...
	/*
	 * pxa1928 Zx/A0/B0 use GIC rather than ICU for irq handling
	 * so dts should guarantee the status as disabled if "mrvl,pxa1928-intc" was defined.
	 */
	if (!of_device_is_available(node))
		return -ENODEV;
...
IRQCHIP_DECLARE(pxa1928_intc, "mrvl,pxa1928-intc", pxa1928_of_init);
#endif
