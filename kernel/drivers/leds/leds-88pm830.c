...
#define PM830_CLS_SW_DEL_200		(1 << 7)
...
#define PM830_MAX_FLASH_CURRENT		(1600)		/* mA */
#define PM830_MAX_TORCH_CURRENT		(400)		/* mA */

static unsigned gpio_en = 0;
			   ...
			   enum led_brightness value)
{
	...
	led->brightness = led->max_current * value / LED_FULL;
	if (value == 0)
		led->brightness = 0;
			 ...
			 struct pm830_led_pdata *pdata)
{
	...
	if (ret)
		return ret;
	ret = of_property_read_u32(np, "max-flash-current", &pdata->max_flash_current);
	if (ret)
		pdata->max_flash_current = PM830_MAX_FLASH_CURRENT;
	ret = of_property_read_u32(np, "max-torch-current", &pdata->max_torch_current);
	if (ret)
		pdata->max_torch_current = PM830_MAX_TORCH_CURRENT;
	return 0;
}
...
static int pm830_led_probe(struct platform_device *pdev)
{
	...
	if (pdev->id == PM830_FLASH_LED) {
		...
		led->max_current = pdata->max_flash_current;
		led->max_current = (led->max_current > PM830_MAX_FLASH_CURRENT) ?
			PM830_MAX_FLASH_CURRENT : led->max_current;
	} else {
		strncpy(led->name, "torch", MFD_NAME_SIZE - 1);
		led->max_current = pdata->max_torch_current;
		led->max_current = (led->max_current > PM830_MAX_TORCH_CURRENT) ?
			PM830_MAX_TORCH_CURRENT : led->max_current;
	}
	...
	/* set CF_OUT OV/UV/OC detection delay to 200us */
	regmap_update_bits(chip->regmap, PM830_CAMERA_FLASH3,
			PM830_CLS_SW_DEL_200, PM830_CLS_SW_DEL_200);
...
static int pm830_led_remove(struct platform_device *pdev)
{
	...
	if (led) {
		...
		cancel_delayed_work_sync(&led->delayed_work);
		/*
		 * led_classdev_unregister will call brightness_set,
		 * which in turn schedule led->work. so we must flush
		 * work after that, but not cancel work.
		 */
		flush_work(&led->work);
	}
...
static void pm830_led_shutdown(struct platform_device *pdev)
{
	pm830_led_remove(pdev);
}
