...
#ifdef LM3532_DEBUG
static unsigned trace_brightness = 0;
				...
				size_t size)
{
	...
	if ((value & pwm_mask) == pwm_value)
		pr_info("%s: %d, PWM value is as requested; "
...
static int lm3532_configure(struct lm3532_data *driver_data)
{
	...
	if (driver_data->revision == 0xF6) {
		...
		lm3532_write_reg(driver_data->client,
				 ...
				 driver_data->pdata->feedback_en_val, __func__);
		lm3532_write_reg(driver_data->client,
...
static int lm3532_probe_dt(struct i2c_client *client)
{
	...
	if (of_property_read_u32(np, "feedback_en_val",
		...
		dev_warn(&client->dev, "feedback_en_val will be setted to its default value\n");
			...
			const struct i2c_device_id *id)
{
	...
	if (!pdata->feedback_en_val)
		pdata->feedback_en_val = LM3532_FEEDBACK_ENABLE_REG_ILED1_EN;
	...
	if (driver_data->revision == 0xF6)
		pdata->output_cfg_val = LM3532_OUTPUT_CFG_REG_DEF_VAL;
	else
		pdata->output_cfg_val = LM3532_OUTPUT_CFG_REG_DEF_VAL_REV1;

#ifdef CONFIG_OF
	if (!of_property_read_u32(np, "output_cfg_val",
					&pdata->output_cfg_val))
		dev_warn(&client->dev, "output_cfg_val overriden by of data\n");
#endif
	...
	if (ret) {
		pr_err("%s: unable to create pwm device file "
		       "for %s: %d\n", __func__, LM3532_LED_NAME, ret);
	...
	if (driver_data->pdata->flags & LM3532_CONFIG_ALS) {
		...
		if (ret == 0)
			...
			pr_err("request_irq %d for lm3532 "
			       "failed: %d\n", client->irq, ret);
			ret = -EINVAL;
...
static const struct dev_pm_ops lm3532_pm_ops = {
	SET_RUNTIME_PM_OPS(lm3532_runtime_suspend, lm3532_runtime_resume, NULL)
};
