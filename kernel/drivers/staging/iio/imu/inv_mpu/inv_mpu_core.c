	...
	const struct i2c_device_id *id)
{
	...
	struct mpu_platform_data *platform_data;
	...
	platform_data->irq_gpio =
		of_get_named_gpio(np, "inven,irq-gpios", 0);
	...
	if (gpio_is_valid(platform_data->irq_gpio)) {
		...
		result = gpio_request(platform_data->irq_gpio,
						"inv_mpu_irq_gpio");
		if (result) {
		...
		result = gpio_direction_input(platform_data->irq_gpio);
		if (result) {
...
out_free:
	...
	if (st->plat_data.power_off) {
		result = st->plat_data.power_off(&st->plat_data);
		if (result < 0) {
			dev_err(&client->dev,
					"power_off failed: %d\n", result);
			return result;
