...
/*
 * Current devfreq can't support dynamic governor switch
 * because data not updated accordingly
 */
#if 0
...
static ssize_t show_available_governors(struct device *d,
				    ...
				    char *buf)
{
	/* One devfreq device only can support one governor */
#if 0
	struct devfreq_governor *tmp_governor;
	ssize_t count = 0;
	...
	return count;
#else
	return show_governor(d, attr, buf);
#endif
}
...
static ssize_t store_qos_min_freq(struct device *dev,
				  struct device_attribute *attr,
				  const char *buf, size_t count)
{
	struct devfreq *df = to_devfreq(dev);
...
static ssize_t show_qos_max_freq(struct device *dev,
				 struct device_attribute *attr,
				 char *buf)
{
...
static ssize_t store_qos_max_freq(struct device *dev,
				  struct device_attribute *attr,
				  const char *buf, size_t count)
{
	struct devfreq *df = to_devfreq(dev);
...
static struct device_attribute devfreq_attrs[] = {
	...
	__ATTR(qos_min_freq, S_IRUGO | S_IWUSR, show_qos_min_freq,
	       store_qos_min_freq),
	__ATTR(qos_max_freq, S_IRUGO | S_IWUSR, show_qos_max_freq,
	       store_qos_max_freq),
	{ },
};
