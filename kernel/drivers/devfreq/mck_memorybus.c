...
#define DDR_DEVFREQ_UPTHRESHOLD 50
				  ...
				  void *reg, unsigned int ver)
{
	...
	if (ver == MCK4) {
		...
		put_fc_spinlock();
	} else if (ver == MCK5 && cpu_is_pxa1928()) {
		spin_lock_irqsave(&pxa1928_mckdfc_lock, flags);
		disable_cp_ddrdfc();
		dfcval = readl(ddrdfc_base);
		while (dfcval & 0x1 << 28) {
			udelay(10);
			if (temp++ > 500) {
				pr_err("ddr takes too long!, DDRDFC_CTRL 0x%x\n",
						readl(ddrdfc_base));
				enable_cp_ddrdfc();
				spin_unlock_irqrestore(&pxa1928_mckdfc_lock,
						flags);
				return;
			}
			dfcval = readl(ddrdfc_base);
...
static int ddr_devfreq_probe(struct platform_device *pdev)
{
	...
	struct resource *irqres, *ddrdfcres;
	...
	ddrdfcres = platform_get_resource(pdev, IORESOURCE_MEM, 1);
	if (!ddrdfcres)
		return -ENODEV;
	ddrdfc_base = ioremap(ddrdfcres->start, resource_size(ddrdfcres));
