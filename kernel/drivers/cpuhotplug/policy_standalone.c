...
struct cpu_time_info {
	...
	unsigned long load;		/* Not consider freq */
	unsigned long avg_load;		/* Consider freq */
};
		   ...
		   unsigned long nr_rq_min, unsigned int cpu_rq_min)
{
	...
	if (avg_load < threshold[nr_online_cpu - 1][LOW]) {
		...
		if ((avg_load >=
		    ...
		    (avg_load <=
		     threshold[nr_online_cpu - 1][HIGH]))
			nop_time = 0;
		else if ((avg_load >=
				     ...
				     unsigned long val, void *data)
{
	...
	for_each_online_cpu(i) {
		...
		/* update idle time */
		idle_time = (unsigned long)cputime64_sub(cur_idle_time,
			    tmp_info->prev_cpu_idle);
		tmp_info->prev_cpu_idle = cur_idle_time;

		/* update wall time */
		wall_time = (unsigned long)cputime64_sub(cur_wall_time,
		...
		/* update avg_load */
		tmp_info->avg_load += ((wall_time - idle_time) * freq->old);
	}
...
static void __ref hotplug_timer(struct work_struct *work)
{
	...
	for_each_online_cpu(i) {
		struct cpu_time_info *tmp_info;
		cputime64_t cur_wall_time, cur_idle_time;
		unsigned long idle_time, wall_time, total_wall_time;
		...
		total_wall_time = (unsigned long)cputime64_sub(cur_wall_time,
		...
		/* update avg_load on current freq */
		tmp_info->avg_load += (wall_time - idle_time) * cur_freq;

		/* get real avg_load(xx%) */
		tmp_info->avg_load = (100 * tmp_info->avg_load)
					/ (total_wall_time * max_performance);

		trace_pxa_hp_single(i, get_cpu_nr_running(i), tmp_info->load,
				    (tmp_info->avg_load * max_performance / 100));
	...
	if ((flag_hotplug == HOTPLUG_IN) &&
		...
		if (NUM_ON_CPUS < qos_max) {
			...
			if (current->policy == SCHED_NORMAL) {
				sched_setscheduler(current, SCHED_FIFO, &param_rt);
				normal = true;
			}
