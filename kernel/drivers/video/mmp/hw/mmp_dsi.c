...
static void dsi_set_video_panel(struct mmp_dsi *dsi,int enable)
{
	struct mmp_dsi_regs *dsi_regs = dsi->reg_base;
...
static void dsi_set_on(struct mmp_dsi *dsi, int status)
{
	...
	if (status) {
		...
		/* prepare panel */
		if (path && path->panel && path->panel->prepare)
			path->panel->prepare(path->panel);
...
#ifdef CONFIG_MMP_PANEL_VX5B1D
		/* unprepare panel */
		if (path && path->panel && path->panel->unprepare)
			path->panel->unprepare(path->panel);
#endif /* CONFIG_MMP_PANEL_VX5B1D */
...
static void dsi_set_status(struct mmp_dsi *dsi, int status)
{
	...
	if (status == MMP_ON_REDUCED) {
		...
		/* need to reset DSI controller */
		dev_warn(dsi->dev, "MMP_ON_REDUCED -> MMP_ON\n");
		status = MMP_ON;
	}
#endif /* CONFIG_MMP_PANEL_VX5B1D */
