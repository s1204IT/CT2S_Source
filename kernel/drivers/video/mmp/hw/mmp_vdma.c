...
static void vdma_vsync_cb(struct mmp_vdma_info *vdma_info)
{
	...
	unsigned long flags, flags_com_reg;
	u32 tmp;

	if (DISP_GEN4(vdma->version)) {
		spin_lock_irqsave(&vdma->common_regs_lock, flags_com_reg);
		...
		spin_unlock_irqrestore(&vdma->common_regs_lock, flags_com_reg);
	}
}
...
static void mmp_vdma_regs_recovery(struct mmp_vdma *vdma)
{
	...
	int i = 0, clk_ctrl_offset;
	unsigned long flags, flags_com_reg;
	u32 tmp;

	if (vdma->regs_store) {
		clk_ctrl_offset =
			offsetof(struct mmp_vdma_reg, clk_ctrl) / sizeof(u32);
		spin_lock_irqsave(&vdma->common_regs_lock, flags_com_reg);
		/* recovery registers */
		while (i < clk_ctrl_offset) {
			writel_relaxed(vdma->regs_store[i],
				vdma->reg_base + i * 4);
			i++;
		}
		/* vdma channel clock need to be enabled when recovery channel
		 * settings, the registers before clk_ctrl_offset were all
		 * common registers which were independent for clk control */
		writel_relaxed(0x0, &vdma_reg->clk_ctrl);
		i++;
		while (i < vdma->regs_len) {
			writel_relaxed(vdma->regs_store[i],
				vdma->reg_base + i * 4);
		...
		/* recovery vdma channel clock control after vdma channel
		 * settings were recoveried. */
		tmp = readl_relaxed(&vdma_reg->clk_ctrl);
		for (i = 0; i < vdma->vdma_channel_num; i++) {
			vdma_info = &vdma->vdma_info[i];
			spin_lock_irqsave(&vdma_info->status_lock, flags);
			if (vdma_info->status != VDMA_ON)
				tmp |= VDMA_CLK_DIS(vdma_info->vdma_id);
			if (vdma_info->status == VDMA_TO_DISABLE_TRIGGERED ||
					vdma_info->status == VDMA_TO_DISABLE) {
				/* Commonly the vdma_info status should be set
				 * as VDMA_DISABLED in vdma_vsync_cb (in irq
				 * handler). For the scenario that display
				 * module was power down when suspend before
				 * irq handler triggered, vdma_info status will
				 * be kept as VDMA_TO_DISABLE_TRIGGERED and irq
				 * was disabled by default when resume, so we
				 * need to correct its status here. */
				vdma_info->status = VDMA_DISABLED;
			}
			spin_unlock_irqrestore(&vdma_info->status_lock, flags);
		}
...
{
	struct mmp_vdma *vdma;

	vdma = container_of(self, struct mmp_vdma, fb_notif);
...
static int vdma_register_fb(struct mmp_vdma *vdma)
{
	memset(&vdma->fb_notif, 0, sizeof(vdma->fb_notif));
	vdma->fb_notif.notifier_call = fb_notifier_callback;
	return fb_register_client(&vdma->fb_notif);
}
...
static void vdma_init(struct mmp_mach_vdma_info *mi)
{
	...
	vdma->name = mi->name;
	spin_lock_init(&vdma->common_regs_lock);
