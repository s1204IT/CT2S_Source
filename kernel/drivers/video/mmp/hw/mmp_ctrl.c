		...
		for (id = 0; id < ctrl->path_num; id++) {
			...
			if (!(vsync_done & vsync_imask(id))) {
				...
				if (atomic_read(&path->irq_en_count)
					&& (atomic_dec_and_test(&path->irq_en_count))
					&& (!(path->irq_count.vsync_check)))
...
static void overlay_set_win(struct mmp_overlay *overlay, struct mmp_win *win)
{
	...
	memcpy(&old_win, &overlay->win, sizeof(struct mmp_win));
	...
	if (path_plat->ctrl->disp_aclk_dfc && win->xsrc != old_win.xsrc)  {
		/*
		 * Do disp axi clk dfc when source size of layer has changed.
		 * Source size increase then increase disp axi clk immediantely.
		 * Source size reduce then decrease disp axi clk later.
		 */
		if (win->xsrc > old_win.xsrc)
			ctrl_aclk_dfc(path);
		else if (win->xsrc < old_win.xsrc)
			schedule_delayed_work(&path->work, HZ/20);
	}
}
...
static void dmafetch_onoff(struct mmp_overlay *overlay, int on)
{
	...
	/*
	 * The control bit of LCD shadow control register is shared by grahic layer
	 * and video layer, so it may be set by the other layer(graphic layer) when
	 * we try to enable one layer(video layer). This can make some shadow registers
	 * of this layer take effect unexpectly and involve unexpect issue, here make
	 * sure the commit of previous shadow registers setting has already been taken
	 * effect before enabling this layer to avoid this situation.
	 */
	if (DISP_GEN4(path_to_ctrl(path)->version)) {
		while (readl_relaxed(ctrl_regs(path) + LCD_SHADOW_CTRL) & SHADOW_TRIG(path->id)) {
			usleep_range(1000, 2000);
			count++;

			if (count >= 20) {
				pr_warn("WARN: The previous commit of LCD shadow registers doesn't take effect now\n");
				break;
			}
		}
	}

	/* Increase disp axi clk first if dmafecth on */
	if (path_plat->ctrl->disp_aclk_dfc && on)
		ctrl_aclk_dfc(path);
	...
	/* Decrease disp axi clk later if dmafecth off */
	if (path_plat->ctrl->disp_aclk_dfc && !on)
		schedule_delayed_work(&path->work, HZ/20);
}
...
static void path_set_timing(struct mmp_path *path)
{
	...
	if (clk && path_clk_req) {
		...
		mode->pixclock_freq = clk_get_rate(clk);
		mode->refresh = mode->pixclock_freq / (total_x * total_y);
	}
}
...
static void overlay_do_onoff(struct mmp_overlay *overlay, int status)
{
	...
	if (status == MMP_ON_REDUCED) {
		...
		for (i = 0; i < path->overlay_num; i++) {
			path->overlays[i].status = on;
			dmafetch_onoff(&path->overlays[i], on);
		}
...
static void overlay_set_status(struct mmp_overlay *overlay, int status)
{
	...
	case MMP_OFF_DMA:
		...
		if (overlay->ops->trigger) {
			struct mmp_shadow *shadow = overlay->shadow;
			if (shadow && shadow->ops->set_dmaonoff)
...
static int overlay_set_addr(struct mmp_overlay *overlay, struct mmp_addr *addr)
{
	...
	if (overlay->path->y_mirror &&
		!DISP_GEN4(path_to_ctrl(overlay->path)->version)) {
		stride = pixfmt_to_stride(overlay->win.pix_fmt);
		addr->phys[0] += MMP_XALIGN(overlay->path->mode.xres) *
			(MMP_YALIGN(overlay->path->mode.yres) - 1) *
			stride;
...
static void path_set_mode(struct mmp_path *path, struct mmp_mode *mode)
{
	...
	/* change pixclock freq to real clock freq we can get */
	mode->pixclock_freq = path->mode.pixclock_freq;
	mode->refresh = path->mode.refresh;
}
...
static inline void path_axi_dfc_work(struct work_struct *work)
{
	struct mmp_path *path = (struct mmp_path *)container_of(work,
			struct mmp_path, work.work);
		...
		struct mmp_mach_path_config *config)
{
	...
	path_info->y_mirror = config->y_mirror;
	...
	if (ctrl->disp_aclk_dfc)
		INIT_DELAYED_WORK(&path->work, path_axi_dfc_work);
...
/*
 * This callback gets called when suspend or resume happen inside a
 * framebuffer driver, and we're switching display power as well ...
 */
				 ...
				 unsigned long event, void *data)
{
	...
	ctrl = container_of(self, struct mmphw_ctrl, fb_notif);
...
static int ctrl_register_fb(struct mmphw_ctrl *ctrl)
{
	memset(&ctrl->fb_notif, 0, sizeof(ctrl->fb_notif));
	ctrl->fb_notif.notifier_call = fb_notifier_callback;
	return fb_register_client(&ctrl->fb_notif);
}
...
static int mmphw_probe(struct platform_device *pdev)
{
	...
	if (IS_ENABLED(CONFIG_OF)) {
		...
		if (of_get_property(np, "marvell,disp-aclk-dfc", NULL))
			ctrl->disp_aclk_dfc = 1;
		...
		for_each_child_of_node(np, child_np) {
			...
			dt_paths_config[i].y_mirror = 0;
			of_property_read_u32(child_np,
					"marvell,y-mirror",
					&dt_paths_config[i].y_mirror);
	...
	if (ctrl->disp_aclk_dfc)
		ctrl_aclk_dfc_init(&pdev->dev);

#ifdef CONFIG_MMP_DISP_DFC
	ctrl_dfc_init(&pdev->dev);
#endif

	ctrl_register_fb(ctrl);
