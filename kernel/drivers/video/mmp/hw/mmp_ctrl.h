...
#define DISP_AXI_TBL (MAX_RES*MAX_LAY)
/*
...
struct mmphw_path_plat {
	...
	struct pm_qos_request   qos_idle_d1p;
	u32 path_config;
	u32 link_config;
};

/* mmp ctrl describes mmp controller related info
 *disp_axi_clk_tbl: all axi clock freq sort asending in this table
 *disp_axi_clk_parent_tbl: parent of axi clock freq table
 */
struct mmphw_ctrl {
	...
	/* Display AXI clock (ACLK) DFC */
	unsigned int disp_aclk_dfc:1;
	unsigned int disp_aclk_div_width;
	struct mutex disp_aclk_lock;
	/* All aclk rate will be sort asending in the table */
	unsigned int disp_aclk_options;
	unsigned int disp_aclk_tbl[30];
	const char *disp_aclk_parent_tbl[30];
