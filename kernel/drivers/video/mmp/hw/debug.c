		...
		char *buf)
{
	...
	s += sprintf(buf + s, "vdma_path");
	s += sprintf(buf + s, "\tvdma->overlay_id");
	s += sprintf(buf + s, "\tvdma->status");
	s += sprintf(buf + s, "\tvdma->sram_paddr");
	s += sprintf(buf + s, "\tvdma->sram_vaddr");
	s += sprintf(buf + s, "\tvdma->sram_size\n");
	for (i = 0; i < vdma->vdma_channel_num; i++) {
		vdma_info = &vdma->vdma_info[i];
		s += sprintf(buf + s, "vdma%d  ", i);
		s += sprintf(buf + s, "\t\t%d", vdma_info->overlay_id);
		s += sprintf(buf + s, "\t\t\t%d", vdma_info->status);
		s += sprintf(buf + s, "\t\t0x%x", vdma_info->sram_paddr);
		s += sprintf(buf + s, "\t\t\t0x%lx", vdma_info->sram_vaddr);
		s += sprintf(buf + s, "\t\t\t%zd\n", vdma_info->sram_size);
	}
	s += sprintf(buf + s, "vdma->status: 0-free;1-requesed;2-allocated;3-released;"
				"4-on;5-to_disable;6-to_disable_triggered;7-disabled\n");
	...
	s += sprintf(buf + s, " - vdma setting\n"
			...
			"[1/0](allocat/free) <[sram_size](Byte)>\n");
		...
		const char *buf, size_t size)
{
	...
	if ('s' == buf[0]) {
		...
		mmp_vdma_reg = (int)simple_strtoul(vol, NULL, 16);
		pr_info("reg @ 0x%x: 0x%x\n", mmp_vdma_reg,
			readl_relaxed(vdma->reg_base + mmp_vdma_reg));
	} else if ('0' == buf[0] && 'x' == buf[1]) {
		/* set the register value */
		tmp = (int)simple_strtoul(buf, NULL, 16);
		writel_relaxed(tmp, vdma->reg_base + mmp_vdma_reg);
		pr_info("set reg @ 0x%x: 0x%x\n", mmp_vdma_reg,
			readl_relaxed(vdma->reg_base + mmp_vdma_reg));
	}
		...
		const char *buf, size_t size)
{
	...
	if ('-' == buf[0]) {
		memcpy(vol, buf + 1, size - 1);
		apical_reg_addr = (int)simple_strtoul(vol, NULL, 16);
		pr_info("reg @ 0x%x: 0x%x\n", apical_reg_addr,
			readl_relaxed(apical->reg_base + apical_reg_addr));
	} else if ('0' == buf[0] && 'x' == buf[1]) {
		/* set the register value */
		tmp = (int)simple_strtoul(buf, NULL, 16);
		writel_relaxed(tmp, apical->reg_base + apical_reg_addr);
		pr_info("set reg @ 0x%x: 0x%x\n", apical_reg_addr,
			readl_relaxed(apical->reg_base + apical_reg_addr));
	}
