...
int fb_overlay_ioctl(struct fb_info *info, unsigned int cmd, unsigned long arg)
{
	int ret;
...
int fb_overlay_compat_ioctl(struct fb_info *info, unsigned int cmd, unsigned long arg)
{
	int ret;
	mutex_lock(&mmpfb_runtime_lock);
	ret = mmpfb_compat_ioctl(info, cmd, arg);
	mutex_unlock(&mmpfb_runtime_lock);
	return 0;
...
static struct fb_ops mmpfb_overlay_ops = {
	...
	.fb_ioctl	= fb_overlay_ioctl,
#ifdef CONFIG_COMPAT
	.fb_compat_ioctl	= fb_overlay_compat_ioctl,
#endif
