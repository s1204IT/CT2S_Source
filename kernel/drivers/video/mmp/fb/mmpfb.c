...
DEFINE_MUTEX(mmpfb_runtime_lock);
		...
		struct fb_info *info)
{
	...
	/*
	 * Pan_display will enable irq once in a frame.
	 * IRQ will be disabled in irq handler, they are pairs.
	 */
...
#ifdef CONFIG_MMP_PANEL_VX5B1D
	static int last_status = -1;
	if (last_status == blank) {
		dev_warn(
			info->dev,
			"mmpfb blank: already set to %d\n", blank);
		return 0;
	}
	last_status = blank;
#endif /* CONFIG_MMP_PANEL_VX5B1D */
...
static int mmpfb_probe(struct platform_device *pdev)
{
	...
	if (modes_num > 0) {
		if (DISP_GEN4(mmp_path_get_version(fbi->path))
			...
			info->var.yres_virtual = MMP_YALIGN(info->var.yres) * fbi->buffer_num;
			fbi->fb_size = PAGE_ALIGN(info->var.xres_virtual * info->var.yres_virtual
					* info->var.bits_per_pixel / 8);
		}
...
static int mmpfb_runtime_suspend(struct device *dev)
{
	...
	if (!lock_fb_info(info))
		return -ENODEV;
	mutex_lock(&mmpfb_runtime_lock);
	atomic_set(&fbi->path->commit, 0);

	mmpfb_power(fbi, MMP_OFF);

	/* Call fb_notifier to power off ctrl after power off fb */
	fb_set_suspend(info, 1);

	mutex_unlock(&mmpfb_runtime_lock);
	unlock_fb_info(info);
...
static int mmpfb_runtime_resume(struct device *dev)
{
	...
	/* Call fb_notifier to power on ctrl before power on fb */
	fb_set_suspend(info, 0);
