...
/* Get refresh rate: also know as FPS */
static ssize_t mmpfb_fps_show(struct device *dev,
...
DEVICE_ATTR(fps, S_IRUGO, mmpfb_fps_show, NULL);
...
int mmpfb_vsync_notify_init(struct mmpfb_info *fbi)
{
	...
	ret = device_create_file(fbi->dev, &dev_attr_fps);
	if (ret < 0) {
