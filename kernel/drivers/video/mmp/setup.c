...
static int __init panel_setup(char *str)
{
	...
	else if (!strcmp(str, "WXGA_P")) {
		fb_size = PAGE_ALIGN(MMP_XALIGN_GEN4(800) *
				MMP_YALIGN_GEN4(1280) * 4 * buf_num);
	}
	else if (!strcmp(str, "WXGA")) {
		fb_size = PAGE_ALIGN(MMP_XALIGN(1280) *
				MMP_YALIGN(800) * 4 * buf_num);
	}
	else {
