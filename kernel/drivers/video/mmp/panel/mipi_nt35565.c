...
static void nt35565_panel_onoff(struct mmp_panel *panel, int on)
{
	if (on) {
		mmp_panel_dsi_ulps_set_on(panel, 0);
		mmp_panel_dsi_tx_cmd_array(panel, nt35565_display_on_cmds,
			ARRAY_SIZE(nt35565_display_on_cmds));
	} else {
		mmp_panel_dsi_tx_cmd_array(panel, nt35565_display_off_cmds,
...
static void nt35565_set_status(struct mmp_panel *panel, int status)
{
	...
	if (status_is_on(status)) {
		...
		if (plat->plat_onoff)
			...
			nt35565_panel_power(panel, skip_on, 1);
		if (!skip_on)
			nt35565_panel_onoff(panel, 1);
	} else if (status_is_off(status)) {
		nt35565_panel_onoff(panel, 0);
		/* power off */
		if (plat->plat_onoff)
			plat->plat_onoff(0);
		else
