/*
 * drivers/video/mmp/panel/mipi_hx8394_ips3p5071.c
...
static void hx8394_ips3p5071_panel_on(struct mmp_panel *panel, int status)
{
	if (status) {
		mmp_panel_dsi_ulps_set_on(panel, 0);
		mmp_panel_dsi_tx_cmd_array(panel,
				hx8394_ips3p5071_display_on_cmds,
				ARRAY_SIZE(hx8394_ips3p5071_display_on_cmds));
	} else {
		mmp_panel_dsi_tx_cmd_array(panel, hx8394_ips3p5071_display_off_cmds,
...
static void hx8394_ips3p5071_set_status(struct mmp_panel *panel, int status)
{
	...
	if (status_is_on(status)) {
		...
		if (!skip_on)
			hx8394_ips3p5071_panel_on(panel, 1);
	} else if (status_is_off(status)) {
		hx8394_ips3p5071_panel_on(panel, 0);
		/* power off */
		if (plat->plat_onoff)
			plat->plat_onoff(0);
		else
