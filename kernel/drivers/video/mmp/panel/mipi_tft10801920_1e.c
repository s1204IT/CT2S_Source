...
static void tft_1e_panel_on(struct mmp_panel *panel)
{
	mmp_panel_dsi_tx_cmd_array(panel, tft_1e_display_on_cmds,
...
static void tft_1e_panel_power(struct mmp_panel *panel, int skip_on, int on)
{
	...
	if (on) {
		if (!skip_on) {
			/* lcd_rst_n going to zero*/
			gpio_direction_output(lcd_rst_n, 0);
			usleep_range(10000, 12000);

			/* LCD_AVDD+ and LCD_AVDD- OFF */
...
static void tft_1e_set_status(struct mmp_panel *panel, int status)
{
	...
	if (status_is_on(status)) {
		...
		if (plat->plat_onoff)
			...
			tft_1e_panel_power(panel, skip_on, 1);
		if (!skip_on)
			tft_1e_panel_on(panel);
	...
	if (IS_ENABLED(CONFIG_OF)) {
		...
		if (ret < 0) {
			...
			pr_debug("panel %s has ddrfreq min request: %u\n",
				 panel_tft_1e.name, panel_tft_1e.ddrfreq_qos);
		}
#endif
	} else {
