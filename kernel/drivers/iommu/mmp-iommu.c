...
static void mmp_iommu_reset(struct iommu_domain *domain)
{
	 ...
	 * also enalbe context fault report.
	 */
	writel_relaxed(0x00000061, cb0_base + SMMU_CB_SCTLR);
	...
	reg = readl_relaxed(gr0_base + SMMU_SCR0);
	/* Enable fault reporting */
	reg |= 0x36;
	writel_relaxed(reg, gr0_base + SMMU_SCR0);
...
static irqreturn_t mmp_iommu_fault_handler(int irq, void *data)
{
	...
	void __iomem *gr0_base, *cb0_base;
	u32 gfsr, gfar, cb_fsr, cb_far, cb_fsynr0;

	gr0_base = drvdata->regbase;
	cb0_base = gr0_base + 0x8000;

	gfsr = readl_relaxed(gr0_base + SMMU_GFSR);
	gfar = readl_relaxed(gr0_base + SMMU_GFAR);
	cb_fsr = readl_relaxed(cb0_base + SMMU_CB_FSR);
	cb_far = readl_relaxed(cb0_base + SMMU_CB_FAR);
	cb_fsynr0 = readl_relaxed(cb0_base + SMMU_CB_FSYNR0);

	writel_relaxed(gfsr, gr0_base + SMMU_GFSR);
	writel_relaxed(cb_fsr, cb0_base + SMMU_CB_FSR);

	dev_info(drvdata->dev, "mmp_iommu_fault_handler\n");
	dev_info(drvdata->dev, "SMMU_GFSR = 0x%08x\n", gfsr);
	dev_info(drvdata->dev, "SMMU_GFAR = 0x%08x\n", gfar);
	dev_info(drvdata->dev, "SMMU_CB_FSR = 0x%08x\n", cb_fsr);
	if (cb_fsr & SMMU_CB_FSR_SS) {
		dev_info(drvdata->dev,
			"The context is stalled, terminate the stall transaction\n");
		writel_relaxed(0x1, cb0_base + SMMU_CB_RESUME);
	}
	dev_info(drvdata->dev, "SMMU_CB_FAR = 0x%08x\n", cb_far);
	dev_info(drvdata->dev, "SMMU_CB_FSYNR0 = 0x%08x\n", cb_fsynr0);
...
static void mmp_iommu_debug_regdump(struct seq_file *s)
{
	struct mmp_iommu_drvdata *drvdata;
	void __iomem *gr0_base;
