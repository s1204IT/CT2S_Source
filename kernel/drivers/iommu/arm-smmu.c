...
static u32 restore_val[50];
static void __iomem *restore_addr[50];
static int windex;

#define writel_relaxed_save(val, addr)		\
	do {					\
		writel_relaxed(val, addr);	\
		restore_val[windex] = val;		\
		restore_addr[windex] = addr;		\
		windex++;			\
	} while (0)
...
void restore_smmu(void)
{
	int i;
	struct arm_smmu_device *curr;

	/* arm_smmu_device_cfg_probe called before reset */
	writel(restore_val[0], restore_addr[0]);

	/* reset is a standalone sequence, could be called w/o
	 * changing s/w state machine, thus seperate it from
	 * smmu restore buffer
	 */
	list_for_each_entry(curr, &arm_smmu_devices, list) {
		arm_smmu_device_reset(curr);
	}

	for (i = 1; i < windex; i++)
		writel(restore_val[i], restore_addr[i]);
}
...
static void arm_smmu_tlb_inv_context(struct arm_smmu_cfg *cfg)
{
	...
	enable_smmu_power_domain(smmu->dev, 1, NO_RESTORE_SMMU);
	...
	enable_smmu_power_domain(smmu->dev, 0, NO_RESTORE_SMMU);
}
...
static void arm_smmu_init_context_bank(struct arm_smmu_domain *smmu_domain)
{
	...
	writel_relaxed_save(reg, gr1_base + ARM_SMMU_GR1_CBAR(root_cfg->cbndx));
	...
	if (smmu->version > 1) {
		...
		if (stage1)
			writel_relaxed_save(reg, cb_base + ARM_SMMU_CB_TTBCR2);
	}

	/* TTBR0 */
	reg = __pa(root_cfg->pgd);
	writel_relaxed_save(reg, cb_base + ARM_SMMU_CB_TTBR0_LO);
	reg = (phys_addr_t)__pa(root_cfg->pgd) >> 32;
	if (stage1)
		reg |= ARM_SMMU_CB_ASID(root_cfg) << TTBRn_HI_ASID_SHIFT;
	writel_relaxed_save(reg, cb_base + ARM_SMMU_CB_TTBR0_HI);
	...
	writel_relaxed_save(reg, cb_base + ARM_SMMU_CB_TTBCR);
	...
	if (stage1) {
		...
		writel_relaxed_save(reg, cb_base + ARM_SMMU_CB_S1_MAIR0);
	}
	...
	reg |= SCTLR_E;
#endif
	writel_relaxed_save(reg, cb_base + ARM_SMMU_CB_SCTLR);
}
					  ...
					  struct arm_smmu_master *master)
{
	...
	for (i = 0; i < master->num_streamids; ++i) {
		...
		writel_relaxed_save(reg,
				gr0_base + ARM_SMMU_GR0_SMR(smrs[i].idx));
	}
				      ...
				      struct arm_smmu_master *master)
{
	...
	for (i = 0; i < master->num_streamids; ++i) {
		...
		writel_relaxed_save(s2cr, gr0_base + ARM_SMMU_GR0_S2CR(idx));
	}
...
static int arm_smmu_attach_dev(struct iommu_domain *domain, struct device *dev)
{
	...
	enable_smmu_power_domain(device_smmu->dev, 1, NO_RESTORE_SMMU);
	...
	enable_smmu_power_domain(device_smmu->dev, 0, NO_RESTORE_SMMU);
...
static int arm_smmu_device_cfg_probe(struct arm_smmu_device *smmu)
{
	...
	if (id & ID0_SMS) {
		...
		writel_relaxed_save(smr, gr0_base + ARM_SMMU_GR0_SMR(0));
...
static void smmu_debug_regdump(struct seq_file *s, struct device *dev, void __iomem *gr0_base)
{
	enable_smmu_power_domain(dev, 1, NO_RESTORE_SMMU);
	seq_printf(s, "\n[iommu] virt(%p)\n", gr0_base);
	...
	enable_smmu_power_domain(dev, 0, NO_RESTORE_SMMU);
}
...
static int smmu_reg_show(struct seq_file *s, void *v)
{
	...
	for (node = rb_first(&smmu->masters); node; node = rb_next(node)) {
		...
		master = container_of(node, struct arm_smmu_master, node);
		if (master->smrs)
			smmu_debug_regdump(s, smmu->dev, gr0_base);
	}
