...
static int __cpufreq_remove_dev(struct device *dev, struct subsys_interface *sif)
{
	...
	if (cpus == 1) {
		...
		if (cpufreq_driver->target) {
			ret = __cpufreq_governor(data, CPUFREQ_GOV_START);
			if (ret) {
				pr_err("%s: Failed to start governor\n", __func__);
				return ret;
			}
			    ...
			    unsigned int relation)
{
	...
	/* In pxa1928 cpufreq implementation, the policy->cur is the real
	 * core frequency, comment following two line code that could
	 * transfer the target_freq from governor to lower level driver */
