...
#ifdef CONFIG_PM
static int st21nfca_suspend(struct device *dev)
{
	struct st21nfca_dev *st21nfca_dev;
...
static int st21nfca_resume(struct device *dev)
{
	struct st21nfca_dev *st21nfca_dev;
	struct i2c_client *client = to_i2c_client(dev);
...
static SIMPLE_DEV_PM_OPS(st21nfca_pm_ops, st21nfca_suspend, st21nfca_resume);
#endif
