...
int phys_in_cma(phys_addr_t phys)
{
	struct cma *cma = dma_contiguous_default_area;
	phys_addr_t fm = __pfn_to_phys(cma->base_pfn);
	phys_addr_t to = __pfn_to_phys(cma->base_pfn + cma->count);

	return ((phys >= fm) && (phys < to)) ? 1 : 0;
}
...
static int cma_info_show(struct seq_file *s, void *unused)
{
	...
	seq_printf(s, "CMA Region: pfn(0x%lx:0x%lx) phy(0x%pa:0x%pa)\n",
