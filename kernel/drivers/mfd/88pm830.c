...
static int device_gpadc_init_830(struct pm830_chip *chip)
{
	...
	/* 3. PREBIAS_GP0/1 can't lower than 3, it's from silicon design team */
			   ...
			   struct pm830_platform_data *pdata)
{
	...
	if (chip->version < PM830_B2_VERSION) {
		dev_err(chip->dev, "not supported 88pm830 version! [0x%x]\n", chip->version);
		BUG_ON(1);
	}
	...
	/* disable the battery detection circuit */
	regmap_update_bits(map, PM830_GPADC_CONFIG2, PM830_BD_EN, 0);

	/* set PREG_SMGO_EN to improve battery priority mode */
	regmap_update_bits(map, PM830_CHG_CTRL15, PREG_SMGO_EN, PREG_SMGO_EN);

	/* set battery priority mode */
	regmap_update_bits(map, PM830_BAT_CTRL2, PM830_BAT_PRTY_EN, PM830_BAT_PRTY_EN);
...
void pm830_shutdown(struct i2c_client *client)
{
	struct pm830_chip *chip = i2c_get_clientdata(client);
