...
static int device_irq_init_805(struct pm80x_chip *chip)
{
	...
	data = (chip->irq_mode) ?
		PM805_STATUS0_INT_WRITE_CLEAR : PM805_STATUS0_INT_READ_CLEAR;
...
void pm805_shutdown(struct i2c_client *client)
{
	unsigned int irq_base;
	struct pm80x_chip *chip = i2c_get_clientdata(client);

	irq_base = regmap_irq_chip_get_base(chip->irq_data);
	disable_irq(irq_base);
}
