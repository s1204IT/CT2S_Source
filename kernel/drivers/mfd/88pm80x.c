...
#define PM800_REG_NUM			(0xef)
#define PM805_REG_NUM			(0x9b)
...
const struct regmap_config pm800_regmap_config = {
	.reg_bits = 8,
	.val_bits = 8,
	.max_register = PM800_REG_NUM,
};

const struct regmap_config pm805_regmap_config = {
	.reg_bits = 8,
	.val_bits = 8,
	.max_register = PM805_REG_NUM,
};
...
int pm80x_init(struct i2c_client *client)
{
	...
	switch (chip->type) {
	case CHIP_PM800:
		ret = regmap_reinit_cache(chip->regmap, &pm800_regmap_config);
		break;
	case CHIP_PM822:
		ret = regmap_reinit_cache(chip->regmap, &pm800_regmap_config);
		break;
	case CHIP_PM86X:
		...
		break;
	case CHIP_PM805:
		ret = regmap_reinit_cache(chip->regmap, &pm805_regmap_config);
		break;
	default:
