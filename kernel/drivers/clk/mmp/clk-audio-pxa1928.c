...
struct pxa1928_clk_audio_pll_table {
...
struct clk_audio {
	...
	int		enabled;
	spinlock_t	*lock;
	struct pxa1928_clk_audio_pll_table *freq_table;
};
...
struct popular_reference_clock_freq refclock_pxa1928[] = {
...
struct post_divider_phase_interpolator pdiv_pxa1928_8k[] = {
...
struct post_divider_phase_interpolator pdiv_pxa1928_11k[] = {
/* audio  over  clkout  prediv  postdiv *
 * -----  ----  -----   -----   ------- */
...
static long __pxa1928_audio_get_rate_table(struct clk_hw *hw,
		struct pxa1928_clk_audio_pll_table *freq_tbl,
			unsigned long drate, unsigned long prate)
{
	...
	if ((drate % 8000) == 0) { /* 8KHz family sample range */
		...
		div_p = &pdiv_pxa1928_8k[0];
		map_size = ARRAY_SIZE(pdiv_pxa1928_8k);
	} else if ((drate % 11025) == 0) { /* 11KHz family sample range */
		/*output 135.475M VCO Clock*/
		div_p = &pdiv_pxa1928_11k[0];
		map_size = ARRAY_SIZE(pdiv_pxa1928_11k);
	} else {
		pr_err("%s : Incorrect sample rate.....\r\n", __func__);
		return -EINVAL;
	}

	for (i = 0; i < ARRAY_SIZE(refclock_pxa1928); i++) {

		/* find audio pll setting */
		ref_p = &refclock_pxa1928[i];
		...
		unsigned long *prate)
{
	...
	if (!__pxa1928_audio_get_rate_table(hw, &freq_tbl, drate, *prate))
		return -EINVAL;
