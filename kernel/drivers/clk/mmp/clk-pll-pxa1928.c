...
/* Because of that the vco_max was 3.2G
 * when pll output larger than 1.6G, post divider could be only set as 1 */
#define PLL_MAX_FOR_DIV2	(1600000000)
...
#define PLL2_PWR_CTRL_SHIFT	(18)
#define PLL2_PWR_CTRL_MASK	(3 << PLL2_PWR_CTRL_SHIFT)
#define PLL2_PWR_CTRL(x)	((x) << PLL2_PWR_CTRL_SHIFT)

#define to_pxa1928_clk_vco(_hw) container_of(_hw, \
					struct pxa1928_clk_pll_vco, hw)
#define to_pxa1928_clk_pll(_hw) container_of(_hw, \
					struct pxa1928_clk_pll_out, hw)
#define readl_mpmu(pll, offset) readl_relaxed(pll->mpmu_base + \
		pll->params->offset)
#define readl_apmu(pll, offset) readl_relaxed(pll->apmu_base + \
		pll->params->offset)
#define writel_mpmu(val, pll, offset) writel_relaxed(val, pll->mpmu_base + \
		pll->params->offset)
#define writel_apmu(val, pll, offset) writel_relaxed(val, pll->apmu_base + \
		pll->params->offset)
		...
		struct pxa1928_clk_pll_vco_table *freq_tbl)
{
	...
	desired_ssc_amp = 25;
	/* spread mode */
	if (vco->flags & PXA1928_PLL_SSC_DOWN_MODE) {
		vco_avg = freq_tbl->output_rate *
			(1000 - desired_ssc_amp / 2) / 1000;
		pll_ctrl3.bit.ssc_mode = 1;
		/* ssc_freq_div = fvco/(4*2*desired_modu_freq) */
		pll_ctrl3.bit.ssc_freq_div =
			(vco_avg >> 3) / desired_modu_freq;
	} else {
		vco_avg = freq_tbl->output_rate;
		pll_ctrl3.bit.ssc_mode = 0;
		/* ssc_freq_div = fvco/(4*4*desired_modu_freq) */
		pll_ctrl3.bit.ssc_freq_div =
			(vco_avg >> 4) / desired_modu_freq;
	}
		...
		struct pxa1928_clk_pll_vco_table *freq_tbl)
{
	...
	spin_lock_irqsave(vco->lock, flags);
	pll_ctrl2.val = readl_mpmu(vco, reg_pll_ctrl2);
	pll_ctrl2.bit.pi_en = 1;
	pll_ctrl2.bit.clk_det_en = 1;
...
static int pxa1928_clk_pll_vco_enable(struct clk_hw *hw)
{
	...
	if (__pxa1928_clk_vco_is_enabled(vco)) {
		spin_lock_irqsave(vco->lock, flags);
		pll_cr.val = readl_mpmu(vco, reg_pll_cr);
		if (vco->flags & PXA1928_PLL_PWR_CTRL) {
			val = readl_apmu(vco, reg_pll_gate);
			val &= ~PLL2_PWR_CTRL_MASK;
			val &= ~(PLL_MASK(vco->params->gate_width) <<
					vco->params->gate_shift);
			val |= PLL2_PWR_CTRL(2);
			writel_apmu(val, vco, reg_pll_gate);
			pll_cr.bit.ctrl = 1;
			pll_cr.bit.sw_en = 1;
			writel_mpmu(pll_cr.val, vco, reg_pll_cr);
		} else if (pll_cr.bit.ctrl) {
	...
	if (freq_tbl->offset_en ||
		(vco->flags & (PXA1928_PLL_SSC_EN | PXA1928_PLL_SSC_ALWAYS_EN)))
		__pxa1928_vco_set_pi(vco, freq_tbl);

	if (freq_tbl->offset_en)
		__pxa1928_vco_set_offset(vco, freq_tbl);

	if (vco->flags & (PXA1928_PLL_SSC_EN | PXA1928_PLL_SSC_ALWAYS_EN))
		__pxa1928_vco_set_ssc(vco, freq_tbl);

	return 0;
}

static void __pxa1928_vco_offset_disable(struct pxa1928_clk_pll_vco *vco)
{
	union pxa1928_pll_ctrl2 pll_ctrl2;
	unsigned long flags;
...
static void __pxa1928_vco_pi_disable(struct pxa1928_clk_pll_vco *vco)
{
	union pxa1928_pll_ctrl2 pll_ctrl2;
	unsigned long flags;
	...
	spin_lock_irqsave(vco->lock, flags);
	pll_ctrl2.val = readl_mpmu(vco, reg_pll_ctrl2);
	pll_ctrl2.bit.pi_en = 0;
	pll_ctrl2.bit.clk_det_en = 0;
	writel_mpmu(pll_ctrl2.val, vco, reg_pll_ctrl2);
	spin_unlock_irqrestore(vco->lock, flags);
}

static void __pxa1928_vco_ssc_disable(struct pxa1928_clk_pll_vco *vco)
{
	union pxa1928_pll_ctrl2 pll_ctrl2;
	unsigned long flags;
...
static void pxa1928_clk_pll_vco_disable(struct clk_hw *hw)
{
	...
	if (freq_tbl->offset_en)
		__pxa1928_vco_offset_disable(vco);
	if (vco->flags & PXA1928_PLL_SSC_ALWAYS_EN)
		return;
	if (freq_tbl->offset_en || (vco->flags & PXA1928_PLL_SSC_EN))
		__pxa1928_vco_pi_disable(vco);
	if (vco->flags & PXA1928_PLL_SSC_EN)
		__pxa1928_vco_ssc_disable(vco);
}
		...
		unsigned long rate, unsigned long prate)
{
	...
	if (rate < PLL_MAX_FOR_DIV2 && !div_sel)
		/*
