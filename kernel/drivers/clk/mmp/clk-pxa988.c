...
void __init pxa988_pll_init(void *mpmu_base, void *apbs_base, void *apmu_base)
{
	...
	if (ddr_mode)
		...
		pll2_vco_params.default_vco_rate = 1595 * MHZ_TO_HZ;
...
static struct cpu_rtcwtc cpu_rtcwtc_tsmc_1L88_a0c_1p5g_p6top11[] = {
	{.max_pclk = 624, .l1_rtc = 0x02222222, .l2_rtc = 0x00002221,},
	{.max_pclk = 1066, .l1_rtc = 0x02666666, .l2_rtc = 0x00006265,},
	{.max_pclk = 1482, .l1_rtc = 0x02EEEEEE, .l2_rtc = 0x0000E2ED,},
};
...
static struct cpu_rtcwtc cpu_rtcwtc_umc_1L88_a0c[] = {
	{.max_pclk = 312, .l1_rtc = 0x02222222, .l2_rtc = 0x00002221,},
	{.max_pclk = 800, .l1_rtc = 0x02666666, .l2_rtc = 0x00006265,},
	{.max_pclk = 1248, .l1_rtc = 0x02AAAAAA, .l2_rtc = 0x0000A2A9,},
	{.max_pclk = 1482, .l1_rtc = 0x02EEEEEE, .l2_rtc = 0x0000E2ED,},
};
...
static struct dfc_level_reg_offset  dfc_level_reg_off = {
	...
	.ddr_clk_div_shift = 2,
	.ddr_clk_div_width = 3,
	.mc_table_num_shift = 5,
	.mc_table_num_width = 3,
	.volt_level_shift = 8,
	.volt_level_width = 3,
};
	...
	void __iomem *ciu_base, void __iomem *dmcu_base)
{
	...
	unsigned int ddr_clk_flag = 0, hwdfc_flag = 0, ui_foundry;
	int i;
	...
	if (cpu_is_pxa1L88()) {
		ddr_clk_flag = MMP_DDR_HWDFC_FEAT | MMP_DDR_RATE_AP_ONLY;
		hwdfc_flag = MMP_DDR_HWDFC_WR;
		ui_foundry = get_foundry();
		if (cpu_is_pxa1L88_a0c()) {
			if (ui_foundry == 2) {
				core_params->cpu_rtcwtc_table = cpu_rtcwtc_umc_1L88_a0c;
				core_params->cpu_rtcwtc_table_size =
					ARRAY_SIZE(cpu_rtcwtc_umc_1L88_a0c);
			} else if (is_1p5G_chip &&
				   ((get_profile_pxa1L88() == 0) ||
				    ((get_profile_pxa1L88() >= 6) &&
				     (get_profile_pxa1L88() <= 11)))) {
				core_params->cpu_rtcwtc_table =
					cpu_rtcwtc_tsmc_1L88_a0c_1p5g_p6top11;
				core_params->cpu_rtcwtc_table_size =
					ARRAY_SIZE(cpu_rtcwtc_tsmc_1L88_a0c_1p5g_p6top11);
			}
		}
	} else {
...
struct xpu_rtcwtc gc2d_rtcwtc_umc_1L88_a0c[] = {
	{.max_rate = 416000000, .rtcwtc = 0x00055555,},
	{.max_rate = 624000000, .rtcwtc = 0x00066666,},
};
...
struct plat_clk_list peri_clk_list[] = {
	...
	{ "sdhci-pxav3.1", "sdh-baseclk", 208000000},
...
static void __init clk_peri_init(void __iomem *apmu_base, void __iomem *ciu_base)
{
	...
	if (cpu_is_pxa1L88()) {
		...
		if (cpu_is_pxa1L88_a0c() && (ui_foundry == 2)) {
			gc2df_parm->rwtctbl = gc2d_rtcwtc_umc_1L88_a0c;
			gc2df_parm->rwtctblsize =
				ARRAY_SIZE(gc2d_rtcwtc_umc_1L88_a0c);
		}
	} else {
	...
	if (cpu_is_pxa1L88_a0c())
		gc2d_fclk_reg.reg_rtcwtc = ciu_base + GPU2D_XTC;
	clk = mmp_clk_register_peri("gc2d", gc_par,
		gc_par_size, 0, apmu_base + APMU_GC2D,
		&gc2d_lock, gc2df_parm, &gc2d_fclk_reg);
...
void __init clk_misc_init(void *apmu_base, void *ciu_base)
{
	...
	if (cpu_is_pxa1L88())
		__raw_writel(0x00011111, ciu_base + GPU_XTC);
	else
...
void __init pxa988_clk_init(void)
{
	...
	tmp32 = USB_AXI_CLK_EN | USB_AXI_CLK_RST;
#ifdef CONFIG_USB_GADGET_CHARGE_ONLY
	if (has_feat_force_dpdm()) {
		tmp32 = __raw_readl(apmu_base + APMU_USB);
		__raw_writel(tmp32 | USB_AXI_CLK_RST, apmu_base + APMU_USB);
		/*
		 * pxa1L88_A0, pxa1088, pxa988 should not reset AXI clock
		 * if reset, after enter UDR, USB phy will pull D+ high
		 */
		tmp32 = USB_AXI_CLK_EN;
	}
#endif /* #ifdef CONFIG_USB_GADGET_CHARGE_ONLY */

	clk = mmp_clk_register_apmu("usb", "usb_pll",
				apmu_base + APMU_USB, tmp32, &clk_lock);
