...
static int periph_clk_is_enabled(struct clk_hw *hw)
{
	struct clk_peri *peri = to_clk_peri(hw);
	unsigned int regval, enable_bit = peri->reg_info->enable_val;

	regval = __raw_readl(peri->reg_addr);
	if (regval & enable_bit)
		return 1;
	return 0;
}
...
struct clk_ops peri_clk_ops = {
	...
	.is_enabled = periph_clk_is_enabled,
...
static int pxa1928_gc_vpu_clk_prepare_zx(struct clk_hw *hw)
{
	struct clk_peri *peri = to_clk_peri(hw);
...
static int pxa1928_gc_vpu_clk_prepare_ax(struct clk_hw *hw)
{
	struct clk_peri *peri = to_clk_peri(hw);
	...
	if (peri->params->comclk) {
		...
		if (peri->params->sibling_clk) {
			if (__clk_is_enabled(peri->params->sibling_clk)) {
				comclk_rate = hw->clk->rate > peri->params->sibling_clk->rate ?
					hw->clk->rate : peri->params->sibling_clk->rate;
				clk_set_rate(peri->params->comclk, comclk_rate);
			} else
				clk_set_rate(peri->params->comclk, hw->clk->rate);
		}
	}
...
static int pxa1928_gc_vpu_clk_prepare(struct clk_hw *hw)
{
	struct clk_peri *peri = to_clk_peri(hw);
	...
	if (pxa1928_is_zx)
		return pxa1928_gc_vpu_clk_prepare_zx(hw);
	else
		return pxa1928_gc_vpu_clk_prepare_ax(hw);
}

static void pxa1928_gc_vpu_clk_unprepare_zx(struct clk_hw *hw)
{
	struct clk_peri *peri = to_clk_peri(hw);
	struct clk *clk_depend;
	int i;
...
static void pxa1928_gc_vpu_clk_unprepare_ax(struct clk_hw *hw)
{
	struct clk_peri *peri = to_clk_peri(hw);
	...
	if (peri->params->comclk) {
		...
		if (peri->params->sibling_clk) {
			if (__clk_is_enabled(peri->params->sibling_clk) &&
					peri->params->sibling_clk->rate < hw->clk->rate) {
				clk_set_rate(peri->params->comclk, peri->params->sibling_clk->rate);
			}
		}
	}
...
static void pxa1928_gc_vpu_clk_unprepare(struct clk_hw *hw)
{
	struct clk_peri *peri = to_clk_peri(hw);

	if (pxa1928_is_zx)
		pxa1928_gc_vpu_clk_unprepare_zx(hw);
	else
		pxa1928_gc_vpu_clk_unprepare_ax(hw);
...
static int pxa1928_gc_vpu_clk_enable_zx(struct clk_hw *hw)
{
	struct clk_peri *peri = to_clk_peri(hw);
...
static int pxa1928_gc_vpu_clk_enable_ax(struct clk_hw *hw)
{
	struct clk_peri *peri = to_clk_peri(hw);
	void __iomem *reg_addr = peri->reg_addr;
	void __iomem *reg_debug = peri->reg_info->reg_debug;
	unsigned long flags = 0;
	unsigned int regval;
	unsigned int divval, shadow_divval;

	if (peri->lock)
		spin_lock_irqsave(peri->lock, flags);

	if (pxa1928_is_a0()) {
		/* set divider filed to enable clk */
		shadow_divval = (peri->reg_info->shadow_regval >>
				peri->reg_info->div_shift) &
			peri->reg_info->div_mask;
		...
		if (shadow_divval) {
			regval &= ~(peri->reg_info->div_mask <<
					peri->reg_info->div_shift);
			regval |= shadow_divval << peri->reg_info->div_shift;
			__raw_writel(regval, reg_addr);
		} else if (!divval && !shadow_divval) {
			regval &= ~(peri->reg_info->div_mask <<
					peri->reg_info->div_shift);
			regval |= 2 << peri->reg_info->div_shift;
			__raw_writel(regval, reg_addr);
		}
	} else {
		CLK_SET_BITS(peri->reg_info->enable_val, 0);
		if (reg_debug) {
			regval = readl(reg_debug);
			regval |= (1 << 26);
			__raw_writel(regval, reg_debug);
		}
	}
...
static int pxa1928_gc_vpu_clk_enable(struct clk_hw *hw)
{
	trace_clock_enable(hw->clk->name, 1, 0);

	if (pxa1928_is_zx)
		return pxa1928_gc_vpu_clk_enable_zx(hw);
	else
		return pxa1928_gc_vpu_clk_enable_ax(hw);
}

static void pxa1928_gc_vpu_clk_disable_zx(struct clk_hw *hw)
{
	struct clk_peri *peri = to_clk_peri(hw);
...
static void pxa1928_gc_vpu_clk_disable_ax(struct clk_hw *hw)
{
	struct clk_peri *peri = to_clk_peri(hw);
	void __iomem *reg_addr = peri->reg_addr;
	void __iomem *reg_debug = peri->reg_info->reg_debug;
	unsigned int regval;
	unsigned long flags = 0, vpu_flags = 0, divmsk;

	if (peri->lock)
		spin_lock_irqsave(peri->lock, flags);

	if (pxa1928_is_a0()) {
		/* clear divider filed to disable clk */
		divmsk = peri->reg_info->div_mask <<
			peri->reg_info->div_shift;
...
static void pxa1928_gc_vpu_clk_disable(struct clk_hw *hw)
{
	trace_clock_disable(hw->clk->name, 0, 0);

	if (pxa1928_is_zx)
		pxa1928_gc_vpu_clk_disable_zx(hw);
	else
		pxa1928_gc_vpu_clk_disable_ax(hw);
}
...
static void __do_gc_vpu_dfc(struct clk_hw *hw, unsigned int new_val,
		unsigned int en_mask, unsigned int rtcwtc, unsigned int fc_flag)
{
	struct clk_peri *peri = to_clk_peri(hw);
	unsigned int regval = 0, clk_is_on = 0, fcreqmsk;
	void __iomem *fc_reg = peri->fc_reg;
	void __iomem *reg_addr = peri->reg_addr;
	void __iomem *reg_rtcwtc = peri->reg_info->reg_rtcwtc;
	unsigned long gc_flags = 0, vpu_flags = 0;
	...
	if (!pxa1928_is_zx) {
		...
		if (reg_rtcwtc && (fc_flag == LOW2HIGH
				|| fc_flag == HIGH2LOW)) {
			pr_debug("updating rtc/wtc, clk name[%s], rtcwtc[%x]\n",
					hw->clk->name, rtcwtc);
			regval = __raw_readl(reg_rtcwtc);
			regval &= 0xffff;
			regval |= rtcwtc;
			__raw_writel(regval, reg_rtcwtc);
			/* update rtc/wtc */
			regval = __raw_readl(reg_addr);
			regval |= 1 << peri->reg_info->rtcwtc_shift;
			__raw_writel(regval, reg_addr);
		}
...
static int gc_vpu_dfc(struct clk_hw *hw, struct clk *best_parent,
		...
		unsigned int rtcwtc, unsigned int fc_flag)
{
	unsigned int muxmask, divmask;
	unsigned int muxval, divval;
	unsigned int regval;
	...
	__do_gc_vpu_dfc(hw, regval, en_mask, rtcwtc, fc_flag);
...
static int pxa1928_gc_vpu_clk_setrate(struct clk_hw *hw, unsigned long rate,
		unsigned long parent_rate)
{
	unsigned long old_rate, comclk_rate;
	unsigned int index = 0;
	struct periph_clk_tbl *periph_pp = NULL;
	struct clk_peri *peri = to_clk_peri(hw);
	...
	periph_pp = __clk_periph_get_opptr(peri, rate, &index);
	rtcwtc = periph_pp->rtcwtc;

	peri->reg_info->shadow_regval &= ~(peri->reg_info->div_mask <<
					peri->reg_info->div_shift);
	peri->reg_info->shadow_regval |= periph_pp->div_val
					<< peri->reg_info->div_shift;

	if (rate == old_rate)
		return 0;

	if (old_rate <= peri->params->cutoff_freq &&
			rate > peri->params->cutoff_freq)
		fc_flag = LOW2HIGH;
	else if (old_rate > peri->params->cutoff_freq &&
			rate <= peri->params->cutoff_freq)
		fc_flag = HIGH2LOW;

	gc_vpu_dfc(hw, periph_pp->parent,
			periph_pp->src_val, periph_pp->div_val,
			peri->reg_info->enable_val, rtcwtc, fc_flag);
	...
	if (peri->params->comclk && periph_pp->comclk_rate) {
		if (!peri->params->sibling_clk)
			...
			if (!__clk_is_enabled(peri->params->sibling_clk))
				...
				comclk_rate = periph_pp->comclk_rate > peri->params->sibling_clk->rate ?
					periph_pp->comclk_rate : peri->params->sibling_clk->rate;
				clk_set_rate(peri->params->comclk, comclk_rate);
			}
		}
	}
...
static unsigned long __gc_vpu_get_rate(struct clk_hw *hw)
{
	struct clk *cur_parent;
	struct clk_peri *peri = to_clk_peri(hw);
	unsigned int mux, div = 1, shadow_divval;
	...
	__clk_get_mux_div(hw, &mux, &div);
	/*
	 * in pxa1928 A0, we clear divider filed to 0 in PMUA_XXX_CLKCTRL to
	 * disable clk, so if we try to get clk rate by qurying hardware
	 * register after disable this clk, we need to use shadow value of
	 * divider, otherwise it will be zero and may cause kernel panic.
	 */
	if (div == 0)
		div = shadow_divval;
	cur_parent = __clk_mux_to_parent(hw, mux);
...
static unsigned long pxa1928_gc_vpu_clk_recalc_rate(struct clk_hw *hw,
					unsigned long parent_rate)
{
	return __gc_vpu_get_rate(hw);
}

/* clk ops for gc and vpu of pxa1928 */
struct clk_ops pxa1928_gc_vpu_clk_ops = {
	.init = periph_clk_init,
	.prepare = pxa1928_gc_vpu_clk_prepare,
	.unprepare = pxa1928_gc_vpu_clk_unprepare,
	.enable = pxa1928_gc_vpu_clk_enable,
	.disable = pxa1928_gc_vpu_clk_disable,
	.round_rate = periph_clk_round_rate,
	.set_rate = pxa1928_gc_vpu_clk_setrate,
	.recalc_rate = pxa1928_gc_vpu_clk_recalc_rate,
	.get_parent = peri_get_parent,
};

static struct clk_ops pxa1928_gc_vpu_clk_gate_ops = {
	.prepare = pxa1928_gc_vpu_clk_prepare,
	.unprepare = pxa1928_gc_vpu_clk_unprepare,
	.enable = pxa1928_gc_vpu_clk_enable_ax,
	.disable = pxa1928_gc_vpu_clk_disable_ax,
};
...
struct clk *pxa1928_clk_register_gc_vpu(const char *name,
		...
		const char **clk_depend, u8 num_depend)
{
	...
	if (params->flags & MMP_PERI_GATE_FLAG)
		init.ops = &pxa1928_gc_vpu_clk_gate_ops;
	else
		init.ops = &pxa1928_gc_vpu_clk_ops;
