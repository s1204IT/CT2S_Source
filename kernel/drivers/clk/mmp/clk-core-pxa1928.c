...
#define DEFAULT_CPU_MAX_SPEED	1900000
...
#define COREFREQ_RTCWTC_SEL_SHIFT		(27)
#define COREFREQ_APBCLK_SHIFT		(20)
#define COREFREQ_ATCLK_SHIFT		(16)
#define COREFREQ_PERIPHCLK_SHIFT	(12)
...
#define APMU_COREAPSS_DEBUG1		(0x3A0)
#define CG_PLL4_EN			(1 << 2)
#define MSK_RTCWTC_LATCH_EN		(1 << 4)

#define KHZ_TO_HZ	(1000)
#define MHZ_TO_KHZ	(1000)
#define PXA1928_4L	2
#define PLL_PARK_INDEX 4
...
static void __iomem *apmu_coreapss_debug1;
...
static struct pll_freq_tb pll4_freq_tb_dis[] = {
...
#endif
	{
		...
		.freq_offset_en = 1,
		.fvco = 2669333333ul,
		.fvco_offseted = 2668000000ul,
		.bypass_en = 0,
		.clk_src_sel = 1,
		.post_div_sel = 1,
		/* after fvco offseted, it should be 1334000000 */
		.pll_clkout = 1334666667ul,
		.diff_post_div_sel = 2,
...
static enum core_clk_sel core_src_zx_maptbl[4] = {
	CORE_PLL1, CORE_PLL2, RESERVE, CORE_PLL2P,
};
...
static enum core_clk_sel core_src_b0_maptbl[4] = {
	CORE_PLL1, CORE_PLL1_1248, CORE_PLL2, RESERVE,
};
...
struct pxa1928_core_opt {
	...
	unsigned int div;
	unsigned int rtc_wtc_hmips;
	unsigned int apbclk_div;
	unsigned int atclk_div;
	unsigned int periphclk_div;
...
static unsigned int pll2_oparray[] = {
	797000, 1057000, 1196000, 1386000, 1508000, 1699000,
};

static struct pxa1928_core_opt core_oparray[] = {
	/*{156000, CORE_PLL1,	4, 0, 8, 3, 4, 1, 624000, 0},*/
	{312000, CORE_PLL1,	2, 0, 8, 3, 4, 2, 624000, 0},
	/*{398000, CORE_PLL2,	2, 0, 8, 3, 4, 4, 797000, 1},
	{531000, CORE_PLL2P,	1, 0, 8, 3, 4, 4, 531000, 3},*/
	{624000, CORE_PLL1,	1, 1, 8, 3, 4, 4, 624000, 0},
	{797000, CORE_PLL2,	1, 1, 8, 3, 4, 4, 797000, 1},
	{1057000, CORE_PLL2,	1, 1, 8, 3, 4, 4, 1057000, 1},
	/*{1386000, CORE_PLL2,	1, 1, 8, 3, 4, 2, 1386000, 1},*/
};

static struct pxa1928_core_opt core_oparray_z2[] = {
	{156000, CORE_PLL1,	4, 0, 8, 3, 4, 2, 624000, 0},
	{312000, CORE_PLL1,	2, 0, 8, 3, 4, 2, 624000, 0},
	{398000, CORE_PLL2,	2, 0, 8, 3, 4, 2, 797000, 1},
	{531000, CORE_PLL2P,	1, 0, 8, 3, 4, 2, 531000, 3},
	{624000, CORE_PLL1,	1, 0, 8, 3, 4, 2, 624000, 0},
	{797000, CORE_PLL2,	1, 0, 8, 3, 4, 2, 797000, 1},
	{1057000, CORE_PLL2,	1, 1, 8, 3, 4, 2, 1057000, 1},
	{1386000, CORE_PLL2,	1, 1, 8, 3, 4, 2, 1386000, 1},
};
...
static struct pxa1928_core_opt core_oparray_a0[] = {
	{156000, CORE_PLL1,	4, 0, 8, 3, 4, 2, 624000, 0},
	{312000, CORE_PLL1,	2, 0, 8, 3, 4, 2, 624000, 0},
	{398000, CORE_PLL2,	2, 0, 8, 3, 4, 2, 797000, 2},
	{624000, CORE_PLL1,	1, 1, 8, 3, 4, 2, 624000, 0},
	{797000, CORE_PLL2,	1, 1, 8, 3, 4, 2, 797000, 2},
	{1057000, CORE_PLL2,	1, 1, 8, 3, 4, 2, 1057000, 2},
	{1196000, CORE_PLL2,	1, 1, 8, 3, 4, 2, 1196000, 2},
	{1386000, CORE_PLL2,	1, 1, 8, 3, 4, 2, 1386000, 2},
	{1508000, CORE_PLL2,	1, 1, 8, 3, 4, 3, 1508000, 2},
};

static struct pxa1928_core_opt core_oparray_b0[] = {
	{312000, CORE_PLL1,	2, 0, 8, 3, 1, 2, 624000, 0, 0x44444400},
	{398000, CORE_PLL2,	2, 0, 8, 3, 1, 2, 797000, 2, 0x44444400},
	{624000, CORE_PLL1,	1, 1, 8, 3, 2, 2, 624000, 0, 0x55555500},
	{797000, CORE_PLL2,	1, 1, 8, 3, 2, 2, 797000, 2, 0x55555500},
	{1057000, CORE_PLL2,	1, 1, 8, 3, 3, 2, 1057000, 2, 0x9999AA00},
	{1248000, CORE_PLL1_1248,	1, 1, 8, 3, 4,  2, 1248000, 1, 0xAAAAAA00},
	{1386000, CORE_PLL2,	1, 1, 8, 3, 4, 2, 1386000, 2, 0xAAAAAA00},
	{1508000, CORE_PLL2,	1, 1, 8, 3, 4, 3, 1508000, 2, 0xAAAAAA00},
	{1699000, CORE_PLL2,	1, 1, 8, 3, 4, 3, 1699000, 2, 0xAAAAAA00},
};
...
static inline void core_set_rwtc(int idx)
{
	...
	if (!cpu_is_pxa1928_z1()) {
		...
		if (pxa1928_is_zx || pxa1928_is_a0()) {
			...
			val |= core_op_array[idx].rtc_wtc;
		}
		writel(val, apmu_coreapss_cfg);
	}
	reg = readl(apmu_coreapss_clkctrl);
	reg &= ~(0x1 << COREFREQ_RTCWTC_SEL_SHIFT);
	reg |= (core_op_array[idx].rtc_wtc_hmips << COREFREQ_RTCWTC_SEL_SHIFT);
...
{
	int div, apbclk, atclk, periphclk, axim, src_sel;
	u32 reg;
	struct pxa1928_core_opt *core_op_array;
	...
	apbclk = core_op_array[idx].apbclk_div;
	atclk = core_op_array[idx].atclk_div;
	periphclk = core_op_array[idx].periphclk_div;
	axim = core_op_array[idx].axim_div;

	trace_pxa_core_clk_chg(CLK_CHG_ENTRY,
			core_op_array[core_idx_restore].freq_khz / MHZ_TO_KHZ,
			core_op_array[idx].freq_khz / MHZ_TO_KHZ);
	if (idx > core_idx_restore) {
		reg = readl(apmu_coreapss_clkctrl);

		reg &= ~(0x1f << COREFREQ_APBCLK_SHIFT);
		reg |= (apbclk << COREFREQ_APBCLK_SHIFT);

		reg &= ~(0x7 << COREFREQ_ATCLK_SHIFT);
		reg |= (atclk << COREFREQ_ATCLK_SHIFT);

		reg &= ~(0x7 << COREFREQ_PERIPHCLK_SHIFT);
		reg |= (periphclk << COREFREQ_PERIPHCLK_SHIFT);
	...
	trace_pxa_core_clk_chg(CLK_CHG_EXIT,
			core_op_array[core_idx_restore].freq_khz / MHZ_TO_KHZ,
			core_op_array[idx].freq_khz / MHZ_TO_KHZ);
				...
				unsigned long prate)
{
	...
	case CORE_PLL1_1248:
		...
		break;
	case CORE_PLL2:
		if (src == CORE_PLL1 || src == CORE_PLL1_1248) {
			core_setrate_idx(i);
...
static void cpu_clk_init(struct clk_hw *hw)
{
	...
	cur_core_op = &core_op_array[core_idx_restore];
...
void __init pxa1928_core_clk_init(void __iomem *apmu_base)
{
	...
	apmu_coreapss_debug1 = apmu_base + APMU_COREAPSS_DEBUG1;
	...
	if (cpu_is_pxa1928_z1()) {
		...
		corefreq_src_shift = 30;
		corefreq_src_msk = 0x3;
		core_src_maptbl = core_src_zx_maptbl;
	} else if (cpu_is_pxa1928_z2()) {
		/* ASC code z2 */
		...
		cur_core_opt = core_oparray_b0;
		cur_core_opt_size = ARRAY_SIZE(core_oparray_b0);
		corefreq_src_shift = 29;
		corefreq_src_msk = 0x7;
		core_src_maptbl = core_src_b0_maptbl;
		/* set bit 3 all the time */
		reg = readl(apmu_coreapss_clkctrl);
		reg |= (1 << COREFREQ_FCEN_SHIFT);
		writel(reg, apmu_coreapss_clkctrl);
		/* clear this bit to enable pll1_1248M */
		reg = readl(apmu_coreapss_debug1);
		reg &= ~CG_PLL4_EN;
		writel(reg, apmu_coreapss_debug1);
	}
...
spinlock_t pxa1928_mckdfc_lock;

/* Interface used to get ddr opt len */
u32 pxa1928_get_axi_op_num(void)
{
	return cur_axi_opt_size;
}

/* Interface used to get axi rate */
u32 pxa1928_get_axi_op_rate(u32 index)
{
	if (index >= cur_axi_opt_size) {
		pr_err("%s index out of range!\n", __func__);
		return -EINVAL;
	}

	return cur_axi_opt[index].aclk1;
}
...
static int get_cur_axi_op(void)
{
	...
	for (i = 0; i < cur_axi_opt_size; i++) {
		...
		if (!pxa1928_is_zx) {
			...
			if (div1 == tmp_axi->aclk_div1 &&
				div2 == tmp_axi->aclk_div2 &&
				div3 == tmp_axi->aclk_div3 &&
				div4 == tmp_axi->aclk_div4 &&
				src == tmp_axi->axi_clk_sel)
				return i;
		}
	}
...
static void axi_fc_seq(struct pxa1928_axi_opt *top)
{
	...
	if (!pxa1928_is_zx) {
		...
		cc.b.axi_div = top->aclk_div1;
		writel(cc.v, apmu_cc_pj);
		spin_unlock(&cc_pj_access_lock);

		cc2.v = readl(apmu_cc2_pj);
		cc2.b.axi2_div = top->aclk_div2;
		cc2.b.axi3_div = top->aclk_div3;
		cc2.b.axi4_div = top->aclk_div4;
		writel(cc2.v, apmu_cc2_pj);
	}
...
static int set_axi_freq(struct pxa1928_axi_opt *old, struct pxa1928_axi_opt *new)
{
	int cur_index, ret = 0;
	unsigned long flags;
	...
	trace_pxa_axi_clk_chg(CLK_CHG_ENTRY,
			old->aclk1 / MHZ_TO_KHZ, new->aclk1 / MHZ_TO_KHZ);
	axi_fc_seq(new);
	trace_pxa_axi_clk_chg(CLK_CHG_EXIT,
			old->aclk1 / MHZ_TO_KHZ, new->aclk1 / MHZ_TO_KHZ);
			   ...
			   unsigned long prate)
{
	...
	trace_pxa_axi11_clk_chg(CLK_CHG_ENTRY,
			cur_axi11_op->aclk / MHZ_TO_KHZ, new_op->aclk / MHZ_TO_KHZ);
	ret = set_axi11_freq(new_op);
	trace_pxa_axi11_clk_chg(CLK_CHG_EXIT,
			cur_axi11_op->aclk / MHZ_TO_KHZ, new_op->aclk / MHZ_TO_KHZ);
...
/* set 2x dclk in apmu */
static struct pxa1928_ddr_opt ddr_discrete_oparray_4l[] = {
	{
		.dclk = 78000,
		.ddr_tbl_index = 0,
...
/* set 2x dclk in apmu */
static struct pxa1928_ddr_opt ddr_discrete_oparray_2l[] = {
	{
		.dclk = 78000,
		.ddr_tbl_index = 0,
		.ddr_clk_sel = DDR_PLL1_624,
...
{
	return cur_ddr_opt_size;
}

/* Interface used to get ddr rate */
u32 pxa1928_get_ddr_op_rate(u32 index)
{
	if (index >= cur_ddr_opt_size) {
		pr_err("%s index out of range!\n", __func__);
		return -EINVAL;
	}

	return cur_ddr_opt[index].dclk;
}
...
void disable_cp_ddrdfc(void)
{
	u32 tmp;

	tmp = readl(hwdfc_base + 0x24);
	tmp |= 0x6;
	writel(tmp, hwdfc_base + 0x24);
}

void enable_cp_ddrdfc(void)
{
	u32 tmp;
...
static void ddr_clk_init(struct clk_hw *hw)
{
	...
	/* a0 discrete ddr keeps 502MHz */
	if (cpu_is_pxa1928_a0())
		pll4_freq_tb_dis[1].fbdiv = 58;

	if (cpu_is_pxa1928_pop())
...
static void lpddr2_ddr_fc_seq(struct pxa1928_ddr_opt *top)
{
	...
	/* 2 make sure cc_pj[27] and cc_sp[27] are set */
	if (pxa1928_is_zx) {
		spin_lock(&cc_pj_access_lock);
		cc.v = readl(apmu_cc_pj);
		...
		if ((tmp & 1 << 27) == 0) {
			tmp |= 1 << 27;
			writel(tmp, apmu_cc_sp);
		}
	}

	/* 3 Igore SP status */
	if (pxa1928_is_zx) {
		tmp = readl(apmu_debug);
		if ((tmp & 0x3) != 3) {
			tmp |= 3;
			writel(tmp, apmu_debug);
		}
	} else {
...
static int ddr_hwdfc_seq(unsigned int level)
{
	...
	ctrl.b.dfc_pllpark_idx = PLL_PARK_INDEX;
		...
		struct pxa1928_ddr_opt *new)
{
	...
	spin_lock_irqsave(&pxa1928_mckdfc_lock, flags);

	trace_pxa_ddr_clk_chg(CLK_CHG_ENTRY,
			old->dclk / MHZ_TO_KHZ, new->dclk / MHZ_TO_KHZ);

	ret = ddr_hwdfc_seq(new->ddr_freq_level);

	trace_pxa_ddr_clk_chg(CLK_CHG_EXIT,
			old->dclk / MHZ_TO_KHZ, new->dclk / MHZ_TO_KHZ);

	if (unlikely(ret == -EAGAIN)) {
		/* still stay at old freq and src */
		spin_unlock_irqrestore(&pxa1928_mckdfc_lock, flags);
		goto out;
	}
...
#ifdef CONFIG_MCK5_ACTIVITY_MONITOR
#define ACT_MON_ENABLE (1 << 3)
static void enable_act_mon(int en)
{
	u32 tmp;

	tmp = readl(get_mccu_base_va() + 0x144);
	if (en == 1)
		tmp |= ACT_MON_ENABLE;
	else
		tmp &= ~ACT_MON_ENABLE;
	writel(tmp, get_mccu_base_va() + 0x144);
}
#endif
				...
				unsigned long prate)
{
...
#ifdef CONFIG_MCK5_ACTIVITY_MONITOR
	/* disable activity monitor interrupt */
	enable_act_mon(0);
#endif
	...
	cur_ddr_op = new_op;
#ifdef CONFIG_MCK5_ACTIVITY_MONITOR
	mck5activity_moniter_window(new_op->dclk);
	if (index == 0) {
		mck5_set_threshold(0, 0);
		mck5_set_threshold(1, 65);
		th_flag = 1;
	} else if (index == cur_ddr_opt_size - 1) {
		mck5_set_threshold(1, 100);
		mck5_set_threshold(0, 30);
		th_flag = 1;
	} else if (th_flag != 0) {
		mck5_set_threshold(0, 30);
		mck5_set_threshold(1, 65);
...
void get_dfc_tables(void __iomem *ciu_base, struct pxa1928_ddr_opt **tbl, int *len)
{
	if (cpu_is_pxa1928_4l_discrete()) {
		*tbl = ddr_discrete_oparray_4l;
		*len = ARRAY_SIZE(ddr_discrete_oparray_4l);
	} else if (cpu_is_pxa1928_pop()) {
		*tbl = ddr_oparray;
		*len = ARRAY_SIZE(ddr_oparray);
	} else {
		*tbl = ddr_discrete_oparray_2l;
		*len = ARRAY_SIZE(ddr_discrete_oparray_2l);
	}
}
EXPORT_SYMBOL(get_dfc_tables);
		...
		void __iomem *ddrdfc_base, void __iomem *ciu_base)
{
	...
	get_dfc_tables(ciu_base, &cur_ddr_opt, &cur_ddr_opt_size);
