 ...
 * in hwdvc_rails.
 */
unsigned int *hwdvc_lpm;
void hwdvc_get_platlpm(u32 *hwdvc_plat_lpm)
{
	if (hwdvc_lpm) {
		pr_err("Platform hwdvc lpm has already defined!\n");
		return;
	}

	hwdvc_lpm = hwdvc_plat_lpm;
	return;
}
EXPORT_SYMBOL(hwdvc_get_platlpm);
