...
#define INT_CLR		(1 << 6)
static void pxa1928_clk_media_power_up(void __iomem *pwr_ctrl, spinlock_t *lock)
{
...
static void pxa1928_clk_media_clk_pwrdown_late(void __iomem *pwr_ctrl,
		spinlock_t *lock)
{
	unsigned long flags = 0;
	u32 val;
...
#define DMMY_CLK	(1 << 4)
static void pxa1928_clk_media_clk_pre_enable(void __iomem *isld_ctrl,
		spinlock_t *lock)
{
	unsigned long flags = 0;
	u32 val;
...
static int pxa1928_clk_disp_rst_enable(struct clk_hw *hw)
{
	...
	val = readl_relaxed(disp->apmu_base + CIU_FABRIC1_CKGT) | 1;
	writel_relaxed(val, disp->apmu_base + CIU_FABRIC1_CKGT);
	if (disp->lock)
		spin_unlock_irqrestore(disp->lock, flags);

	/* 3. power up */
	pxa1928_clk_media_power_up(disp->apmu_base + DISP_PWR_CTRL, disp->lock);

	/* 4. dummy clock for SRAM access */
	pxa1928_clk_media_clk_pre_enable(disp->apmu_base + ISLD_LCD_CTRL,
			disp->lock);
...
static void pxa1928_clk_disp_rst_disable(struct clk_hw *hw)
{
	struct mmp_clk_disp *disp = to_mmp_clk_disp(hw);
	unsigned long flags = 0;
	u32 val;
	...
	pxa1928_clk_media_clk_pwrdown_late(disp->apmu_base + DISP_PWR_CTRL,
			disp->lock);
}

static int pxa1928_clk_disp_rst_prepare(struct clk_hw *hw)
{
	struct mmp_clk_disp *disp = to_mmp_clk_disp(hw);
...
static void pxa1928_clk_disp_rst_unprepare(struct clk_hw *hw)
{
	struct mmp_clk_disp *disp = to_mmp_clk_disp(hw);
	struct clk *clk_depend;
	int i;
...
static struct clk_ops pxa1928_clk_disp_rst_ops = {
	.prepare = pxa1928_clk_disp_rst_prepare,
	.unprepare = pxa1928_clk_disp_rst_unprepare,
	.enable = pxa1928_clk_disp_rst_enable,
	.disable = pxa1928_clk_disp_rst_disable,
};
...
static void mmp_clk_disp_div_disable(struct clk_hw *hw)
{
	struct mmp_clk_disp *disp = to_mmp_clk_disp(hw);
	u32 val;
...
static void mmp_clk_disp_mux_disable(struct clk_hw *hw)
{
	struct mmp_clk_disp *disp = to_mmp_clk_disp(hw);
	u32 val;
...
static struct clk_ops mmp_clk_disp_div_ops = {
	...
	.disable = mmp_clk_disp_div_disable,
...
static struct clk_ops mmp_clk_disp_mux_ops = {
	...
	.disable = mmp_clk_disp_mux_disable,
		...
		void __iomem *apmu_base, struct mmp_clk_disp *disp)
{
	...
	case MMP_DISP_RST_CTRL:
		/* FIXME: this ops only for pxa1928 A0 platform */
		init.ops = &pxa1928_clk_disp_rst_ops;
		break;
	default:
