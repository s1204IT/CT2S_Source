...
static int clk_apbc_pwm_enable(struct clk_hw *hw)
{
	...
	if (apbc->flags & APBC_PWM_APB_CTRL)
		clk_apb = hw->clk;
	else
		clk_apb = apbc->clk_share;
	apbc_apb = to_clk_apbc(clk_apb->hw);

	clk_apbc_set_reg(apbc, 0, APBC_FNCLK);
	if ((hw->clk->enable_count + apbc->clk_share->enable_count) == 0) {
...
static int clk_apbc_pwm_prepare(struct clk_hw *hw)
{
	struct clk_apbc *apbc = to_clk_apbc(hw);

	if (apbc->clk_share)
		/* FIXME: no need to update after share clock exist */
		return 0;
	 ...
	 */
	if (!strcmp(hw->clk->name, "pwm1")) {
		apbc->clk_share = clk_get_sys("mmp-pwm.0", NULL);
	} else if (!strcmp(hw->clk->name, "pwm2")) {
		apbc->clk_share = clk_get_sys("mmp-pwm.3", NULL);
		apbc->flags |= APBC_PWM_APB_CTRL;
	} else if (!strcmp(hw->clk->name, "pwm3")) {
		apbc->clk_share = clk_get_sys("mmp-pwm.2", NULL);
	} else {
		apbc->clk_share = clk_get_sys("mmp-pwm.1", NULL);
		apbc->flags |= APBC_PWM_APB_CTRL;
	}

	BUG_ON(IS_ERR(apbc->clk_share));

	return 0;
}

struct clk_ops clk_apbc_pwm_ops = {
	.prepare = clk_apbc_pwm_prepare,
