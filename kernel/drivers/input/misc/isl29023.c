				 ...
				 struct device_attribute *attr, char *buf)
{
	...
	return sprintf(buf, "%d\n", isl29023_get_adc_value(client) >> 1);
}
...
#ifdef CONFIG_OF
static int isl29023_i2c_dt_get_gpio(struct i2c_client *client, const char *name)
{
    int ret;
...
static int isl29023_probe_dt(struct i2c_client *client)
{
	struct fsl_mxc_lightsensor_platform_data	*pdata	= client->dev.platform_data;

	pdata->irq_gpio   = isl29023_i2c_dt_get_gpio(client, "irq-gpio");
	pdata->rext = 499;	/* calibration: 499K->700K */
				    ...
				    const struct i2c_device_id *id)
{
...
#ifdef CONFIG_OF
	ls_data = kzalloc(sizeof(*ls_data), GFP_KERNEL);
	if (ls_data == NULL) {
		dev_err(&client->dev, "kzalloc(platform_data) failed.\n");
		return -ENOMEM;
	}
	client->dev.platform_data = ls_data;
	err = isl29023_probe_dt(client);
	if (err != 0) {
		dev_err(&client->dev,
			"isl29023_probe_dt() failed.(%d)\n", err);
		kfree(ls_data);
		return -EINVAL;
	}
	ls_data = client->dev.platform_data;
#else  /* !CONFIG_OF */
	...
	input_dev->name = "ISL_light_sensor";
	...
	input_report_abs(data->input, ABS_MISC, 0);
	input_sync(data->input);
...
#ifdef CONFIG_OF
static struct of_device_id isl29023_of_idtable[] = {
...
static struct i2c_driver isl29023_driver = {
	.driver = {
		...
		.owner	= THIS_MODULE,
#ifdef CONFIG_OF
		.of_match_table = of_match_ptr(isl29023_of_idtable),
#endif /* CONFIG_OF */
	},
