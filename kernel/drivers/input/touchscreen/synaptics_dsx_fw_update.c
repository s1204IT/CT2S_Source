...
static ssize_t fwu_sysfs_fw_version_show(struct device *dev,
		struct device_attribute *attr, char *buf);
		...
		struct device_attribute *attr, char *buf)
{
	...
	return snprintf(buf, PAGE_SIZE, "%02x %02x %02x %02x\n", firmware_id[0],
			firmware_id[1], firmware_id[2], firmware_id[3]);
}
...
static enum flash_area fwu_go_nogo(void)
{
	...
	unsigned long image_firmware_id;
	unsigned char firmware_id[4];
	unsigned char config_id[4];
	char *strptr;
	char *image_pr = kzalloc(sizeof(MAX_FIRMWARE_ID_LEN), GFP_KERNEL);
	...
	image_pr = kzalloc(sizeof(MAX_FIRMWARE_ID_LEN), GFP_KERNEL);
	...
	firmware_id[3] = 0;
	device_firmware_id = extract_uint(firmware_id);
	...
	while (strptr[index] >= '0' && strptr[index] <= '9') {
		image_pr[index] = strptr[index];
		index++;
	}
	image_pr[index] = 0;

	retval = sstrtoul(image_pr, 10, &image_firmware_id);
	if (retval ==  -EINVAL) {
		dev_err(&i2c_client->dev,
	...
	dev_info(&i2c_client->dev,
			...
			device_firmware_id,
			(unsigned int)image_firmware_id);
	if (image_firmware_id > device_firmware_id) {
		flash_area = UI_FIRMWARE;
		goto exit;
	}
	...
	device_config_id =  extract_uint(config_id);
	...
	dev_info(&i2c_client->dev,
			...
			fwu->config_data[2],
			fwu->config_data[3]);
	image_config_id =  extract_uint(fwu->config_data);
		...
		char *buf, loff_t pos, size_t count)
{
	...
	if (count < fwu->config_size) {
		dev_err(&rmi4_data->i2c_client->dev,
				"%s: Not enough space (%lu bytes) in buffer\n",
				__func__, count);
		return -EINVAL;
	}
