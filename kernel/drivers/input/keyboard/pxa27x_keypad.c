...
static int pxa27x_keypad_get_existing_keys(struct device *dev,
				  unsigned int *existing_keys)
{
	struct device_node *np = dev->of_node;
	if (!np) {
	...
	if (of_property_read_bool(np, "marvell,direct-keys"))
		*existing_keys |= DIRECT_KEYS;
	if (of_property_read_bool(np, "marvell,matrix-keys"))
		*existing_keys |= MATRIX_KEYS;
	if (of_property_read_bool(np, "marvell,rotary-keys"))
		*existing_keys |= ROTARY_KEYS;
	/*if non specific keys, try all (WA for backward compatibility)*/
	if (*existing_keys == 0)
		*existing_keys = DIRECT_KEYS | MATRIX_KEYS | ROTARY_KEYS;
	return 0;
}
...
static int pxa27x_keypad_build_keycode_from_dt(struct pxa27x_keypad *keypad)
{
	...
	error = pxa27x_keypad_get_existing_keys(dev, &existing_keys);
	if (error) {
		dev_err(dev, "failed to parse keypad existing keys\n");
		return error;
	}
	if (existing_keys & MATRIX_KEYS) {
...
void __weak clear_keypad_wakeup(void)
{
	return;
}

static irqreturn_t pxa27x_keypad_irq_wakeup_handler(int irq, void *dev_id)
{
	struct pxa27x_keypad *keypad = dev_id;
	unsigned long kpc = keypad_readl(KPC);
...
static void disable_direct_keypad_irq(struct pxa27x_keypad *keypad)
{
	unsigned long kpc = 0;
	kpc = keypad_readl(KPC);
	keypad_writel(KPC, kpc & ~KPC_DE);
}
...
static int pxa27x_keypad_suspend(struct device *dev)
{
	...
	if (device_may_wakeup(&pdev->dev))
		...
		disable_direct_keypad_irq(keypad);
		clk_disable_unprepare(keypad->clk);
	}
	return 0;
}
...
static int pxa27x_keypad_probe(struct platform_device *pdev)
{
	...
	if (of_property_read_bool(np, "marvell,irq_wakeup")) {
		irq_wakeup = platform_get_irq(pdev, 1);
		if (irq_wakeup < 0) {
			dev_err(&pdev->dev, "failed to get keypad irq_wakeup\n");
			error = -ENXIO;
			goto failed_free;
		}
	}

	keypad->irq_wakeup = irq_wakeup;
	...
	*/
	...
	if (keypad->irq_wakeup > 0) {
		error = request_irq(irq_wakeup, pxa27x_keypad_irq_wakeup_handler,
				    0, pdev->name, keypad);
		if (error) {
			dev_err(&pdev->dev, "failed to request IRQ_WAKEUP\n");
...
static int pxa27x_keypad_remove(struct platform_device *pdev)
{
	...
	free_irq(keypad->irq, keypad);
	if (keypad->irq_wakeup > 0)
		free_irq(keypad->irq_wakeup, keypad);
	clk_put(keypad->clk);
