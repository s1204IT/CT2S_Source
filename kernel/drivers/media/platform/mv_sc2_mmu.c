...
static int msc2_setup_sglist(struct vb2_buffer *vb)
{
	...
	for (i = 0; i < vb->num_planes; i++) {
		...
		info = &mbuf->ch_info[i];
		info->dma_desc_nent = dma_map_sg(dev, sgd->sglist,
					sgd->num_pages, DMA_FROM_DEVICE);
		if (info->dma_desc_nent <= 0) {
			info->dma_desc_nent = 0;
			while ((--i) >= 0) {
				sgd = vb2_dma_sg_plane_desc(vb, i);
				info = &mbuf->ch_info[i];
				dma_unmap_sg(dev, sgd->sglist,
					sgd->num_pages, DMA_FROM_DEVICE);
				info->dma_desc_nent = 0;
			}
		...
		desc = info->dma_desc;
		for_each_sg(sgd->sglist, sg, info->dma_desc_nent, j) {
...
static int msc2_unmap_sglist(struct vb2_buffer *vb)
{
	...
	for (i = 0; i < vb->num_planes; i++) {
		sgd = vb2_dma_sg_plane_desc(vb, i);
		info = &mbuf->ch_info[i];
		dma_unmap_sg(dev, sgd->sglist, sgd->num_pages, DMA_FROM_DEVICE);
		info->dma_desc_nent = 0;
	}
	return 0;
}
