...
{
	return sc2_pwr_ctrl(level, SC2_MOD_B52ISP);
}
				...
				struct v4l2_subdev_fh *fh)
{
	...
	struct b52isp_idi *idi = container_of(blk, struct b52isp_idi, block);
	int ret;

	ret = isp_block_tune_power(blk, 1);
	b52_set_base_addr(blk->reg_base);

	if (idi->parent->irq_disabled) {
		enable_irq(idi->parent->irq);
		idi->parent->irq_disabled = 0;
	}
...
static int b52isp_path_hw_open(struct isp_block *block)
{
	int ret;
	if (!get_b52fw_loaded()) {
		ret = b52_load_fw(block->dev, block->reg_base);
		if (ret < 0)
			return ret;
...
static int b52isp_path_set_profile(struct isp_subdev *isd)
{
	...
	case SDCODE_B52ISP_IDI2:
		...
		r_pad = media_entity_remote_source(src->pads + B52PAD_IDI_IN);
		if (unlikely(WARN_ON(r_pad == NULL))) {
			ret = -EPIPE;
			goto exit;
		}
		...
		cur_cmd->sensor = sd =
				media_entity_to_v4l2_subdev(r_pad->entity);
		cur_sensor = to_b52_sensor(sd);
		switch (cur_sensor->pos) {
		case B52_SENSOR_I2C_PRIMARY_MASTER:
			cur_cmd->src_type = CMD_SRC_PRIMARY_SNSR;
			pipe->csi_sel = SDCODE_CCICV2_CSI0;
			break;
		case B52_SENSOR_I2C_SECONDARY_MASTER:
			cur_cmd->src_type = CMD_SRC_SECONDAY_SNSR;
			pipe->csi_sel = SDCODE_CCICV2_CSI1;
			break;
		default:
			ret = -EINVAL;
			goto exit;
               ...
               struct b52isp_brightness *arg)
{
       ...
       case TYPE_READ:
               ...
               arg->yoffset_h = b52_readb(0x65b12);
	...
	struct b52isp_sharpness *arg)
{
	...
	while (REG_FW_MIN_SHARPNESS + i <= REG_FW_MAX_SHARPNESS) {
		b52_writeb(base + REG_FW_MIN_SHARPNESS + i, arg->sha_arr[i]);
		i++;
	}
...
fail_double:
	...
	if (lpipe->meta_cpu == NULL) {
		...
		lpipe->meta_cpu = dmam_alloc_coherent(b52isp->dev,
					lpipe->meta_size + 1, &lpipe->meta_dma,
					GFP_KERNEL);
		if (lpipe->meta_cpu == NULL) {
			lpipe->meta_dma = 0;
			d_inf(1, "failed to allocate metadata buffer for %s",
...
link_off:
	...
	do {
		...
		if (lpipe->meta_cpu) {
			dmam_free_coherent(b52isp->dev, lpipe->meta_size + 1,
		...
		struct plat_cam *pcam, int num_planes, int mac_id, int port)
{
	...
	if (ret < 0) {
		d_inf(1, "Failed to update MMU addr");
		return ret;
	}
	...
	struct isp_vnode *vnode, struct isp_videobuf *buffer)
{
	...
	if (lpipe_isd->sd_code == SDCODE_B52ISP_PIPE1 ||
		lpipe_isd->sd_code == SDCODE_B52ISP_DUMP1 ||
		lpipe_isd->sd_code == SDCODE_B52ISP_MS1)
		id = 0;
	else if (lpipe_isd->sd_code == SDCODE_B52ISP_PIPE2 ||
		lpipe_isd->sd_code == SDCODE_B52ISP_DUMP2 ||
		lpipe_isd->sd_code == SDCODE_B52ISP_MS2)
		id = 1;
	else {
		id = 0;
		d_inf(1, "%s, id does not match, set id to 0", __func__);
	}

	if ((buffer->vb.v4l2_buf.type ==
		 V4L2_BUF_TYPE_VIDEO_OUTPUT_MPLANE) ||
		(buffer->vb.num_planes <= meta_pid)) {
		d_inf(1, "%s, num planes error", __func__);
		return -EINVAL;
	}
	...
	if (type == V4L2_PLANE_SIGNATURE_PIPELINE_INFO)
		b52_read_pipeline_info(id, lpipe->pinfo_buf);
...
static int b52isp_mac_handler(struct isp_subdev *isd, unsigned long event)
{
	...
	if (time_before64(laxi->irq_jiffies + HZ, get_jiffies_64())) {
		laxi->irq_jiffies = get_jiffies_64();
		laxi->irq_cnt = 0;
	}
	if (laxi->irq_cnt++ > 150) {
		if (!laxi->parent->irq_disabled) {
			d_inf(2, "%s disable IRQ:cnt %d", isd->subdev.name, laxi->irq_cnt);
			disable_irq(laxi->parent->irq);
			laxi->parent->irq_disabled = 1;
		}
	}

	if (irqstatus & VIRT_IRQ_FIFO)
		laxi->overflow = 1;
	if (laxi->overflow && (irqstatus & VIRT_IRQ_START))
...
check_drop:
	if (irqstatus & VIRT_IRQ_DROP) {
		...
		if (!buffer) {
			...
			d_inf(5, "%s: buffer in the BusyQ when drop! idle:%d, busy:%d",
				  vnode->vdev.name,
...
recheck:
	if (laxi->dma_state != B52DMA_ACTIVE) {
		...
		if (irqstatus & (VIRT_IRQ_FIFO | VIRT_IRQ_DONE))
			pr_info_ratelimited("%s unmatched EOF IRQ: %X\n",
						isd->subdev.name, irqstatus);
	} else {
		if (irqstatus & (VIRT_IRQ_FIFO | VIRT_IRQ_DONE))
			goto check_eof_irq;
		if (irqstatus & (VIRT_IRQ_START | VIRT_IRQ_DROP)) {
			pr_info_ratelimited("%s unmatched SOF IRQ: %X\n",
						isd->subdev.name, irqstatus);
			if (irqstatus & VIRT_IRQ_DROP)
				goto check_drop;
		}
...
disable:
	...
	while (laxi->dma_state != B52DMA_HW_NO_STREAM) {
		...
		if (cnt++ > 500) {
			d_inf(1, "%s err wait HW_NO_STREAM", laxi->isd.subdev.name);
			break;
		}
	}
		...
		struct plat_vnode *pvnode, int stream)
{
	...
	if (laxi->stream) {
		...
		/* open ISP sccb clk, process raw and memory sensor needn't enable sccb clk*/
		if ((lpipe->cur_cmd->cmd_name != CMD_RAW_PROCESS)
				&& (lpipe->cur_cmd->path != B52ISP_ISD_MS1)
				&& (lpipe->cur_cmd->path != B52ISP_ISD_MS2))
			b52_enable_sccb_clk(lpipe->cur_cmd, 1);
		/* open pipeline clk*/
		if ((lpipe->cur_cmd->path == B52ISP_ISD_PIPE1)
			|| (lpipe->cur_cmd->path == B52ISP_ISD_MS1)
			|| (lpipe->cur_cmd->path == B52ISP_ISD_DUMP1))
				b52_enable_pipe_clk(PIPELINE_1, 1);
		else if ((lpipe->cur_cmd->path == B52ISP_ISD_PIPE2)
			|| (lpipe->cur_cmd->path == B52ISP_ISD_MS2)
			|| (lpipe->cur_cmd->path == B52ISP_ISD_DUMP2))
				b52_enable_pipe_clk(PIPELINE_2, 1);
		...
		case CMD_IMG_CAPTURE:
			...
			for (i = 0; i < lpipe->path_arg.nr_frame; i++) {
				isp_vb = isp_vnode_get_idle_buffer(vnode);
				if (isp_vb == NULL) {
					d_inf(1, "%s: request capture of %d frames, but only %d buffer ready",
						lpipe->isd.subdev.name,
...
disable_axi:
		...
		if ((lpipe->cur_cmd->path == B52ISP_ISD_PIPE1)
			|| (lpipe->cur_cmd->path == B52ISP_ISD_MS1)
			|| (lpipe->cur_cmd->path == B52ISP_ISD_DUMP1))
				b52_enable_pipe_clk(PIPELINE_1, 0);
		else if ((lpipe->cur_cmd->path == B52ISP_ISD_PIPE2)
			|| (lpipe->cur_cmd->path == B52ISP_ISD_MS2)
			|| (lpipe->cur_cmd->path == B52ISP_ISD_DUMP2))
				b52_enable_pipe_clk(PIPELINE_2, 0);

		/* close sccb clk */
		if ((lpipe->cur_cmd->cmd_name != CMD_RAW_PROCESS)
				&& (lpipe->cur_cmd->path != B52ISP_ISD_MS1)
				&& (lpipe->cur_cmd->path != B52ISP_ISD_MS2))
			b52_enable_sccb_clk(lpipe->cur_cmd, 0);
					...
					struct plat_vnode *pvnode)
{
	...
	if (pipe->isd.sd_code < SDCODE_B52ISP_HS) {
		...
		if (vnode->buf_type == V4L2_BUF_TYPE_VIDEO_CAPTURE_MPLANE) {
			...
			if (item->cmd_name == CMD_IMG_CAPTURE) {
				...
				pipe->cur_cmd->cmd_name = item->cmd_name;
			} else
...
static void b52isp_axi_remove(struct b52isp *b52isp)
{
	...
	for (i = 0; i < b52isp->hw_desc->nr_axi * 3; i++) {
		...
		b52isp->blk[B52ISP_BLK_AXI1 + i / 3] = NULL;
	}
}
...
static irqreturn_t b52isp_irq_handler(int irq, void *data)
{
	...
	ret = b52_ack_xlate_irq(event);
	if (ret < 0) {
		err_cnt++;
		if (err_cnt > 5) {
			/* if empty interrupt repeats, reset ISP power */
			sc2_pwr_ctrl(0xDEAD, SC2_MOD_B52ISP);
		}
		return IRQ_HANDLED;
	}
	/* we only count for contiguous errors */
	err_cnt = 0;
	...
	for (i = 0; i < b52isp->hw_desc->nr_axi; i++) {
		...
		if (paxi->event & event[i])
			d_inf(4, "mac bit already set,%x:%x", paxi->event, event[i]);
		paxi->event |= event[i];
...
static int b52isp_probe(struct platform_device *pdev)
{
	...
	b52isp->irq = res->start;
	b52isp->irq_disabled = 0;
