...
static int sensor_read(struct i2c_client *client, u32 reg, u8 reg_len,
		u8 *value, u32 data_len)
{
	int i;
	int ret;
	u8 *reg_val = (u8 *)&reg;
	/*
	 * Because kernel do not use dynamical array size
	 * in stack, and max num of the buf size is 4 as reg
	 * bytes, asume to define it to max and handle all the
	 * scenarios
	 */
	u8 buf_reg[4];
...
static int sensor_write(struct i2c_client *client, u32 reg, u8 reg_len,
	       u32 value, u32 data_len)
{
	int i;
	int ret;
	u8 *reg_val  = (u8 *)&reg;
	u8 *data_val = (u8 *)&value;
	/*
	 * Because kernel do not use dynamical array size
	 * in stack, and max num of the buf size is 8 as reg
	 * bytes + data bytes, asume to define it to max and
	...
	struct i2c_msg msg = {
		.addr  = client->addr,
		.flags = 0,
		.len   = reg_len + data_len,
		.buf   = buf,
	};

	for (i = 0; i < reg_len; i++)
		buf[reg_len - i - 1] = *(reg_val + i);
	for (i = 0; i < data_len; i++)
		buf[reg_len + i] = *(data_val + i);
			...
			enum sensor_register_access_e access_type)
{
	...
	case SENSOR_REG_READ:
		ret = sensor_read(client, param->reg, core->reg_len,
			(u8 *)&param->value, param->data_len);
		break;
	case SENSOR_REG_WRITE:
		ret = sensor_write(client, param->reg, core->reg_len,
				...
				struct v4l2_subdev *sd)
{
	...
	for (j = 0; j < 2; j++) {
		ret = sensor_read(client, pid->pid_addr[j], core->reg_len, &v, 1);
		...
		const struct i2c_device_id *id)
{
	...
	ret = of_property_read_u32(core->np, "type", (u32 *)&flag);
	if (ret < 0)
		flag = REG_TYPE_16;
	switch (flag) {
	case REG_TYPE_8:
	...
	case REG_TYPE_16:
		...
		break;
	default:
		dev_warn(&client->dev, "Failed to get register length\n");
		return -EINVAL;
	}
