...
static void area51_close_notify(struct isp_build *build)
{
	struct isp_pipeline *pipe;

	/* 1st, stream off all the pipeline */
	list_for_each_entry(pipe, &build->pipeline_pool, hook) {
		mutex_lock(&pipe->st_lock);
		if (pipe->state != ISP_PIPELINE_BUSY)
			goto next;
		dev_info(build->media_dev.dev,
			"area51: force close of pipeline");
		isp_pipeline_set_stream(pipe, 0);
next:
		mutex_unlock(&pipe->st_lock);
	}
	/* 2nd, release all the pipeline */
	list_for_each_entry(pipe, &build->pipeline_pool, hook)
		isp_pipeline_clean(pipe);
}
...
static int mvisp_detect_vcm(struct area51_device *isp,
			...
			struct i2c_adapter *adapter)
{
	...
	struct device_node *vcmdev_np;
	int ret;

	vcmdev_np = of_parse_phandle(subdev_np, "vcm-subdev", 0);
	if (!vcmdev_np)
		return -1;
	ret = of_property_read_u32(vcmdev_np, "reg", (u32 *)&(info.addr));
	if (ret < 0)
		return ret;
	info.of_node = vcmdev_np;
	of_property_read_string(vcmdev_np, "vcm-name", (char const **)&s);
	strcpy(info.type, s);
	info.platform_data = subdev_np;
...
static int mvisp_detect_sensor(struct area51_device *isp)
{
	...
	struct device_node *host_np = NULL;
	...
	do {
		...
		if (subdev != NULL) {
			...
			mvisp_detect_vcm(isp, subdev_np, adapter);
			break;
		}
	} while (subdev_np != NULL);
#ifdef CONFIG_HOST_SUBDEV
	host_np = of_get_child_by_name(isp->np, "backhost");
	if (host_np == NULL)
		return -1;
	isp->sd_host = host_subdev_create(isp->manager->dev,
					"host subdev", 0, NULL);
	if (isp->sd_host == NULL) {
		pr_err("create host subdev ret:%d\n", ret);
		return -1;
	}
	ret = v4l2_device_register_subdev(&isp->manager->v4l2_dev,
					&isp->sd_host->subdev);
	if (ret < 0)
		pr_err("register host subdev ret:%d\n", ret);
	flash = mvisp_board_flash_create(isp->manager->dev, 0,
				host_np);
	if (flash != NULL) {
		ret = v4l2_device_register_subdev(
				&isp->manager->v4l2_dev,
				flash);
		if (ret == 0)
			host_subdev_add_guest(isp->sd_host, flash,
					SUBDEV_FLASH, 0);
	}
#endif
	if (!isp->two_sensor_support)
...
static int area51_probe(struct platform_device *pdev)
{
	...
	isp->manager->add_vdev = area51_add_ispsd;
	isp->manager->close_vdev = area51_close_notify;
