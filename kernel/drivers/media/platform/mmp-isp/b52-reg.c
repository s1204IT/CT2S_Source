...
static struct b52_sensor *curr_sensor;
static void twsi_do_work(struct work_struct *data);
static DECLARE_WORK(twsi_work, twsi_do_work);
...
#undef	USE_OPTIMIZE_PROCESS_RAW
...
static void twsi_do_work(struct work_struct *data)
{
	...
	struct b52_sensor *psensor = curr_sensor;
...
/*
 * since the dvs sometime does not update them in the meta data in time,
 * need to read them again after MAC done.
 */
int b52_read_dvs_info(int pipe_id, u8 *buffer)
{
	int i;
	u32 reg;
	...
	/* read preview cropx and cropy */
	for (i = REG_OI_PREVIEW_CROPX; i <= REG_OI_PREVIEW_CROPY + 1; i++)
		buffer[i] = b52_readb(reg + i);

	/* read video1/2 cropx/y and sharpness */
	for (i = REG_OI_VIDEO1_CROPX; i <= REG_OI_SHARPNESS + 1; i++)
		buffer[i] = b52_readb(reg + i);
...
void b52_enable_mac_clk(int mac_id, int enable)
{
	static int mac1_refcnt;
	static int mac2_refcnt;
	static int mac3_refcnt;
	u8 val, val1;

	if (enable) {
		val = b52_readb(REG_TOP_CLK_RST1);
		val1 = b52_readb(REG_TOP_CLK_RST3);
		if (mac_id == 0) {
			if (mac1_refcnt == 0)
				val |= MAC_1_CLK_EN;
			mac1_refcnt++;
		...
		b52_writeb(REG_TOP_CLK_RST1, val);
		if (mac_id == 2) {
			if (mac3_refcnt == 0)
				val1 |= MAC_3_CLK_EN;
			mac3_refcnt++;
		}
		b52_writeb(REG_TOP_CLK_RST3, val1);
	} else {
		val = b52_readb(REG_TOP_CLK_RST1);
		val1 = b52_readb(REG_TOP_CLK_RST3);
		if (mac_id == 0) {
...
void b52_enable_pipe_clk(int ppl_id, int enable)
{
	u8 val;
	static int isp_pipe_1_clk_refcnt;
	static int isp_pipe_2_clk_refcnt;

	if (enable) {
		val = b52_readb(REG_TOP_CLK_RST1);
		if (ppl_id == 0) {
			if (isp_pipe_1_clk_refcnt == 0)
				val |= PIPE_1_CLK_EN;

			isp_pipe_1_clk_refcnt++;
		}
		if (ppl_id == 1) {
			if (isp_pipe_2_clk_refcnt == 0)
				val |= PIPE_2_CLK_EN;

			isp_pipe_2_clk_refcnt++;
		}
		b52_writeb(REG_TOP_CLK_RST1, val);
	} else {
		val = b52_readb(REG_TOP_CLK_RST1);
		if (ppl_id == 0) {
			if (--isp_pipe_1_clk_refcnt > 0)
				return;
		/* keep pipeline1 clk always on, because FW will be unstable
		 * if this clock is gated while other ISP power is on. */
#if 0
			val &= ~PIPE_1_CLK_EN;
#endif
			isp_pipe_1_clk_refcnt = 0;
		}
		if (ppl_id == 1) {
			if (--isp_pipe_2_clk_refcnt > 0)
				return;
			...
			val &= ~PIPE_2_CLK_EN;
			isp_pipe_2_clk_refcnt = 0;
		}
		b52_writeb(REG_TOP_CLK_RST1, val);
	}
}
void b52_enable_sccb_clk(struct b52isp_cmd *cmd, int enable)
{
	u8 val;
	static int isp_sccb_1_clk_refcnt;
	static int isp_sccb_2_clk_refcnt;
	struct v4l2_subdev *sd = cmd->sensor;
	struct b52_sensor *sensor = to_b52_sensor(sd);
	if (enable) {
		if (sensor->pos == B52_SENSOR_I2C_PRIMARY_MASTER) {
			/*
			the SCCB1 clk has been enabled
			thie just disable the SCCB2 clk
			*/
			if (isp_sccb_1_clk_refcnt == 0) {
				val = b52_readb(REG_TOP_CLK_RST0);
				b52_writeb(REG_TOP_CLK_RST0,
						val&(~SCCB_2_CLK_EN));
			}
			isp_sccb_1_clk_refcnt++;
		} else if (sensor->pos == B52_SENSOR_I2C_SECONDARY_MASTER) {
			/*
			the SCCB2 clk has been enabled
			thie just disable the SCCB1 clk
			*/
			if (isp_sccb_2_clk_refcnt == 0) {
				val = b52_readb(REG_TOP_CLK_RST0);
				b52_writeb(REG_TOP_CLK_RST0,
						val&(~SCCB_1_CLK_EN));
			}
			isp_sccb_2_clk_refcnt++;
		}
	}
}
...
int b52_load_fw(struct device *dev, void __iomem *base)
{
	int ret, i;
...
#if 0
	/*first disable sccb1 and sccb2 clk, open them when they will be used */
	val = b52_readb(REG_TOP_CLK_RST0);
	b52_writeb(REG_TOP_CLK_RST0, val & 0x9F);
#endif
	/*first, disable mac1/2/3 isp pipeline2, open them when they will be used
	 * pipeline1 clk always on when ISP is working, due to HW limitation */
	val = b52_readb(REG_TOP_CLK_RST1);
	b52_writeb(REG_TOP_CLK_RST1, val & 0x4F);
	val = b52_readb(REG_TOP_CLK_RST3);
	b52_writeb(REG_TOP_CLK_RST3, val & 0xEF);
		...
		struct v4l2_pix_format *fmt, int src)
{
	...
	switch (src) {
	case CMD_SRC_PRIMARY_SNSR:
		cfg->type |= SRC_PRIMARY_MIPI;
		break;
	case CMD_SRC_SECONDAY_SNSR:
		cfg->type |= SRC_SECONDAY_MIPI;
		break;
	case CMD_SRC_AXI:
		cfg->type |= SRC_MEMORY;
		break;
...
static int __b52_isp_read_i2c(struct b52_sccb *sccb, u8 pos)
{
	...
	b52_isp_wait_sccb_idle(base, 10000);
...
int b52_dns_process(struct b52isp_cmd *cmd, u8 dns)
{
	...
	u32 fmt;
	int ret = 0;
/****** the fmt is the internal fmt for UV denoise process ******/
/** if we use the UYVY fmt, firmware will use no optimise mode**/

#ifdef	USE_OPTIMIZE_PROCESS_RAW
...
static int b52_cmd_process_raw(struct b52isp_cmd *cmd)
{
	...
	if (cmd->flags & BIT(CMD_FLAG_LINEAR_YUV))
		b52_writeb(CMD_REG5, ENABLE_LINEAR_YUV);
...
static int __maybe_unused b52_cmd_awb(struct b52isp_cmd *cmd)
{
	...
	ret = wait_cmd_done(CMD_AWB_MODE);
	mutex_unlock(&cmd_mutex);
	return ret;
}
...
static int b52_set_aecagc_reg(struct v4l2_subdev *sd, int p_num)
{
	...
	case I2C_REG_NUM_2:
		...
		*/
		if (pxa1928_is_a0())
			...
			if (type == SONY_SENSOR || type == SAMSUNG_SENSOR||type == SAMSUNG_ASNY_SENSOR)
	...
	case I2C_REG_NUM_1:
		...
		/*FIXME
		* SCCB always use 16bit width to handle registers and its value.
		* But IM219 again register has only 8bit.
		* So we have to set the AEC_ADDR_6 to 0x00 and ACE_MSK_6 to 0x00
		*/
			b52_writew(base + REG_FW_SSOR_AEC_ADDR_6, 0x00);
			...
			u8 plane, u8 mac, u8 port)
{
	...
	*/
	...
	if (mac_overflow[mac][port]) {
		spin_unlock_irqrestore(&mac_overflow_lock, irq_flags);
		pr_err("(%d, %d) not set addr due to overflow\n", mac, port);
		return 0;
	}
 ...
 */
int b52_ack_xlate_irq(__u32 *event)
{
	...
	if (reg & INT_CMD_DONE) {
		...
		if (pxa1928_is_a0()) {
			twsi_reg = b52_readb(REG_FW_AGC_AF_FLG);
			if (CMD_WB_EXPO_GAIN <= twsi_reg
				&& twsi_reg <= CMD_WB_FOCUS) {
				error = 0;
...
static ssize_t b52_proc_write_reg(
	struct file *filp,
	...
	loff_t *offset)
{
	...
	sscanf(buf, "%c %x %x %d", &readwrite, &addr, &tmp, &pos);
