...
static unsigned int reg_info[2];
static unsigned int weight_setting_offset;
static unsigned int weight_setting[2];
...
static void ispdma_cpu_ddr_qos(struct isp_ispdma_device *ispdma,
					int *freq)
{
	pm_qos_update_request(&ddrfreq_qos_req_min, *freq);
	pr_debug("ispdma: ddrfreq qos %d\n", *freq);
}
...
static int ispdma_pwr_ctrl(void *pdev, int power_on)
{
	struct isp_block *block = (struct isp_block *)pdev;
...
static int dmax_video_stream_on_notify(struct isp_vnode *vnode)
{
	struct isp_dma_mod *mod = video_get_ispsd(vnode)->drv_priv;
	struct isp_ispdma_device *ispdma = mod->core;
	struct v4l2_mbus_framefmt *format = &ispdma->agent.fmt_pad[mod->pad_id];
	struct isp_videobuf *buffer = NULL;
	unsigned long flags;
	void __iomem *hw_base;
	int ret = 0;

	if (weight_setting_offset) {
		hw_base = ioremap(reg_info[0], reg_info[1]);
		if (hw_base == NULL) {
			pr_err("error to ioremap memory controller base\n");
			return -1;
		}

		if ((mod->pad_id == ISPDMA_PAD_CODE_SRC) && (format->width == 1920)) {
			writel(weight_setting[1], hw_base + weight_setting_offset);
			pr_info("1080P camcorder is on\n");
		}
...
static int dmax_video_stream_off_notify(struct isp_vnode *vnode)
{
	...
	if (weight_setting_offset) {
		...
		if ((mod->pad_id == ISPDMA_PAD_CODE_SRC) && (format->width == 1920)) {
			writel(weight_setting[0], hw_base + weight_setting_offset);
			pr_info("1080P camcorder is off\n");
		}
...
static int ispdma_probe(struct platform_device *pdev)
{
	...
	if (of_property_read_u32_array(pdev->dev.of_node,
					"mrvl,memory-priority-addr", reg_info, 2)) {
		dev_err(dev, "Failed to get memory register info\n");
	}

	if (of_property_read_u32(pdev->dev.of_node,
					"mrvl,memory-weight-setting-offset", &tmp)) {
		dev_err(dev, "Failed to get memory weight setting offset\n");
	} else
		weight_setting_offset = tmp;

	if (of_property_read_u32_array(pdev->dev.of_node,
					"mrvl,memory-weight-setting", weight_setting, 2)) {
		dev_err(dev, "Failed to get memory weight setting\n");
	}
