...
static void ccic_ctrl_reset(struct mmp_camera_dev *pcdev);
...
{
	clk_prepare_enable(pcdev->ahb_clk);
...
{
	clk_disable_unprepare(pcdev->ahb_clk);
...
static irqreturn_t mmp_camera_frameirq(int irq, void *data)
{
	...
	if (irqs & IRQ_OVERFLOW) {
		...
		pcdev->overflow_cnt += 5;
	} else
		pcdev->overflow_cnt = pcdev->overflow_cnt > 0 ?
				pcdev->overflow_cnt - 1 : 0;

	/* FIXME here do reset when occur continuous overflow
	 * and this overflow_cnt:20 is tested by AE*/
	if (pcdev->overflow_cnt > 20) {
		dev_err(dev, "Overflow irq occurs!\n");
		ccic_reg_write(pcdev, REG_IRQSTAT, irqs);
		for (frame = 0; frame < pcdev->nbufs; frame++)
			clear_bit(CF_FRAME_SOF0 + frame, &pcdev->flags);
		ccic_ctrl_reset(pcdev);
		pcdev->overflow_cnt = 0;
		return IRQ_HANDLED;
	}
...
{
	pcdev->ahb_clk = devm_clk_get(&pcdev->pdev->dev, "LCDCIHCLK");
	if (IS_ERR(pcdev->ahb_clk))
		return PTR_ERR(pcdev->ahb_clk);
