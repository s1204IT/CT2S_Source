...
#define   CSI_ERROR_IRQS 	0xff000000  /*  CSI error */
...
#define REG_CCIC_FRAME_BYTE_CNT 0x23c

#define REG_APMU_CCIC_GATE 0x028
#define REG_APMU_CCIC_RST 0x050
#define REG_APMU_CCIC2_GATE 0x118
#define REG_APMU_CCIC2_RST 0x024
#define REG_APMU_CCIC_DBG 0x088
/*
...
struct mmp_camera_dev {
	...
	int frame_rate;
	int dma_burst;
	int mclk_src, mclk_div;
	struct clk *axi_clk;
	struct clk *fn_mux_clk;
	struct clk *fn_div_clk;
	struct clk *fn_clk;
	struct clk *phy_mux_clk;
	struct clk *phy_clk;
	struct clk *sphy_div_clk;
	struct clk *sphy_clk;
	struct clk *arb_clk;
	struct clk *ahb_clk;
	const char *fn_parent;
	const char *phy_parent;
	int mbus_fmt_num;
