...
static int mccic_add_device(struct soc_camera_device *icd)
{
	...
	if (sdata->manual_mclk_ctrl) {
		sdata->mclk = mcam_dev->mclk;
	}
	...
	soc_camera_power_on(&mcam_dev->pdev->dev, ssdd);
	/* CCIC/ISP/MMU power on */
	sc2_pwr_ctrl(1, SC2_MOD_CCIC);
...
power_off:
	sc2_pwr_ctrl(0, SC2_MOD_CCIC);
	soc_camera_power_off(&mcam_dev->pdev->dev, ssdd);
			...
			struct v4l2_format *f)
{
	...
	dev_info(icd->parent, "%s, num %d\n", __func__, pix_mp->num_planes);
	return 0;
}
