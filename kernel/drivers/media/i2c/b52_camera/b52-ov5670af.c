...
#include "b52-ov5670af.h"

static int OV5670AF_get_dphy_desc(struct v4l2_subdev *sd,
...
static int OV5670AF_get_pixelclock(struct v4l2_subdev *sd, u32 *rate, u32 mclk)
{
	/*calculate sysclk*/
	int temp1, temp2;
...
static int OV5670AF_read_OTP(struct b52_sensor *sensor,
			struct b52_sensor_otp *otp, u32 *flag)
{
	int otp_flag, otp_base, temp, i;
	int vcm_id, driver_id;
	...
	if (otp_base != 0) {
		*flag = 0x80; /*valid info in OTP*/
		...
		vcm_id = OV5670_read_i2c(sensor, otp_base + 7);
		driver_id = OV5670_read_i2c(sensor, otp_base + 8);
	} else {
	...
	pr_err("Marvell_Unifiled_OTP_ID_INF for OV5670: Module=0x%x, Lens=0x%x, VCM=0x%x, DriverIC=0x%x\n",
		(*otp).module_id, (*otp).lens_id, vcm_id, driver_id);
	/*OTP WB Calibration*/
...
static int OV5670AF_update_wb(struct b52_sensor *sensor,
			struct b52_sensor_otp *otp, u32 *flag)
{
...
static void OV5670AF_otp_access_start(struct b52_sensor *sensor)
{
	int temp;
...
static void OV5670AF_otp_access_end(struct b52_sensor *sensor)
{
	int temp;
	temp = OV5670_read_i2c(sensor, 0x5002);
	OV5670_write_i2c(sensor, 0x5002, temp|(0x08));
...
static int OV5670AF_update_otp(struct v4l2_subdev *sd,
				struct b52_sensor_otp *otp)
{
	int ret = 0;
	int flag = 0;
	...
	if (otp->otp_type == SENSOR_TO_SENSOR) {
		...
		OV5670AF_otp_access_start(sensor);

		/*read otp data firstly*/
		ret = OV5670AF_read_OTP(sensor, otp, &flag);

		/*access otp data end*/
		OV5670AF_otp_access_end(sensor);

		if (ret < 0)
			goto fail;
		/*apply some otp data, include awb*/
		if (otp->otp_ctrl & V4L2_CID_SENSOR_OTP_CONTROL_WB) {
			ret = OV5670AF_update_wb(sensor, otp, &flag);
			if (ret < 0)
				goto fail;
		}
