...
#include "b52-ov13850r2a.h"

static int OV13850r2a_get_mipiclock(struct v4l2_subdev *sd, u32 *rate, u32 mclk)
{
	int temp1, temp2;
	int Pll2_predivp, Pll2_prediv2x, Pll2_mult, Pll2_divsp;
	int Pll2_divm, Pll2_divs2x;
	...
	b52_sensor_call(sensor, i2c_read, 0x3615, &temp1, 1);
	temp2 = temp1 & 0x03;
	...
	*rate = mclk / Pll2_predivp * 2 / Pll2_prediv2x * Pll2_mult / Pll2_divm;
...
static int OV13850r2a_get_pixelclock(struct v4l2_subdev *sd, u32 *rate, u32 mclk)
{
	int temp1, temp2;
	int Pll2_divmipi;
	u32 mipi_clk;
	int Pll2_divmipi_map[] = {4, 5, 6, 8};
	struct b52_sensor *sensor = to_b52_sensor(sd);

	OV13850r2a_get_mipiclock(sd, &mipi_clk, mclk);
	b52_sensor_call(sensor, i2c_read, 0x0304, &temp1, 1);
	temp2 = temp1 & 0x03;
	Pll2_divmipi = Pll2_divmipi_map[temp2];
	*rate = mipi_clk / Pll2_divmipi * sensor->drvdata->nr_lane;
