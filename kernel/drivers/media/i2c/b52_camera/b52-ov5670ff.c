...
#include "b52-ov5670ff.h"

static int OV5670FF_get_dphy_desc(struct v4l2_subdev *sd,
...
static int OV5670FF_get_pixelclock(struct v4l2_subdev *sd, u32 *rate, u32 mclk)
{
	/*calculate sysclk*/
	int temp1, temp2;
...
static int OV5670FF_read_OTP(struct b52_sensor *sensor,
...
static int OV5670FF_update_wb(struct b52_sensor *sensor,
			struct b52_sensor_otp *otp, u32 *flag)
{
...
static void OV5670FF_otp_access_start(struct b52_sensor *sensor)
{
	int temp;
...
static void OV5670FF_otp_access_end(struct b52_sensor *sensor)
{
	int temp;
	temp = OV5670_read_i2c(sensor, 0x5002);
	OV5670_write_i2c(sensor, 0x5002, temp|(0x08));
...
static int OV5670FF_update_otp(struct v4l2_subdev *sd,
				struct b52_sensor_otp *otp)
{
	int ret = 0;
	int flag = 0;
	...
	if (otp->otp_type == SENSOR_TO_SENSOR) {
		...
		OV5670FF_otp_access_start(sensor);

		/*read otp data firstly*/
		ret = OV5670FF_read_OTP(sensor, otp, &flag);

		/*access otp data end*/
		OV5670FF_otp_access_end(sensor);

		if (ret < 0)
			goto fail;
		/*apply some otp data, include awb*/
		if (otp->otp_ctrl & V4L2_CID_SENSOR_OTP_CONTROL_WB) {
			ret = OV5670FF_update_wb(sensor, otp, &flag);
			if (ret < 0)
				goto fail;
		}
