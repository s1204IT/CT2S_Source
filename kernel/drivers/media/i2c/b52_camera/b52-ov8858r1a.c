/* Marvell ISP OV8858R1A Driver
 *
 * Copyright (C) 2009-2014 Marvell International Ltd.
 *
 * Based on mt9v011 -Micron 1/4-Inch VGA Digital Image OV8858R1A
 *
...
#include "b52-ov8858r1a.h"

static int OV8858R1A_get_mipiclock(struct v4l2_subdev *sd, u32 *rate, u32 mclk)
{
	int temp1, temp2;
...
static int OV8858R1A_get_dphy_desc(struct v4l2_subdev *sd,
			struct csi_dphy_desc *dphy_desc, u32 mclk)
{
	return 0;
}

static int OV8858R1A_get_pixelclock(struct v4l2_subdev *sd, u32 *rate, u32 mclk)
{
	int temp1, temp2;
	int PLL1_divmipi;
	u32 mipi_clk;
	...
	struct b52_sensor *sensor = to_b52_sensor(sd);
	OV8858R1A_get_mipiclock(sd, &mipi_clk, mclk);
...
static void OV8858R1A_write_i2c(struct b52_sensor *sensor, u16 reg, u8 val)
{
	b52_sensor_call(sensor, i2c_write, reg, val, 1);
}
static u8 OV8858R1A_read_i2c(struct b52_sensor *sensor, u16 reg)
{
	int temp1;
	b52_sensor_call(sensor, i2c_read, reg, &temp1, 1);
	return temp1;
}
static void OV8858R1A_OTP_access_start(struct b52_sensor *sensor)
{
	int temp;
	temp = OV8858R1A_read_i2c(sensor, 0x5002);
	OV8858R1A_write_i2c(sensor, 0x5002, (0x00 & 0x08)|(temp & (~0x08)));

	OV8858R1A_write_i2c(sensor, 0x100, 0x01);
	OV8858R1A_write_i2c(sensor, 0x3d84, 0xC0);
	OV8858R1A_write_i2c(sensor, 0x3d88, 0x70);
	OV8858R1A_write_i2c(sensor, 0x3d89, 0x10);
	OV8858R1A_write_i2c(sensor, 0x3d8A, 0x71);
	OV8858R1A_write_i2c(sensor, 0x3d8B, 0x84);
	OV8858R1A_write_i2c(sensor, 0x3d81, 0x01);
	usleep_range(10000, 10010);

	return;
}
static int OV8858R1A_OTP_access_end(struct b52_sensor *sensor)
{
	int temp, i;

	for (i = 0x7010; i <= 0x7184; i++)
		OV8858R1A_write_i2c(sensor, i, 0);

	temp = OV8858R1A_read_i2c(sensor, 0x5002);
	OV8858R1A_write_i2c(sensor, 0x5002, (0x08 & 0x08)|(temp & (~0x08)));
	OV8858R1A_write_i2c(sensor, 0x100, 0x00);
...
static int check_otp_info(struct b52_sensor *sensor)
{
	...
	flag = OV8858R1A_read_i2c(sensor, OTP_DRV_START_ADDR);
	if ((flag & 0xc0) == 0x40)
		addr = 0x7011;
...
static int check_otp_wb(struct b52_sensor *sensor)
{
	...
	flag = OV8858R1A_read_i2c(sensor, OTP_DRV_WB_START_ADDR);
	if ((flag & 0xc0) == 0x40)
		addr = 0x7021;
	else if ((flag & 0x30) == 0x10)
		addr = 0x7026;
...
static int check_otp_lenc(struct b52_sensor *sensor)
{
	...
	flag = OV8858R1A_read_i2c(sensor, OTP_DRV_LENC_START_ADDR);
	if ((flag & 0xc0) == 0x40)
		addr = 0x703b;
	else if ((flag & 0x30) == 0x10)
		addr = 0x70a9;
...
{
	otp->module_id = OV8858R1A_read_i2c(sensor, addr);
	otp->lens_id = OV8858R1A_read_i2c(sensor, addr + 1);
				...
				struct b52_sensor_otp *otp)
{
	...
	temp = OV8858R1A_read_i2c(sensor, addr + 4);
	otp->rg_ratio = (OV8858R1A_read_i2c(sensor, addr) << 2)
					+ ((temp >> 6) & 0x03);
	otp->bg_ratio = (OV8858R1A_read_i2c(sensor, addr + 1) << 2)
					+ ((temp >> 4) & 0x03);
	otp->user_data[0] = (OV8858R1A_read_i2c(sensor, addr + 2) << 2)
					+ ((temp >> 2) & 0x03);
	otp->user_data[1] = (OV8858R1A_read_i2c(sensor, addr + 3) << 2)
					+ (temp & 0x03);
	return 0;
}
static int read_otp_lenc(struct b52_sensor *sensor, int addr, u8 *lenc)
{
	...
	for (i = 0; i < OTP_DRV_LENC_SIZE; i++)
		lenc[i] = OV8858R1A_read_i2c(sensor, addr + i);
...
{
	if (r_gain >= 0x400) {
		OV8858R1A_write_i2c(sensor, 0x5032, r_gain >> 8);
		OV8858R1A_write_i2c(sensor, 0x5033, r_gain & 0x00ff);
	}
	if (g_gain >= 0x400) {
		OV8858R1A_write_i2c(sensor, 0x5034, g_gain >> 8);
		OV8858R1A_write_i2c(sensor, 0x5035, g_gain & 0x00ff);
	}
	if (b_gain >= 0x400) {
		OV8858R1A_write_i2c(sensor, 0x5036, b_gain >> 8);
		OV8858R1A_write_i2c(sensor, 0x5037, b_gain & 0x00ff);
	}
	return 0;
}
static int update_lenc(struct b52_sensor *sensor, u8 *lenc)
{
	...
	temp = OV8858R1A_read_i2c(sensor, 0x5000);
	temp = 0x80 | temp;

	OV8858R1A_write_i2c(sensor, 0x5000, temp);
	for (i = 0; i < OTP_DRV_LENC_SIZE; i++)
		OV8858R1A_write_i2c(sensor, OTP_DRV_LSC_REG_ADDR + i, lenc[i]);
...
static int OV8858R1A_update_otp(struct v4l2_subdev *sd,
				struct b52_sensor_otp *otp)
{
	int ret = 0;
	struct b52_sensor *sensor = to_b52_sensor(sd);
	OV8858R1A_OTP_access_start(sensor);
	...
	OV8858R1A_OTP_access_end(sensor);
	}
	return 0;
}
