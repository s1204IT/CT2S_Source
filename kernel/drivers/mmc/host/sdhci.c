...
static void sdhci_prepare_data(struct sdhci_host *host, struct mmc_command *cmd)
{
	...
	if ((host->quirks2 & SDHCI_QUIRK2_DMA_CLOCK_FORCE_ON) &&
		...
		if (host->ops->clk_force_on)
			host->ops->clk_force_on(host, 1);
	}
#endif
}
...
void sdhci_send_command(struct sdhci_host *host, struct mmc_command *cmd)
{
	...
	if ((cmd->flags & MMC_RSP_OPCODE) &&
			!(host->quirks2 & SDHCI_QUIRK2_FAKE_CMD_INDEX_ERR))
		...
		if (host->vqmmc) {
			...
			if (!ret && !host->mmc->regulator_vqmmc_enabled) {
				if (!regulator_enable(host->vqmmc)) {
					regulator_set_optimum_mode(host->vqmmc,
						MMC_UA_LOAD_REQUIREMENT);
...
static int sdhci_execute_tuning(struct mmc_host *mmc, u32 opcode)
{
	...
	if (!tuning_loop_counter) {
		...
		if (!(ctrl & SDHCI_CTRL_TUNED_CLK)) {
			...
			pr_info(DRIVER_NAME ": Tuning passed, "
				"tuning_loop_counter = %d\n", tuning_loop_counter);
	}
...
static void sdhci_tasklet_finish(unsigned long param)
{
	...
	if (!(host->flags & SDHCI_DEVICE_DEAD) &&
		...
		sdhci_writel(host, SDHCI_INT_DATA_MASK, SDHCI_INT_STATUS);
	}
	...
	if (host->data) {
		...
		* Not need to do __sdhci_finish_data once more
...
int sdhci_suspend_host(struct sdhci_host *host)
{
	...
	if (ret) {
		...
		if (host->quirks2 & SDHCI_QUIRK2_HOLDSUSPEND_AFTER_REQUEST) {
			/* If suspend fail, enable the wake lock for bus busy */
...
int sdhci_resume_host(struct sdhci_host *host)
{
	...
	if (host->quirks2 & SDHCI_QUIRK2_HOLDSUSPEND_AFTER_REQUEST) {
		/* if system start to resume, En busbusy wakelock again */
		host->busbusy_wakelock_en = 1;
	}
