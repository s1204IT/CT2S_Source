		...
		struct sdhci_pxa_platdata *pdata)
{
	...
	if (pdata->flags & PXA_FLAG_NEW_RX_CFG_REG) {
		 ...
		 */
		tmp_reg |= (dtr_data->rx_delay & regdata->RX_SDCLK_DELAY_MASK) << RX_SDCLK_DELAY_SHIFT;
	...
	if (pdata && pdata->dtr_data) {
		...
		if (dtr_data->tx_delay) {
			...
			if ((MMC_TIMING_UHS_SDR104 == timing)
				...
				tmp_reg |= (dtr_data->tx_delay & regdata->TX_DELAY_MASK) << TX_DELAY1_SHIFT;
			else
		...
		u32 opcode, int point, unsigned long flags)
{
	...
	dev_dbg(mmc_dev(host->mmc), "point: %d, cmd.error: %d,"
...
static int pxav3_execute_tuning_nodvfs(struct sdhci_host *host, u32 opcode)
{
	...
	pr_info("%s: tunning %s at 0x%d, pass window length is 0x%d\n",
...
static int dvfs_request_success;
		...
		unsigned long rails, void *data)
{
	...
	if (vl->newlv == atomic_read(&cur_dvfs_level))
		dvfs_request_success = 1;
	else
		dvfs_request_success = 0;
...
static int pxav3_execute_tuning_dvfs(struct sdhci_host *host, u32 opcode)
{
	...
	sdh_tunning_savefreq();

#ifdef CONFIG_PXA_DVFS
#ifdef CONFIG_ARM64
		hwdvc_notify_register(&dvfs_notifier); /* debug use */
#else
	...
	do {
		...
		dvfs_request_success = 0;
#endif
		sdh_tunning_updatefreq(dvfs_level_updated);
		pxa_sdh_request_dvfs_level(host, dvfs_level_updated);
#ifdef CONFIG_PXA_DVFS
		if (!dvfs_request_success) {
			pr_err("%s: dvfs request level %d failed and tuning stopped\n",
...
#ifdef CONFIG_PXA_DVFS
#ifdef CONFIG_ARM64
		hwdvc_notify_unregister(&dvfs_notifier);
#else
...
static void pxav3_clk_force_on(struct sdhci_host *host, unsigned int force_on)
{
	u16 tmp;
...
static const struct sdhci_ops pxav3_sdhci_ops = {
	...
	.clk_force_on = pxav3_clk_force_on,
};
		...
		struct device *dev, struct sdhci_pxa_platdata *pdata)
{
	...
	if (!of_property_read_u32_index(np, "marvell,sdh-dvfs-levels", 1, &tmp))
		...
		pdata->dvfs_level_max = pxa_sdh_get_highest_dvfs_level(host); /* default max dvfs value */
	...
	if (of_property_read_bool(np, "marvell,sdh-dtr-data")) {
		...
		of_property_for_each_u32(np, "marvell,sdh-dtr-data", prop, p, timing) {
			...
			p = of_prop_next_u32(prop, p, &val);
			if (!p) {
				dev_err(dev, "missing fakeclk_tuned_level for timing %d\n",
						timing);
			} else {
				dtr_data[timing].fakeclk_tuned_level = val;
			}
			p = of_prop_next_u32(prop, p, &val);
			if (!p) {
				dev_err(dev, "missing fakeclk_fixed_en for timing %d\n",
						timing);
			} else {
				dtr_data[timing].fakeclk_fixed_en = val;
			}
		}
...
static int sdhci_pxav3_runtime_suspend(struct device *dev)
{
	...
	if (pltfm_host->clk) {
		if (host->quirks2 & SDHCI_QUIRK2_BASE_CLOCK_ALWAYS_ON) {
			if (!(host->quirks2 & SDHCI_QUIRK2_MANUAL_CARD_DETECT))
				goto fakeclk;
		} else {
			spin_lock_irqsave(&host->lock, flags);
