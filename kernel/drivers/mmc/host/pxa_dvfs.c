...
int pxa_sdh_enable_dvfs(struct sdhci_host *host)
{
	...
	if (pdata->fakeclk_fixed) {
		if (pdata->dtr_data[timing].fakeclk_fixed_en) {
...
int pxa_sdh_dvfs_timing_prepare(struct sdhci_host *host, unsigned char timing)
{
	...
	if (pdata && pdata->dtr_data) {
		...
		if (dtr_data->fakeclk_tuned_level)
			pxa_sdh_request_dvfs_level(host, dtr_data->fakeclk_tuned_level);

		if (pdata->fakeclk_fixed) {
			if (dtr_data->fakeclk_fixed_en && !__clk_is_prepared(pdata->fakeclk_fixed))
				clk_prepare_enable(pdata->fakeclk_fixed);
			else if (!dtr_data->fakeclk_fixed_en && __clk_is_prepared(pdata->fakeclk_fixed))
				clk_disable_unprepare(pdata->fakeclk_fixed);
		}
	}
	return 0;
