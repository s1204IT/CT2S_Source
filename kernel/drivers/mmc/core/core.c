...
*/
void mmc_start_bkops(struct mmc_card *card, bool from_exception, bool poweroff)
{
	int err;
	int timeout;
	...
	if (card->ext_csd.raw_bkops_status < EXT_CSD_BKOPS_LEVEL_2 &&
	    from_exception && !poweroff)
		return;

	mmc_claim_host(card->host);
	if ((card->ext_csd.raw_bkops_status >= EXT_CSD_BKOPS_LEVEL_2) || poweroff) {
				    ...
				    struct mmc_async_req *areq, int *error)
{
	...
	if (host->areq) {
		...
		if (host->card && mmc_card_mmc(host->card) &&
			...
			mmc_start_bkops(host->card, true, false);
	}
