...
#define GEU_DRIVER_VERSION	"GEU drvier 0.1.4"
...
#define GEU_READ_OEM_UUID	_IOW(GEU_IOCTL_MAGIC, 9, unsigned int)
...
static int GEU_read_oem_uuid(
		struct geu_private *geu,
		unsigned int *puiOemUuidHigh,
		unsigned int *puiOemUuidLow)
{
	unsigned int puiRead[2];
	struct device *dev = geu->p_device;
	...
	puiRead[0] = GEUReadReg(geu, GEU_FUSE_VAL_OEM_UID_H);
	puiRead[1] = GEUReadReg(geu, GEU_FUSE_VAL_OEM_UID_L);

	if (copy_to_user(puiOemUuidHigh, &(puiRead[0]), sizeof(unsigned int))) {
		clk_disable(geu->geu_clk);
		mutex_unlock(&(geu->geu_lock));
		return -EFAULT;
	}

	if (copy_to_user(puiOemUuidLow, &(puiRead[1]), sizeof(unsigned int))) {
		clk_disable(geu->geu_clk);
		mutex_unlock(&(geu->geu_lock));
		return -EFAULT;
	}
...
#endif
{
	...
	case GEU_READ_LIFECYCLE:
		{
			if (!access_ok
			    (VERIFY_WRITE, geu_arg.arg0, 2 * sizeof(unsigned int))) {
				return -EFAULT;
			}
			ret =
	...
	case GEU_READ_OEM_UUID:
		{
			if (!access_ok
			    (VERIFY_WRITE, geu_arg.arg0, sizeof(unsigned int))) {
				return -EFAULT;
			}
			if (!access_ok
			    (VERIFY_WRITE, geu_arg.arg1, sizeof(unsigned int))) {
				return -EFAULT;
			}
			ret =
				GEU_read_oem_uuid(geu, (void __user *)geu_arg.arg0,
...
static int mmp_geu_probe(struct platform_device *pdev)
{
	...
	if (geu->geu_iobase == NULL) {
		dev_err(dev, "GEU: ioremap fail from 0x%pa to 0x%pa\n",
			&r->start, &r->end);
		goto fail_free_res;
	}
