...
#include <linux/vmalloc.h>
#ifdef CONFIG_MRVL_LOG_LAST_KMSG
#include <linux/proc_fs.h>
#endif
...
static void __init mmp_log_save_old(void);
#else
static inline void mmp_log_save_old(void)
{
}
#endif
...
static int __init mmp_log_buf_init(void)
{
	...
	if (mmplog_reserved) {
		...
		pr_info("%s: *mmp_log_mag:%x *mmp_log_pos:%x mmp_log_buf:%p mmp_log_size:%u\n",
...
static void __init mmp_log_save_old(void)
{
	...
	last_kmsg_size = min(mmp_log_size, *mmp_log_pos);
	last_kmsg_buffer = vmalloc(last_kmsg_size);

	if (last_kmsg_size && last_kmsg_buffer) {
		memcpy(last_kmsg_buffer, mmp_log_buf, last_kmsg_size);
...
static ssize_t mmp_log_read_old(struct file *file, char __user * buf,
...
static const struct file_operations last_kmsg_file_ops = {
	.owner = THIS_MODULE,
	.read = mmp_log_read_old,
};

static int __init mmp_log_late_init(void)
{
...
late_initcall(mmp_log_late_init);
#endif
