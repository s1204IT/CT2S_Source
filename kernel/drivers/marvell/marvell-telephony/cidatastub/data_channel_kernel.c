...
#define DPRINT(fmt, args ...)   printk(KERN_INFO "CI_DATA_STUB: " fmt, ## args)
...
static inline int shouldRetry(int tried_times, int err)
{
...
static void ciCsdStubEventHandler(UINT8 *rxmsg)
{
	...
	if (pData->svcId != CiCsdStubSvc_Id) {
		printk(KERN_ERR "%s, svcId(%d) is incorrect, expect %d",
		...
		while (1) {
			ret =
			    clientCiDataIndicateCallback(rxmsg +
							 SHM_HEADER_SIZE);
			tried_times++;
			if (!shouldRetry(tried_times, ret))
				break;
