...
static void data_path_tx_func(unsigned long arg)
{
	...
	while (consumed_slot < max_tx_shots) {
		...
		if (shm_is_xmit_full(rbctl)) {
			 ...
			 * also check current fc status, if tx_stopped
			 * is already sent to cp, do not try to interrupt cp again
			 * it is useless, and just make cp busier
			 * BTW:
			 * this may have race condition here, but as cp side have
			 * a watermark for resume interrupt, we can assume it is safe
			 */
