...
#define SKB_LLIST_HEAD(name)	struct skb_llist_head name = SKB_LLIST_HEAD_INIT(name)
 ...
 */
static inline bool skb_llist_add(struct sk_buff *new, struct skb_llist_head *head)
{
...
static inline void skb_llist_enqueue(struct skb_llist *list, struct sk_buff *skb)
{
	skb_llist_add(skb, &list->head);
	atomic_inc(&list->len);
}

static inline void skb_llist_queue_head(struct skb_llist *list, struct sk_buff *skb)
{
