	...
	if (PppLogDb.IsEnabled) {
		...
		for (i = 0; i < numVariables; i++) {
			...
			varsString[varsStringIndex++] =
				hexArray[(currentEntry->Vars[i] >> 28) & 0xF];
			varsString[varsStringIndex++] =
				hexArray[(currentEntry->Vars[i] >> 24) & 0xF];
			varsString[varsStringIndex++] =
				hexArray[(currentEntry->Vars[i] >> 20) & 0xF];
			varsString[varsStringIndex++] =
				hexArray[(currentEntry->Vars[i] >> 16) & 0xF];
			varsString[varsStringIndex++] =
				hexArray[(currentEntry->Vars[i] >> 12) & 0xF];
			varsString[varsStringIndex++] =
				hexArray[(currentEntry->Vars[i] >> 8) & 0xF];
			varsString[varsStringIndex++] =
				hexArray[(currentEntry->Vars[i] >> 4) & 0xF];
			varsString[varsStringIndex++] =
				hexArray[(currentEntry->Vars[i]) & 0xF];
			varsString[varsStringIndex++] = ' ';
#endif
		}
#if defined(PPP_DEBUG)
...
#if defined(PPP_DEBUG)
	pr_debug("\n\r[PPP] %s(L:%d) - %s\n\r",
		PppLogEntryTypesStrings[type], entryNum, varsString);
#endif
}
