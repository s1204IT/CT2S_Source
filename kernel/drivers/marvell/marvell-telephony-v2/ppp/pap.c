...
static void papSendAuthenticateResp(PppControlS *pppControl)
{
	...
	pppControl->PppAuthenticationParams.PapAuthenticationParams =
		&pppControl->PapAuthenticationParams;

	if ((PPP_AUTHENTICATE
		 (pppControl->AuthenticateCallbackFunc, pppControl->Cid,
		 &pppControl->PppAuthenticationParams) == 0)) {
		...
		/* Returning zero length message, so total length
		 * is 6 bytes including header */
...
static void papSaveAuthenticationParams(PppControlS *pppControl)
{
	...
	*pppControl->PapAuthenticationParams.Password = '\0';
	...
	ASSERT(len <= 50);
	if (len != 0) {
		memcpy(pppControl->PapAuthenticationParams.Username,
			&papMessage->Data[index], len);
		index += len;
		len++;
		...
		ASSERT(len <= 50);
		if (len != 0) {
			memcpy(pppControl->PapAuthenticationParams.Password,
				&papMessage->Data[index], len);
			len++;
			pppControl->PapAuthenticationParams.Password[len] =
				'\0';
		}
	}
