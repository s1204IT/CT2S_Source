...
/* Feb 2014, Do APSE need:  #include "psd_data_channel.h" */
#include "ppp.h"
...
#include "serial_if.h"
/* Feb 2014, APSE has: include modem_if.h */

PppControlS PppControl;
static U_INT PppInitCount;
...
static unsigned short loopbackport;
static int loppback_dlmult;

void setLoopBack(unsigned short port, int DLMult)
{
	loopbackport = port;
	loppback_dlmult = DLMult;
}

#if ((defined(CONFIG_USB_ACM_PPP_MODE) || \
	defined(CONFIG_USB_PXA955_ACM_PPP_MODE)))
#define SERIAL_REGISTER(a)		gs_register_data_service(a)
#define SERIAL_UNREGISTER()		gs_unregister_data_service()
#else
#define SERIAL_REGISTER(a)		serial_register_data_service(a)
#define SERIAL_UNREGISTER()		serial_unregister_data_service()
#endif

static int __init ppp_module_init(void)
{
	dataCbFuncs DataCbFuncs = {
		PppInit, PppDeinit, PppReset, PppSetCid, PppMessageReq,
		PppUpdateIpParams
	};
	pr_info(
"pppRegisterToTTYService: before calling gs_register_data_service\n");
	SERIAL_REGISTER(DataCbFuncs);
	return 0;
}
...
{
	pr_info(
"pppUnregisterToTTYService: before calling gs_unregister_data_service\n");
	SERIAL_UNREGISTER();
}
...
static void pppStartTerminateTimer(PppControlS *pppControl, U_INT timeout,
	PppTimeoutCallbackFunc toCb)
{
	pppControl->PppTimerTerminate.expires =
		jiffies + msecs_to_jiffies(timeout);
	pppControl->PppTimerTerminate.function = toCb;
	mod_timer(&pppControl->PppTimerTerminate,
		pppControl->PppTimerTerminate.expires);
}
...
int pppRelayMessageFromComm(const U_CHAR *buf, U_INT count, U_CHAR cid)
{
	...
	if (PppControl.PppState == PPP_STATE_CONNECTED) {
		PPP_TRACE_BUF("pppRelayMessageFromComm ", (U_CHAR *) buf,
			count);

		/*
		 * Check if there is an available queue slot for the incoming
		 * packet. If no available slots available, drop the packet.
		 * we do not want to consume more and more memory to
		 * hold messages, system will run into OOM condition
		 * It is ok if we have some msg over 'max' we dont care so
		 * much, but we dont want to consume more and more
		 */
		if (PppControl.MsgQRef->msgCount > PppControl.MsgQRef->maxNum) {
			PppControl.QUEFULLDrops++;
			PPP_PRINTF(KERN_ERR
				"[PPP] QUEFULLDrops - Dropped %d CP "
				"packets due to queue full\n",
					(int)PppControl.QUEFULLDrops);
			return 1;
		}
		...
		if (messageFromComm != NULL) {
			...
			PPP_SEND_Q_MSG(PppControl.MsgQRef, &qMsg,
				sizeof(QueueMessageS));
		} else {
			PppControl.CPOOMDrops++;
			PPP_PRINTF(KERN_ERR
				"[PPP] OOM - Dropped %d CP packets\n",
				(int)PppControl.CPOOMDrops);
		}
...
static void pppRelayMessageToComm(PppControlS *pppControl)
{
	...
	/* Adjust length to send to COMM to include 2 bytes
	 * that were "dropped" in pppDecodeFrame */
	...
	if (pppControl->Cid != 0xFF)
		sendDataReqtoInternalList(pppControl->Cid, buf, length);
		/* Feb 2014, APSE use: pppSendDataReqtoPSDChannel */

	PPP_LOG_EXIT();

}

void pppSendCommMessageInd(PppControlS *pppControl, MessageBufS *ptr)
{
...
static U_INT pppHandleIncomingFrame(PppControlS *pppControl)
{
	PppFrameS *pppFrame;
	U_CHAR *p;
	U_SHORT proto;
	...
	if ((p[0] == PPP_ALLSTATIONS) && (p[1] == PPP_UI)) {
		...
		pppControl->InFrameContainer->Length -= 2;
	...
	case PPP_IPv6CP: /* Feb 2014 addition */
...
void pppProcessQMsg(QueueMessageS *qMsg)
{
	IpcpConnectionParamsS *ipParams;
	U_INT success = 0;
	PPP_LOG_ENTRY(MDB_pppProcessQMsg);

	if (unlikely(PppControl.Cid == 0xFF)) {
		pr_info("%s: Cid == 0xFF - drop msg\n", __func__);
		switch (qMsg->Type) {
		case PPP_MSG_REQ:
	...
	case PPP_MSG_REQ:
		...
		success = pppHandleIncomingFrame(&PppControl);
		PppFreeMessage((MessageBufS *)qMsg->Ptr);
		if ((!success) &&
			(PppControl.PppState <
			PPP_STATE_IPCP_SENT_ACK))
			PppStartLcpHandshake();
		break;

	case PPP_MSG_SEND_REQ:
		LcpSendConfigReq(&PppControl);
		break;
	case PPP_MSG_INIT_CONFIG_REQ:
		if (!PppControl.isReceivedLcpConfigReq)
			LcpSendConfigReq(&PppControl);
		break;
	...
	case PPP_IPCP_SEND_REQ:
		...
		PPP_LOG_3(ipParams->IpAddress, ipParams->PrimaryDns,
			ipParams->SecondaryDns);
	...
	case PPP_SEND_TERMINATE_REQ:
		pppStartTerminateTimer(&PppControl, PPP_TERMINATE_TIMEOUT,
			pppHandleTerminateTimoeut);
	...
	case PPP_LCP_ECHO_REQ_TIMEOUT:
		PPP_LOG();
		PPP_PRINTF(KERN_ERR
			"[PPP] Echo Request Timeout, Num Tries = %d\n",
			PppControl.lcpEchoReqTimeoutCount);

		/* If we reached max echo reuest timeouts,
		 * send terminate request (and print some debug info) */
		if (PppControl.lcpEchoReqTimeoutCount >=
			LCP_ECHO_REQ_NUM_TRIES) {
			PPP_PRINTF
				(KERN_ERR
				"[PPP] %s: %d echo requests failed on"
				" timeout, send terminate request\n",
				__func__, PppControl.lcpEchoReqTimeoutCount);
			/* we want to do proper termination (with timer) */
			PppSendTerminateReq();
		} else {
			PppControl.lcpEchoReqTimeoutCount++;
	...
	case PPP_IPCP_GET_IP_REQ:
		/* Feb 2014 - activate "AT+CGDCONT?\x0d" - list of cids? */
...
static void pppHandler(void)
{
	...
	while (1) {
		PPP_LOG();
		status =
			PPP_RECV_Q_MSG(PppControl.MsgQRef, &qMsg,
				sizeof(QueueMessageS));
		if (status != PPP_SUCCESS) {
			PPP_PRINTF(KERN_ERR
				"[PPP] %s: receive msg from queue error\n",
				__func__);
			continue;
		}
		...
		if (qMsg.Type == PPP_MSG_DEINIT) {
			/* Since de-init of PPP driver was called,
			we now exit the task */
...
static void pppSetLcpParams(PppControlS *pppControl)
{
	...
	PppControl.lcpXmitParams.MagicNumber.Number = 0;
	PppControl.lcpRecvParams.MagicNumber.Number = 0;
	PPP_SET_BIT(PppControl.lcpXmitParams.supportedParams, LCP_MAGIC);
	PPP_LOG_1(PppControl.lcpXmitParams.MagicNumber.Number);

	PPP_LCP_SET_PFC(pppControl->lcpXmitParams, FALSE);
	/* we support this feature
	 * but we will not enable it until configured */
	PPP_SET_BIT(pppControl->lcpXmitParams.supportedParams, LCP_PFC);
	PPP_LOG_2(PppControl.lcpXmitParams.isPfcEnabled,
		PppControl.lcpXmitParams.supportedParams);

	PPP_LCP_SET_ACFC(pppControl->lcpXmitParams, FALSE);
	/* we support this feature
	 * but we will not enable it until configured */
	PPP_SET_BIT(pppControl->lcpXmitParams.supportedParams, LCP_ACFC);
	PPP_LOG_2(PppControl.lcpXmitParams.isAcfcEnabled,
		PppControl.lcpXmitParams.supportedParams);
...
void PppSendToCallback(PppControlS *pppControl, U_CHAR *buf, U_INT length)
{
	...
	if (!pppControl->RxCallbackFunc) {
		pr_err("%s: Undefined RxCallbackFunc\n", __func__);
		return;
	}

	if (pppControl->Cid != 0xFF)
		if (pppControl->RxCallbackFunc(buf, length, PppControl.Cid)
			== length)
			return;

	/* did not manage to send all data for active PDP */
	if (pppControl->Cid != 0xFF)
		PppTerminate();
}
...
void PppReset(void)
{
	...
	PppControl.isReceivedLcpConfigReq = FALSE;
	PppControl.isReceivedLcpConfigAck = FALSE;
	PppControl.numWaitForConfigRequest = LCP_MAX_TIMES_WAIT_FOR_CONFIG_REQ;
	memset(PppControl.ipv6cp_recv_interface_id, 0,
		sizeof(PppControl.ipv6cp_recv_interface_id));
	memset(PppControl.ipv6cp_xmit_interface_id, 0,
		sizeof(PppControl.ipv6cp_xmit_interface_id));
	PppControl.ipcpXmitUpdated = FALSE;

	PppControl.CPOOMDrops = 0;
	PppControl.APOOMDrops = 0;
	PppControl.QUEFULLDrops = 0;

	del_timer(&PppControl.ChapTimer);
	del_timer(&PppControl.PppTimerTerminate);
	del_timer(&PppControl.LcpEchoReqTimeoutTimer);
	del_timer(&PppControl.LcpConfigReqTimer);
	/* on Reset, we also stop the callback func */
...
void PppInit(PppInitParamsS *PppInitParams)
{
	...
	if (PppInitCount == 0) {
		...
		PppLogInit();
		/*  init the timers */
		init_timer(&PppControl.ChapTimer);
		init_timer(&PppControl.LcpConfigReqTimer);
		init_timer(&PppControl.LcpEchoReqTimeoutTimer);
		init_timer(&PppControl.PppTimerTerminate);
		/* setup fixed data of timers */
		PppControl.ChapTimer.data =
			(unsigned long)&PppControl;
		PppControl.LcpConfigReqTimer.data =
			(unsigned long)&PppControl;
		PppControl.LcpEchoReqTimeoutTimer.data =
			(unsigned long)&PppControl;
		PppControl.PppTimerTerminate.data =
			(unsigned long)&PppControl;

		PppControl.RxCallbackFunc =
			PppInitParams->RxCallbackFunc;
		PppControl.TerminateCallbackFunc =
			PppInitParams->TerminateCallbackFunc;
		PppControl.ConnectCallbackFunc =
			PppInitParams->ConnectCallbackFunc;
		PppControl.AuthenticateCallbackFunc =
			PppInitParams->AuthenticateCallbackFunc;
		/* Feb 2014 - activate "AT+CGDCONT?\x0d" - list of cids? */
		PppControl.GetIpCallbackFunc =
			PppInitParams->GetIpCallbackFunc;
		PppControl.ServiceType = PppInitParams->ServiceType;
		/* we call init_completion so we can wait for the task
		(which is created few lines below..)
		when deInit is called  (or PPPremoval) */
		...
		if (PppControl.OutFrameContainer.Buf == NULL
			|| PppControl.OutMessageContainer.Buf == NULL) {
		...
		PppControl.MsgQRef =
			PPP_CREATE_Q_MSG(PPP_QUEUE_NUM_MESSAGES,
				sizeof(QueueMessageS));
		if (PppControl.MsgQRef == NULL) {
			PPP_PRINTF(KERN_ERR
				"[PPP] %s: create msg queue error\n",
				__func__);
			goto err1;
		}
		/* Create Task */
...
err2:
	PPP_DELETE_Q_MSG(PppControl.MsgQRef);
err1:
	if (PppControl.OutFrameContainer.Buf)
		PPP_FREE(PppControl.OutFrameContainer.Buf);
	if (PppControl.OutMessageContainer.Buf)
		PPP_FREE(PppControl.OutMessageContainer.Buf);
}
...
static void pppRelayMessageToCommDirect(MessageBufS *newMessage)
{
	...
	/* Adjust length to send to COMM to include
	 * 2 bytes that were "dropped" in pppDecodeFrame */
	...
	pipv4 = (struct iphdr *)buf;
	pudp = (struct udphdr *)(buf + ((U_INT)pipv4->ihl) * 4);
	...
	if (((loopbackport) &&
		(pipv4->version == 4) && (pipv4->protocol == IPPROTO_UDP) &&
		(!memcmp(&pudp->dest, &loopbackport, 2))) ||
		(loopbackport == 0xffff)) {
		int i = 0;
		long temp;

		memcpy(&temp, &pipv4->saddr, 4);
		memcpy(&pipv4->saddr, &pipv4->daddr, 4);
		memcpy(&pipv4->daddr, &temp, 4);

		for (i = 0; i < loppback_dlmult; i++)
			pppRelayMessageFromComm(buf, length, PppControl.Cid);

	} else if (PppControl.Cid != 0xFF)
		sendDataReqtoInternalList(PppControl.Cid, buf, length);
		/*  Feb 2014, APSE call here pppSendDataReqtoPSDChannel */
...
void PppMessageReq(const U_CHAR *buf, U_INT length)
{
	...
	BOOL isEnd = FALSE;
	/* s = running on source buffer, received from PC
	 * d = runs on destination buffer */
	U_CHAR *s, *d;
	QueueMessageS qMsg;
	...
	do {
		...
		if (PppControl.IsRxFragmented) {
			...
			if (newMessage == NULL) {
				...
				PPP_PRINTF(KERN_ERR
					"[PPP] %s: Fragmented Flag Error"
					" - Dropped %d AP packets\n",
					__func__,
					(int)PppControl.APOOMDrops);
				PPP_LOG_EXIT();
				return;
			}
		...
		while ((length > 0) && (!isEnd)) {
			if (*s == PPP_FLAG) {
				...
				/* protocol+FCS is also copied to buffer
				 * so add 3 bytes to MRU limit */
				if (newMessage->Length <
					PppControl.lcpRecvParams.Mru.Mru + 3) {
					PPP_ESCAPE_C(*d, *s);
		...
		if (newMessage->Length > PppControl.lcpRecvParams.Mru.Mru + 3) {
			...
			PPP_PRINTF(KERN_ERR
			"[PPP] Message too long, dropping...length=%d mru=%d\n",
				(int)newMessage->Length,
			...
			if (PPP_GOODFCS == PppControl.IncomingFcs) {
				/* Message is OK. newMessage.Length
				 * includes FCS, so decrease it by 2 */
				newMessage->Length -= 2;
				PPP_LOG_3((U_INT)newMessage,
					(U_INT)newMessage->Buf,
					newMessage->Length);
				PPP_TRACE_BUF("In-Decoded: ",
					newMessage->Buf,
					newMessage->Length);

				if (pppIsWriteThrough(newMessage) == TRUE) {
					if (PppControl.PppState >=
						PPP_STATE_IPCP_SENT_ACK) {
						pppRelayMessageToCommDirect
							(newMessage);
					} else
					/* incoming IP message when
					 * not connected yet -send LC
					 * Pconfiguration request again. */
						PppStartLcpHandshake();
					...
					PPP_SEND_Q_MSG(PppControl.MsgQRef,
						&qMsg, sizeof(qMsg));
					newMessage = NULL;
				}
			} else {
				...
				newMessage = NULL;
				PPP_PRINTF(KERN_ERR
				"[PPP] Wrong checksum, dropping message %x\n",
					 PppControl.IncomingFcs);
			}
...
void PppCreateMessageFrame(void)
{
	...
	for (msgLen = PppControl.OutMessageContainer.Length; msgLen > 0;
		msgLen--) {
		if ((*msgPtr < PPP_TRANS) || (*msgPtr == PPP_FLAG) ||
			(*msgPtr == PPP_ESCAPE)) {
			frameLen++;
	...
	PPP_TRACE_BUF("CreateMessageFrame: ", PppControl.OutFrameContainer.Buf,
		frameLen);
...
void PppUpdateIpParams(IpcpConnectionParamsS *inIpParams)
{
	...
	PPP_LOG_3(inIpParams->IpAddress, inIpParams->PrimaryDns,
		inIpParams->SecondaryDns);

	ipParams =
		(IpcpConnectionParamsS *)
		PPP_MALLOC(sizeof(IpcpConnectionParamsS));
	...
	memcpy(ipParams, inIpParams, sizeof(IpcpConnectionParamsS));
...
void PppSetCid(U_INT cid)
{
	if (cid != 0xFF)
		...
		if (PppControl.Cid != 0xFF) {
			pr_err("%s: NotUnreg %d due to data coming from COMM...\n",
				__func__, PppControl.Cid
				);
			/* first one would 'unreg' then set the 'new cid'
			* (unaasigned) - lets not 'unreg', due to timing,
			* this can cause issues with data still coming
			* from COMM side.
			* CiDataDownlinkUnregister(PppControl.cid); */
			PppControl.Cid = cid;
		}
}
...
{
	QueueMessageS qMsg;
	qMsg.Type = PPP_MSG_INIT_CONFIG_REQ;
	PPP_SEND_Q_MSG(PppControl.MsgQRef, &qMsg, sizeof(qMsg));
}
#if defined(PPP_UNIT_TEST)
