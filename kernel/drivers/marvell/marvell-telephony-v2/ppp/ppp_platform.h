...
/* Feb 2014:  Next line not in APSE (linux_types.h) */
...
#define	TRACE_BUF_SIZE_MAX 256
#define	TRACE_BUF_SIZE_LIMIT 64
#endif
...
#define PPP_CREATE_Q_MSG(nUMmESSAGES, sIZEoFmESSAGE) \
		linuxPppCreateMsgQ(nUMmESSAGES, sIZEoFmESSAGE)
#define PPP_DELETE_Q_MSG(qREF) \
		linuxPppDeleteMsgQ(qREF)
#define PPP_RECV_Q_MSG(qREF, pTR, sIZE) \
		linuxPppRecvMsgQ(qREF, (U_CHAR *)pTR, sIZE, PPP_SUSPEND)
#define PPP_SEND_Q_MSG(qREF, pTR, sIZE) \
		linuxPppSendMsgQ(qREF, (U_CHAR *)pTR, sIZE)
...
#define PPP_PRINTF		printk_ratelimited
...
#endif
typedef int (*PppAuthenticateCallbackFunc) (unsigned char cid,
...
PPP_STATUS linuxPppRecvMsgQ(PPP_Q_REF_TYPE qRef,
	U_CHAR *ptr, U_INT size, U_INT timeout);
...
/* Feb 2014 what is it for? list of CIDs? ( "AT+CGDCONT?\x0d" ) */
