...
static void lcpStartEchoReqTimer(PppControlS *pppControl, U_INT timeout,
				 PppTimeoutCallbackFunc toCb)
{
	PPP_LOG_ENTRY(MDB_lcpStartEchoReqTimer);
	PPP_LOG_1(timeout);

	pppControl->LcpEchoReqTimeoutTimer.expires =
		jiffies + msecs_to_jiffies(timeout);
	pppControl->LcpEchoReqTimeoutTimer.function = toCb;

	mod_timer(&pppControl->LcpEchoReqTimeoutTimer,
		pppControl->LcpEchoReqTimeoutTimer.expires);
...
static void lcpHandleEchoRequest(PppControlS *pppControl)
{
	...
	/* pppControl->InMessage->Data; point to LcpMessage.Id, skipping Type */
	U_CHAR *inbuf = (U_CHAR *)&inLcpMsg->Id;
	/* length of LCP message without message type, which is modified. */
	U_SHORT inLen;
	...
	inbuf++;
	inLen -= 2;
	/* put our magic number in place */
	PPP_APPEND_LONG(buf,
		index, pppControl->
		lcpXmitParams.MagicNumber.Number, fcs);
	inLen -= 4;
	/* skip incoming magic number position */
	inbuf += 4;
	while (inLen != 0) {
		PPP_APPEND(buf, index, *inbuf, fcs);
		inbuf++;
...
static void lcpHandleEchoReqAck(PppControlS *pppControl)
{
	...
	lcpStartEchoReqTimer(pppControl, LCP_ECHO_REQ_INTERVAL_TIME,
		lcpSendNextEchoReq);
...
static void lcpHandleConfigAck(PppControlS *pppControl)
{
	...
	del_timer(&pppControl->LcpConfigReqTimer);
	pppControl->isReceivedLcpConfigAck = TRUE;

	if (PPP_IS_BIT_SET
		(pppControl->lcpXmitParams.supportedParams, LCP_PFC)) {
	...
	if (PPP_IS_BIT_SET
		(pppControl->lcpXmitParams.supportedParams, LCP_ACFC)) {
	...
	pppControl->lcpXmitParams.MagicNumber.Number =
		pppControl->lcpXmitMagicTemp;
	if (!pppControl->isReceivedLcpConfigReq) {
		pppControl->LcpConfigReqTimer.expires =	jiffies +
			msecs_to_jiffies(PPP_TIMEOUT_WAIT_FOR_CONFIG_REQUEST);
		pppControl->LcpConfigReqTimer.function =
			LcpHandleConfigRequestWaitTimeout;
		mod_timer(&pppControl->LcpConfigReqTimer,
			pppControl->LcpConfigReqTimer.expires);
	}
	PPP_LOG_EXIT();
}
...
static void lcpHandleConfigRej(PppControlS *pppControl)
{
	...
	while (len > 0) {
		...
		PPP_CLEAR_BIT(pppControl->lcpXmitParams.supportedParams,
			lcpOption->Type);
...
static void lcpHandleConfigReq(PppControlS *pppControl)
{
	...
	while (len > 0) {
		...
		{
			U_CHAR *p = lcpOption->Data;
			GETLONG(pppControl->lcpRecvParams.MagicNumber.Number,
				p);
		}
			break;
		case LCP_PFC:
	...
	pppControl->isReceivedLcpConfigReq = TRUE;
	del_timer(&pppControl->LcpConfigReqTimer);
	if (isAckMessage) {
		QueueMessageS qMsg;
		lcpSendAck(pppControl);
		if (!pppControl->isReceivedLcpConfigAck) {
			qMsg.Type = PPP_MSG_SEND_REQ;
			PPP_LOG_1(qMsg.Type);
...
/* CCP is rejected using LCP Protocol Reject message */
/************************************************************************/
void CcpHandleIncomingFrame(PppControlS *pppControl)
{
	...
	/* Use LcpMessage type to obtain message length; */
	LcpMessageS *lcpMessage = (LcpMessageS *) pppControl->InMessage->Data;
	U_SHORT inLen;
	...
	/* Header + 2 bytes for 0x80FD + CCP message length */
	PPP_APPEND_SHORT(buf, index, 4 + 2 + inLen, fcs);
...
void LcpSendConfigReq(PppControlS *pppControl)
{
	...
	for (i = 0; i < LCP_MAX_OPTIONS; i++) {
		if (PPP_IS_BIT_SET
			(pppControl->lcpXmitParams.supportedParams, i)) {
			switch (i) {
			case LCP_MRU:
				PPP_APPEND(buf, index,
					pppControl->lcpXmitParams.
					Mru.Header.Type,
					fcs);
				PPP_APPEND(buf, index,
					pppControl->lcpXmitParams.
					Mru.Header.Length,
					fcs);
				PPP_APPEND_SHORT(buf, index,
					pppControl->lcpXmitParams.
					Mru.Mru, fcs);
				PPP_LOG_1(
					pppControl->lcpXmitParams.Mru.Mru);
				len += LCP_MRU_LEN;
				break;
			...
			case LCP_ACCM:
				PPP_APPEND(buf, index,
					...
					fcs);
				PPP_APPEND(buf, index,
					pppControl->lcpXmitParams.Accm.Header.
					Length,
					fcs);
				PPP_APPEND_LONG(buf, index,
					pppControl->lcpXmitParams.
					Accm.Accm, fcs);
				PPP_LOG_1(
					pppControl->lcpXmitParams.Accm.Accm);
				len += LCP_ACCM_LEN;
				break;

			case LCP_AUTH:
				if (pppControl->lcpXmitParams.
					AuthProtocol.Proto == PPP_PAP) {
					PPP_APPEND(buf, index,
						pppControl->lcpXmitParams.
						AuthProtocol.Header.Type,
						fcs);
					PPP_APPEND(buf, index,
						pppControl->lcpXmitParams.
						AuthProtocol.Header.Length,
						fcs);
					PPP_APPEND_SHORT(buf, index,
						pppControl->lcpXmitParams.
						AuthProtocol.Proto,
						fcs);
					PPP_LOG_1(pppControl->lcpXmitParams.
						AuthProtocol.Proto);
					len += LCP_AUTH_LEN;

				} else if (pppControl->lcpXmitParams.
						AuthProtocol.Proto ==
						PPP_CHAP) {
					PPP_APPEND(buf, index,
						pppControl->lcpXmitParams.
			...
			case LCP_MAGIC:
				PPP_APPEND(buf, index,
					pppControl->lcpXmitParams.MagicNumber.
					Header.Type,
					fcs);
				PPP_APPEND(buf, index,
					pppControl->lcpXmitParams.MagicNumber.
					...
					fcs);
				/*
				 * if we received a magic number from remote,
				 *  let's use it to generate our own
				*/
				if (pppControl->
					lcpRecvParams.MagicNumber.Number
					!= 0) {
					magic =	pppControl->
						lcpRecvParams.MagicNumber.
						Number;
					PPP_LOG_1(magic);
					magic ^= 0x7EA5A57E;
					PPP_LOG_1(magic);
				...
				PPP_LOG_1(pppControl->lcpXmitParams.
					MagicNumber.Number);
				pppControl->lcpXmitMagicTemp = magic;
	...
	PppCreateMessageFrame();
	/* wait for LCP Config Request Ack or timeout */
	pppControl->LcpConfigReqTimer.expires = jiffies +
		msecs_to_jiffies(PPP_TIMEOUT_WAIT_FOR_CONFIG_ACK);
	pppControl->LcpConfigReqTimer.function =
		LcpHandleConfigRequestWaitTimeout;
	mod_timer(&pppControl->LcpConfigReqTimer,
		pppControl->LcpConfigReqTimer.expires);
...
void LcpSendEchoReq(PppControlS *pppControl)
{
	...
	lcpStartEchoReqTimer(pppControl, LCP_ECHO_REQ_TIMEOUT,
		lcpHandleEchoReqTimeout);
...
void LcpHandleConfigRequestWaitTimeout(unsigned long arg)
{
	QueueMessageS qMsg;
	PppControlS *pppControl = (PppControlS *)arg;
	if (pppControl->isReceivedLcpConfigReq)
	...
	if (--pppControl->numWaitForConfigRequest > 0)
