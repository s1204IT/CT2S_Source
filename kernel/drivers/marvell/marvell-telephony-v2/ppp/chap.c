...
static void chapHandleChallengeTimoeut(unsigned long val)
{
	...
	PPP_PRINTF("[PPP] %s: timeout occurred\n", __func__);
...
static void chapStartTimer(PppControlS *pppControl)
{
	...
	pppControl->ChapTimer.expires = jiffies +
		msecs_to_jiffies(CHAP_CHALLENGE_RESPONSE_TIMEOUT_IN_MSEC);
	pppControl->ChapTimer.function = chapHandleChallengeTimoeut;
	mod_timer(&pppControl->ChapTimer, pppControl->ChapTimer.expires);


}

static void chapSendAuthSuccessOrFail(PppControlS *pppControl,
	U_CHAR success_or_fail_code)
{
	const char *message[] = {
		"Authentication Successful", "Authentication Failed" };
	char *pMessage =
		(char *)message[success_or_fail_code -
...
static void chapCreateNewChallange(PppControlS *pppControl)
{
	...
	for (i = 0; i < sizeof(fixedChallengeValue); i++) {
		pppControl->chapChallenge.challengeValue[i] =
			fixedChallengeValue[i];
	}
	pppControl->chapChallenge.challengeValueLength =
		sizeof(fixedChallengeValue);

#else
	PPP_PRINTF(KERN_ERR
	"[PPP] %s: Variable challenge length is currently not supported",
	__func__);
#endif
...
void ChapSendAuthChallenge(PppControlS *pppControl)
{
	...
	PPP_APPEND(buf, index, pppControl->chapChallenge.challengeValueLength,
		   fcs);

	for (i = 0; i < pppControl->chapChallenge.challengeValueLength;
		i++, len++) {
		PPP_APPEND(buf, index,
			   pppControl->chapChallenge.challengeValue[i], fcs);
	}

	for (i = 0; i < pppControl->chapChallenge.challengeNameLength;
		i++, len++) {
		PPP_APPEND(buf, index,
			   pppControl->chapChallenge.challengeName[i], fcs);
	}
...
{
	ChapMessageS *chapMessage =
		(ChapMessageS *)pppControl->InMessage->Data;
	...
	if (chapResponse->responseValueLength > PPP_MAX_CHAP_RESPONSE_LENGTH) {
		PPP_PRINTF("[PPP] %s: responseValueLength greater than %d\n",
			__func__, PPP_MAX_CHAP_RESPONSE_LENGTH);

		LcpSendTerminateReq(pppControl);
	} else {
		pppControl->chapResponse.responseValueLength =
			chapResponse->responseValueLength;
		PPP_PRINTF("[PPP] %s: responseValueLength = %d\n",
			__func__, chapResponse->responseValueLength);
		for (i = 0; i < chapResponse->responseValueLength; i++) {
			pppControl->chapResponse.responseValue[i] =
				chapResponse->responseValue[i];
		}

		chapSendAuthSuccessOrFail(pppControl,
					  CHAP_AUTHENTICATE_SUCCESS);
	}
...
void ChapHandleIncomingFrame(PppControlS *pppControl)
{
	...
	default:
		PPP_PRINTF("[PPP] %s: received ChapMessageS->Type = %d\n",
