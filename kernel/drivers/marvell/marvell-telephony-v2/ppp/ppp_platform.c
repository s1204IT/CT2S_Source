...
PPP_Q_REF_TYPE linuxPppCreateMsgQ(U_INT numMessages, U_INT sizeOfMessage)
{
	...
	qRef->msgCount = 0;
	/* we start semaphor with 1 to avoid checkPatch warning */
	sema_init(&(qRef->sema), 1);
	spin_lock_init(&qRef->fifo_lock);
	/* decrement semaphore, as we need it to be set on 0 */
	down(&qRef->sema);
	return qRef;
}
...
PPP_STATUS linuxPppSendMsgQ(PPP_Q_REF_TYPE qRef, U_CHAR *ptr, U_INT size)
{
	...
	/* No actual limit on Q, but we check #of items
	     before calling add..to Q in the PPP logic
	     so not to congest the system */
...
{
	U_INT i;
	char dbuf[TRACE_BUF_SIZE_MAX] = "*";

	pr_debug("\n\r[PPP] %s  %d- ", string, length);
	if (length > TRACE_BUF_SIZE_LIMIT)
		length = TRACE_BUF_SIZE_LIMIT;

	for (i = 0; length > 0; length--, i++)
		sprintf(dbuf + i * 3, "%02X ", buf[i]);

	pr_debug("%s\n", dbuf);

}
#endif /* #if defined (PPP_DEBUG) */
