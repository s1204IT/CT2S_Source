				 ...
				 U_CHAR ipcpRejectsListCount)
{
	...
	pr_debug("[ipcp SendConfigReject]\n");
...
static void ipcpHandleConfigAck(PppControlS *pppControl)
{
	...
	pr_debug("[ipcp HandleConfigAck]\n");
	pppControl->ipcpRecvAck = TRUE;
	pppControl->PppState = PPP_STATE_CONNECTED;
	/* As we are now connected, register with xxx the cb to give us data */
...
static void ipcpHandleConfigReq(PppControlS *pppControl)
{
	...
	/* BS - fix no DNS after long delay before entering data state - start*/
		...
		if (!pppControl->isConnectIndSent) {
			...
			/* Feb 2014 - what is the purpose of this?
			AT+CGDCONT.. list of cids? */
			if (PPP_CONNECT
	...
	/* Feb 2014, this was  removed by apse:	msleep_interruptible(1000); */
		...
		PPP_LOG();
	/* BS - fix no DNS after long delay before entering data state - end*/
	} else {
		pppControl->PppState = PPP_STATE_IPCP_SENT_ACK;
	...
	PppOptionS *ipcpRejectsList[], U_CHAR ipcpRejectsListCount)
{
	...
	pr_debug("[ipv6cp SendConfigReject]\n");
...
static void ipv6cpHandleConfigReq(PppControlS *pppControl)
{
	...
	pr_debug("[ipv6cp HandleConfigReq]\n");
	GETSHORT_NOINC(ipcpMessageLength, &ipcpMessageHeader->Length);
	PPP_LOG_1(ipcpMessageLength);
	...
	while (ipcpMessageLength > 0) {
		...
		if (ipcpCurrentOption->Type == IPV6CP_OPT_INTERFACE_IDENTIFIER
			...
			pr_debug(
"[%s] IPV6CP_OPT_INTERFACE_IDENTIFIER: %02x%02x:%02x%02x:%02x%02x:%02x%02x\n",
				__func__,
	...
	if (ipcpRejectsListCount > 0) {
		...
		if (!pppControl->isConnectIndSent) {
			...
			if (PPP_CONNECT
				(pppControl->ConnectCallbackFunc,
				 pppControl->Cid)) {
				/* Feb 2014 - what is the purpose of
				this request, sends 'AT+CGDCONT?\x0d'
				- get list of CIDs? */
				QueueMessageS qMsg;
				qMsg.Type = PPP_IPCP_GET_IP_REQ;
		...
		pppControl->PppState = PPP_STATE_CONNECTED;
		/* As we are now connected, register with xxx
		the cb to give us data */
