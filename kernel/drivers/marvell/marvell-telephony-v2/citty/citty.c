...
struct buf_struct {
	unsigned char *p_buf[NUM_CITTY_BUF];
	int i_data_len[NUM_CITTY_BUF];
	int i_buf_in;
	int i_buf_out;
	wait_queue_head_t g_inq;
	wait_queue_head_t g_outq;
...
struct buf_struct tx_citty_buf[CITTY_TTY_MINORS];
...
void cci_init_buffer(struct buf_struct *buffer)
{
	...
	for (i = 0; i < NUM_CITTY_BUF; i++) {
		buffer->p_buf[i] = kmalloc(CITTY_BUF_SIZE, GFP_KERNEL);
		if (!buffer->p_buf[i])
	...
	sema_init(&(buffer->g_sem), 1);

	/* init the queue */
	init_waitqueue_head(&(buffer->g_inq));
	init_waitqueue_head(&(buffer->g_outq));
...
*
*******************************************************************/
size_t read_citty_buffer(char *buf, struct buf_struct *citty_buf, short action)
{
	size_t count;
	unsigned char *pbuf;
	...
	l_sem = &(citty_buf->g_sem);
	F_ENTER();

	/* Enter critial section */
	if (down_interruptible(l_sem))
		return -ERESTARTSYS;

	while (citty_buf->i_buf_in == citty_buf->i_buf_out) {
		up(l_sem);	/* release the lock */
		...
		if (wait_event_interruptible
			(citty_buf->g_inq,
			(citty_buf->i_buf_in != citty_buf->i_buf_out))) {
			/* waken up by signal, get the lock and process it */
			if (down_interruptible(l_sem)) {
				pr_err("Error down_interruptible.\n");
				return -ERESTARTSYS;
			}
			break;
	...
	/* Double check */
	if (citty_buf->i_buf_in == citty_buf->i_buf_out) {
		up(l_sem);	/* release the lock */
		/* return -ERESTARTSYS; */
		return 0;
	}
	...
	cur_buf_index = citty_buf->i_buf_out++;
	pbuf = citty_buf->p_buf[cur_buf_index];

	/*
	 *  Check if it is flipped
	 */
	if (citty_buf->i_buf_out >= NUM_CITTY_BUF)
		citty_buf->i_buf_out = citty_buf->i_buf_out % NUM_CITTY_BUF;
	...
	count = citty_buf->i_data_len[cur_buf_index];
	...
	wake_up_interruptible(&citty_buf->g_outq);
...
{
	if (buffer->i_buf_out == buffer->i_buf_in)
		return (NUM_CITTY_BUF - 1) * CITTY_BUF_SIZE;

	return (((buffer->i_buf_out + NUM_CITTY_BUF -
		  buffer->i_buf_in) % NUM_CITTY_BUF) - 1) * CITTY_BUF_SIZE;
}

size_t write_citty_buffer(struct buf_struct *citty_buf,
			  const char *buf, size_t count, short action)
{
	...
	if (down_interruptible(l_sem)) {
		pr_err("\"%s\" Error: Unable to down SEM.\n", current->comm);
		return -ERESTARTSYS;
	}

	/* Make sure there's space to write */
	while (spacefree(citty_buf) == 0) {	/* full */
		...
		prepare_to_wait(&citty_buf->g_outq, &wait, TASK_INTERRUPTIBLE);

		if (spacefree(citty_buf) == 0) {
			/* seem like it is bad: scheduling while atomic */
			schedule();
		}
		finish_wait(&citty_buf->g_outq, &wait);
		if (signal_pending(current)) {
			pr_err("\"%s\" Error: Unable to signal_pending.\n",
			       current->comm);
			/* signal: tell the fs layer to handle it */
			return -ERESTARTSYS;
		}
	...
	cur_buf_index = citty_buf->i_buf_in++;
	pbuf = citty_buf->p_buf[cur_buf_index];
	 ...
	 *  Check if it is flipped
	 */
	if (citty_buf->i_buf_in >= NUM_CITTY_BUF)
		citty_buf->i_buf_in = citty_buf->i_buf_in % NUM_CITTY_BUF;
	...
	if (action == COPY_FROM_USER) {
		PDEBUG("%s: going to copy_from_user at buf ", __func__);
		PDEBUG("index %d and count %d",
		     cur_buf_index, count);
		...
		PDEBUG("%s: going to COPY_FROM_CITTY at buf ", __func__);
		PDEBUG("index %d and count %d",
		     cur_buf_index, count);
	...
	/* saving datalen */
	citty_buf->i_data_len[cur_buf_index] = count;

	up(l_sem);

	/* finally, awake any reader */
	/* blocked in read() and select() */
	wake_up_interruptible(&citty_buf->g_inq);
		       ...
		       int count)
{
	 ...
	 */
	retval =
	    write_citty_buffer(&tx_citty_buf[tty->index], buffer, count,
			       COPY_FROM_CITTY);
...
static int citty_write_room(struct tty_struct *tty)
{
	...
	/* CHECKPOINT */
	/* room = CITTY_BUF_SIZE * spacefree(  &tx_citty_buf ); */
	room = CITTY_BUF_SIZE * spacefree(&tx_citty_buf[tty->index]);
			      ...
			      struct ktermios *old_termios)
{
	...
	if (I_IXOFF(tty) || I_IXON(tty)) {
		...
		if (I_IXOFF(tty))
			PDEBUG(
			" - INBOUND XON/XOFF is enabled, XON = %2x, XOFF = %2x",
		...
		if (I_IXON(tty))
			PDEBUG(
			" - OUTBOUND XON/XOFF is enabled, XON = %2x, XOFF = %2x",
			start_char, stop_char);
		else
			      ...
			      unsigned int cmd, unsigned long arg)
{
	...
	if (copy_to_user((void *)arg,
		...
		pr_err("Failed to copy to user for tcgets.\n");
		return -EFAULT;
	}
	F_LEAVE();
...
{
	/*
	 * (NUM_CITTY_BUF-spacefree(tx_citty_buf[tty->index]))
	 * CITTY_BUF_SIZE;
	 */
	return 0;
}
		    ...
		    loff_t *f_pos)
{
	...
	PDEBUG("tx_citty_buf[%d].i_buf_out: %d, i_buf_in: %d\n",
		   minor_num,
	       tx_citty_buf[minor_num].i_buf_out,
		   tx_citty_buf[minor_num].i_buf_in);

	retval = read_citty_buffer(buf, &tx_citty_buf[minor_num], COPY_TO_USER);
...
unsigned int cctdev_poll(struct file *filp, poll_table *wait)
{
	...
	down(&tx_citty_buf[minor_num].g_sem);

	poll_wait(filp, &tx_citty_buf[minor_num].g_inq, wait);

	if (tx_citty_buf[minor_num].i_buf_out !=
		tx_citty_buf[minor_num].i_buf_in)
		mask |= POLLIN | POLLRDNORM;

	mask |= POLLOUT | POLLWRNORM;

	up(&tx_citty_buf[minor_num].g_sem);
...
static int __init citty_init(void)
{
	...
	for (i = 0; i < CITTY_TTY_MINORS; ++i) {
		/* Init buffer */
		cci_init_buffer(&tx_citty_buf[i]);
...
static void __exit citty_exit(void)
{
	...
	for (i = 0; i < CITTY_TTY_MINORS; ++i) {
		...
		cci_free_buffer(&tx_citty_buf[i]);
	}
