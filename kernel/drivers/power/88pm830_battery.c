...
#define FAST_MONITOR_INTERVAL		(HZ * 10)
...
#define NTC_BIAS_CHANGE_TEMP3	(info->switch_thr[2])
...
/* when battery is removed, do NOT queue delayed work */
static bool is_battery_removed = false;

/* helper function */
static int pm830_get_batt_jeita(void)
{
	return ccnt_data.jeita;
}
...
static int pm830_gp0_bias_set(struct pm830_battery_info *info, int bias)
{
	...
	dev_info(info->dev, "GPADC0 bias is changed from %duA to %duA\n",
		info->gp0_bias_curr[info->bias], info->gp0_bias_curr[bias]);
...
static int pm830_get_batt_temp(struct pm830_battery_info *info, int *data)
{
	...
	*data = ((buf[0] & 0xff) << 4) | (buf[1] & 0x0f);
	...
	dev_dbg(info->dev, "temp=%d(C), NTC_voltage=%d(mV), ADC-val=0x%x, bias=%d(uA)\n",
		temp, voltage, *data, info->gp0_bias_curr[info->bias]);
...
static int pm830_handle_tbat(struct pm830_battery_info *info)
{
	...
	int ret, temp, old_temp, old_bias;
	int i;
	...
	do {
		i = 0;
		old_temp = 0;
		/* get a stable read of battery temperature */
		while (i < info->tbat_ov_samp_count) {
			ret = pm830_get_batt_temp(info, &temp);
			if (ret < 0) {
				dev_info(info->chip->dev, "%s: read battery temperature fail\n",  __func__);
				return 0;
			}
			...
			if (i == 0) {
				old_temp = temp;
				i++;
			} else if (abs(temp - old_temp) > 1)
				i = 0;
			else
				i++;
			if (info->tbat_ov_samp_sleep)
				msleep(info->tbat_ov_samp_sleep);
		}
		...
		case NTC_BIAS_1:
			if (temp > NTC_BIAS_CHANGE_TEMP1)
				ret = pm830_gp0_bias_set(info, NTC_BIAS_2);
			break;
		case NTC_BIAS_2:
			if (temp < NTC_BIAS_CHANGE_TEMP1)
				ret = pm830_gp0_bias_set(info, NTC_BIAS_1);
			else if (temp > NTC_BIAS_CHANGE_TEMP2)
				ret = pm830_gp0_bias_set(info, NTC_BIAS_3);
			break;
		case NTC_BIAS_3:
			if (temp < NTC_BIAS_CHANGE_TEMP2)
				ret = pm830_gp0_bias_set(info, NTC_BIAS_2);
			else if (temp > NTC_BIAS_CHANGE_TEMP3)
				ret = pm830_gp0_bias_set(info, NTC_BIAS_4);
			break;
		case NTC_BIAS_4:
			if (temp < NTC_BIAS_CHANGE_TEMP3)
				ret = pm830_gp0_bias_set(info, NTC_BIAS_3);
			break;
		default:
			ret = -EINVAL;
		...
		if (ret < 0) {
			dev_info(info->chip->dev, "%s: set gpadc0 bias current fail\n", __func__);
			return ret;
		}

	} while (old_bias != info->bias);

	info->bat_params.temp = temp * 10;

	/* get battery health status according to battery temp */
	ccnt_data.jeita = JEITA_INV;
	if (temp < info->tbat_thr[0]) {
		...
		tbat_hi_th = info->tbat_thr[1] + 1;
		info->bat_params.health = POWER_SUPPLY_HEALTH_COLD;

	} else if (temp < info->tbat_thr[2]) {
		dev_info(info->chip->dev, "%s: TBAT is LTR\n", __func__);
		tbat_lo_th = info->tbat_thr[1] - 1;
		tbat_hi_th = info->tbat_thr[2] + 1;
		info->bat_params.health = POWER_SUPPLY_HEALTH_GOOD;
		ccnt_data.jeita = JEITA_LTR;

	} else if (temp < info->tbat_thr[3]) {
		dev_info(info->chip->dev, "%s: TBAT is STR\n", __func__);
		tbat_lo_th = info->tbat_thr[2] - 1;
		tbat_hi_th = info->tbat_thr[3] + 1;
		info->bat_params.health = POWER_SUPPLY_HEALTH_GOOD;
		ccnt_data.jeita = JEITA_STR;

	} else if (temp < info->tbat_thr[4]) {
		dev_info(info->chip->dev, "%s: TBAT is HTR\n", __func__);
		tbat_lo_th = info->tbat_thr[3] - 1;
		tbat_hi_th = info->tbat_thr[4] + 1;
		info->bat_params.health = POWER_SUPPLY_HEALTH_GOOD;
		ccnt_data.jeita = JEITA_HTR;

	} else if (temp < info->tbat_thr[5]) {
		dev_err(info->chip->dev, "%s: TBAT too hot!\n", __func__);
		tbat_lo_th = info->tbat_thr[4] - 1;
		tbat_hi_th = info->tbat_thr[5];
	...
	switch (info->bias) {
	case NTC_BIAS_1:
		tbat_hi_th = MIN(tbat_hi_th, NTC_BIAS_CHANGE_TEMP1 + 1);
		break;
	case NTC_BIAS_2:
		tbat_hi_th = MIN(tbat_hi_th, NTC_BIAS_CHANGE_TEMP2 + 1);
		tbat_lo_th = MAX(tbat_lo_th, NTC_BIAS_CHANGE_TEMP1 - 1);
		break;
	case NTC_BIAS_3:
		tbat_hi_th = MIN(tbat_hi_th, NTC_BIAS_CHANGE_TEMP3 + 1);
		tbat_lo_th = MAX(tbat_lo_th, NTC_BIAS_CHANGE_TEMP2 - 1);
		break;
	case NTC_BIAS_4:
		tbat_lo_th = MAX(tbat_lo_th, NTC_BIAS_CHANGE_TEMP3 - 1);
		break;
	default:
		return -EINVAL;
	}
	...
	if (ret < 0)
		return ret;
	dev_dbg(info->chip->dev, "new thresholds: low=%d(C),%d(mV),0x%x; high=%d(C),%d(mV),0x%x\n",
		tbat_lo_th, gpadc_lo_th, pm830_get_adc_value(PM830_GPADC_GPADC0, gpadc_lo_th),
		tbat_hi_th, gpadc_hi_th, pm830_get_adc_value(PM830_GPADC_GPADC0, gpadc_hi_th));
...
static void pm830_tbat_work(struct work_struct *work)
{
	...
	/* irq handle is finished, re-enable it */
	enable_irq(info->tbat_irq);
}
...
static irqreturn_t pm830_tbat_handler(int irq, void *data)
{
	...
	dev_info(info->chip->dev, "%s is served\n", __func__);

	/* make sure the interrupt is disabled until configuring new thresholds */
	disable_irq_nosync(info->tbat_irq);
...
static void pm830_read_soc(struct pm830_battery_info *info)
{
	...
	/*
	 * for normal case, if cap > soc, we need to ramp down cap to soc.
	 * but, in order to handle low battery voltage cases:
	 * 1. when battery voltage drop to power off threshold, we need to decrease cap 1%
	 * every cycle, no matter the relation between soc and cap. but we must ensure that
	 * cap >= 1
	 * 2. when battery voltage drop to safe power off threshold, we need to decrease cap
	 * 1% every cycle and ensure that cap >= 0
	 */

	/* safe threshold crossed - decrease cap to 0% */
	if (vbat_mv <= info->safe_power_off_threshold) {
		dev_info(info->dev, "voltage lower than %d\n", info->safe_power_off_threshold);
		soc = 0;

	/* low-bat threshold crossed - decrease cap to 1% */
	} else if (vbat_mv <= info->power_off_threshold) {
		dev_info(info->dev, "voltage lower than %d\n", info->power_off_threshold);
		soc = 1;

	/* voltage is OK - ramp up cap to 2% */
	} else if (soc <= 1)
		soc = 2;
	...
	case POWER_SUPPLY_STATUS_FULL:
		...
		/*
		 * in case we are in supplement mode and real soc is decreasing
		 * below supplement threshold, need to make sure fake soc is
		 * decreasing as well
		 */
		if (soc < info->supp_mode_th) {
			dev_dbg(info->dev, "supplement, cap: %d --> soc: %d:\n",
					cap, soc);
			if (soc > cap)
				cap++;
			else if (soc < cap)
	...
	case POWER_SUPPLY_STATUS_CHARGING:
		...
		/* clamp soc to 99% until reaching end-of-charge */
		soc = (soc > 99) ? 99 : soc;
		 ...
		 * in case we are in supplement mode and real soc is decreasing,
		 * need to make sure fake soc is decreasing as well
		 */
		if (soc >= cap) {
			if (soc - cap > 2)
				cap++;
		...
		/* else */
		dev_info(info->dev, "supplement, cap: %d --> soc: %d:\n", cap, soc);
		bat_status = POWER_SUPPLY_STATUS_DISCHARGING;
		bat_full = false;
		/* supplement mode: go through */
	...
	default:
		...
		if (soc < cap) {
			dischg_count++;
			dev_info(info->dev, "cap: %d, dischg_count: %d\n",
					cap, dischg_count);
			if (dischg_count >= 2) {
				cap--;
				dischg_count = 0;
...
{
	int ibat, data = 0;
	int prev_cap = info->bat_params.cap;
	int prev_soc = ccnt_data.soc;
	int prev_volt = info->bat_params.volt;
	int prev_status = info->bat_params.status;
	char *bat_status[] = {
		"UNKNOWN",
		"CHARGING",
		"DISCHARGING",
...
static void pm830_battery_work(struct work_struct *work)
{
	...
	/*
	 * perform fast update when:
	 * - real-cap is low
	 * - cap and real-cap have big difference
	 */
	if ((abs(ccnt_data.soc - info->bat_params.cap) > 3) || (ccnt_data.soc <= LOW_BAT_CAP))
		queue_delayed_work(info->bat_wqueue, &info->monitor_work, FAST_MONITOR_INTERVAL);
	else
		queue_delayed_work(info->bat_wqueue, &info->monitor_work, MONITOR_INTERVAL);
}
...
static void pm830_fg_setup(struct pm830_battery_info *info)
{
	 ...
	 */
	data = mask = PM830_VBAT_SMPL_NUM(3) | PM830_IBAT_EOC_EN | PM830_IBAT_SLP_TH(3);
	regmap_update_bits(info->chip->regmap, PM830_CC_MISC2, mask, data);
...
static void pm830_init_fg(struct pm830_battery_info *info)
{
	...
	regmap_update_bits(info->chip->regmap, PM830_GPADC_CONFIG2, PM830_BD_EN, 0);
	...
	if (info->bat_ntc) {
		...
		data = BIAS_GP0_SET(1);
		mask = BIAS_GP0_MASK;
		regmap_update_bits(info->chip->regmap, PM830_GPADC0_BIAS, mask, data);

		/* enable GPADC0 bias */
		mask = PM830_GPADC0_BIAS_EN | PM830_GPADC0_GP_BIAS_OUT;
		regmap_update_bits(info->chip->regmap, PM830_GPADC_BIAS_ENA, mask, mask);

		/* enable GPADC measurement */
		data = mask = PM830_GPADC0_MEAS_EN;
		regmap_update_bits(info->chip->regmap, PM830_GPADC_MEAS_EN, mask, data);

		/* wait for voltage to be stable */
		msleep(20);

		/* read GPADC0 voltage */
		regmap_bulk_read(info->chip->regmap, PM830_GPADC0_MEAS1, buf, 2);
		data = ((buf[0] & 0xff) << 4) | (buf[1] & 0x0f);
		voltage = pm830_get_adc_volt(PM830_GPADC_GPADC0, data);

		/* check if battery is present or not */
		if (voltage < 700)
			info->bat_params.present = 1;
		else
			info->bat_params.present = 0;
		dev_info(info->dev, "GPADC0 = %d[mV], battery present = %d\n",
			 voltage, info->bat_params.present);

		/* set bias for battery temperature */
		if (info->temp_range_num)
	...
	if (info->bat_params.present == 0) {
		dev_info(info->dev, "battery not present, power supply mode...\n");
			       ...
			       union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CAPACITY_LEVEL:
		/* report real capacity to charger */
		val->intval = ccnt_data.soc;
		break;
	default:
		return -ENODEV;
	}
			 ...
			 struct pm830_bat_pdata *pdata)
{
	...
	ret = of_property_read_u32(np, "tbat-ov-samp-count", &pdata->tbat_ov_samp_count);
	if (ret)
		pdata->tbat_ov_samp_count = 1;
	ret = of_property_read_u32(np, "tbat-ov-samp-sleep", &pdata->tbat_ov_samp_sleep);
	if (ret)
		pdata->tbat_ov_samp_sleep = 0;
	...
	ret = of_property_read_u32(np, "supp-threshold", &pdata->supp_mode_th);
	if (ret)
		return ret;
	...
	if (pdata->temp_range_num) {
		ret = of_property_read_u32_array(np, "switch-thr", pdata->switch_thr, 3);
		if (ret)
			return ret;
		ret = of_property_read_u32_array(np, "r-tbat-thr", pdata->r_tbat_thr, 6);
		if (ret)
			return ret;
		ret = of_property_read_u32_array(np, "gp0-bias-curr", pdata->gp0_bias_curr, 4);
		if (ret)
			return ret;
...
static int pm830_battery_probe(struct platform_device *pdev)
{
	...
	info->tbat_ov_samp_count = pdata->tbat_ov_samp_count;
	info->tbat_ov_samp_sleep = pdata->tbat_ov_samp_sleep;
	...
	info->supp_mode_th = pdata->supp_mode_th;
	...
	if (info->temp_range_num) {
		...
		/*
		 * init bias to the range that cover the room temperature(25 C),
		 * so in most cases, no need to switch bias during boot.
		 */
		info->bias = NTC_BIAS_1;
		for (i = 0; i < info->temp_range_num - 1; i++) {
			if (info->switch_thr[i] < 25)
				info->bias = i + 1;
		}
	...
	chip->get_batt_jeita = pm830_get_batt_jeita;
	info->chip = chip;
	info->dev = &pdev->dev;
	info->bat_params.status = POWER_SUPPLY_STATUS_UNKNOWN;
	/* default type [Lion] */
	...
	info->tbat_irq = platform_get_irq_byname(pdev, "gpadc0-tbat");
	if (ret < 0) {
		dev_err(info->dev, "failed to get gpadc0-tbat irq!\n");
		goto out_ntc;
	}
	ret = devm_request_threaded_irq(info->dev, info->tbat_irq, NULL,
					pm830_tbat_handler,
					IRQF_ONESHOT | IRQF_NO_SUSPEND,
					"gpadc0-tbat", info);
	if (ret < 0) {
		dev_err(info->dev, "failed to request irq for gpadc0-tbat\n");
		goto out_ntc;
	}
...
out_irq:
	devm_free_irq(info->dev, info->tbat_irq, info);
...
static int pm830_battery_remove(struct platform_device *pdev)
{
	...
	/*
	 * make sure no delayed work is queued again after resource
	 * has been freed.
	 * power_supply_changed() will call pm830_external_power_changed(),
	 * which will queue charge_work (which cause OOPS).
	 */
	is_battery_removed = true;

	devm_free_irq(info->dev, info->tbat_irq, info);

	cancel_delayed_work_sync(&info->monitor_work);
	cancel_delayed_work_sync(&info->charged_work);
	cancel_delayed_work_sync(&info->cc_work);
	cancel_work_sync(&info->tbat_work);
