...
#define PM830_CHG_MPPT_WGHT_OFFSET	(0)
#define PM830_CHG_MPPT_WGHT_MASK	(0x7 << PM830_CHG_MPPT_WGHT_OFFSET)
#define PM830_CHG_MPPT_BIDIR_EN		(0x1 << 4)
#define PM830_CHG_CTRL19		(0x55)
#define PM830_CHG_MPPT_START_OFFSET	(0)
#define PM830_CHG_MPPT_START_MASK	(0xf << PM830_CHG_MPPT_START_OFFSET)
#define PM830_CHG_MPPT_PER_OFFSET	(4)
#define PM830_CHG_MPPT_PER_MASK		(0x7 << PM830_CHG_MPPT_PER_OFFSET)
...
static inline int get_fastchg_cur(struct pm830_charger_info *info)
{
	...
	ret = (info->fastchg_cur[info->pm830_jeita] - 500) / 100;
...
static inline int get_fastchg_vol(struct pm830_charger_info *info)
{
	...
	ret = (info->fastchg_vol[info->pm830_jeita] - 3600) / 25;
	if (ret > 0x24)
		ret = 0x24;
...
{
	unsigned int limit_cur, ret = 0;

	limit_cur = info->limit_cur;
...
static int pm830_get_batt_property(struct pm830_charger_info *info,
					enum power_supply_property prop, int *val)
{
	...
	if (!psy || !psy->get_property) {
		dev_err(info->dev, "get battery power supply failed\n");
		return -EINVAL;
	}
...
static int pm830_is_chg_allowed(struct pm830_charger_info *info)
{
	...
	ret = pm830_get_batt_property(info, POWER_SUPPLY_PROP_STATUS, &val);
	if (ret) {
		dev_err(info->dev, "get battery property %d failed\n", POWER_SUPPLY_PROP_STATUS);
		return 0;
	}
	...
	ret = pm830_get_batt_property(info, POWER_SUPPLY_PROP_PRESENT, &val);
	if (ret) {
		dev_err(info->dev, "get battery property failed.\n");
		return 0;
	}
	...
	ret = pm830_get_batt_property(info, POWER_SUPPLY_PROP_HEALTH, &val);
	if (ret) {
		dev_err(info->dev, "get battery property failed.\n");
		return 0;
	}

	if (val != POWER_SUPPLY_HEALTH_GOOD) {
	...
	if (!info->allow_recharge) {
		...
		ret = pm830_get_batt_property(info, POWER_SUPPLY_PROP_CAPACITY_LEVEL, &val);
		if (ret) {
			dev_err(info->dev, "get battery property failed\n");
			return 0;
		}

		if (val <= RECHARGE_THRESHOLD)
	...
	if (!info->allow_chg_after_overvoltage) {
		...
		ret = pm830_get_batt_property(info, POWER_SUPPLY_PROP_VOLTAGE_NOW, &val);
		if (ret) {
			dev_err(info->dev, "get battery property failed.\n");
			return 0;
		}

		if (val >= OVERVOLTAGE_RECHARGE_LEVEL) {
			      ...
			      union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_ONLINE:
		...
		if ((value & 0x1) == PM830_CHG_DET) {
			...
			if (info->pm830_status == POWER_SUPPLY_STATUS_CHARGING)
				dev_err(info->dev,
					"Warning! charger is offline but is in chargring mode\n");
		}
		break;
	...
	case POWER_SUPPLY_PROP_VOLTAGE_MAX:
		if (info->pm830_jeita < JEITA_STR || info->pm830_jeita > JEITA_HTR)
			val->intval = 0;
		else
			val->intval = info->fastchg_vol[info->pm830_jeita];
		break;
	case POWER_SUPPLY_PROP_CURRENT_MAX:
		if (info->pm830_jeita < JEITA_STR || info->pm830_jeita > JEITA_HTR)
			val->intval = 0;
		else
			val->intval = info->fastchg_cur[info->pm830_jeita];
		break;
	default:
		return -EINVAL;
	}
 ...
 */
static int vbat_array[2] = {3600, 3800};
static int pm830_set_wa_th(struct pm830_charger_info *info)
{
	 ...
	 */
	array_size = ARRAY_SIZE(vbat_array);
	offset = 1;
 ...
 */
static void pm830_mppt_enable(struct pm830_charger_info *info, int cur_limit)
{
	int mask, val;
	...
	if (cur_limit > 0) {
		cur_limit = cur_limit > info->mppt_max_cur ? info->mppt_max_cur : cur_limit;
		...
		/* enable MPPT bidirectional and set mppt wght */
		mask = PM830_CHG_MPPT_WGHT_MASK | PM830_CHG_MPPT_BIDIR_EN;
		val = PM830_CHG_MPPT_BIDIR_EN | (info->mppt_wght << PM830_CHG_MPPT_WGHT_OFFSET);
		regmap_update_bits(info->regmap, PM830_CHG_CTRL18, mask, val);

		/* set mppt start current, mppt observation period and enable mppt algorithm */
		val = (cur_limit << PM830_CHG_MPPT_START_OFFSET)
				| (info->mppt_per << PM830_CHG_MPPT_PER_OFFSET)
				| PM830_CHG_MPPT_EN;
		regmap_write(info->regmap, PM830_CHG_CTRL19, val);

		/* set mppt status as enabled */
		atomic_set(&mppt_enable, 1);
	} else {
		regmap_update_bits(info->regmap, PM830_CHG_CTRL19, PM830_CHG_MPPT_EN, 0);
...
static int pm830_start_charging(struct pm830_charger_info *info)
{
	...
	/* Clear CHG_START bit */
	regmap_update_bits(info->regmap, PM830_CHG_CTRL1, CHG_START, 0);
	...
	regmap_write(info->regmap, PM830_CHG_CTRL7, get_limit_cur(info));
	...
	/* Start charging... */
	regmap_update_bits(info->regmap, PM830_CHG_CTRL1, CHG_START, CHG_START);

	/* if it's not recharge - set status as charging */
	if (info->pm830_status != POWER_SUPPLY_STATUS_FULL)
...
static void pm830_chg_ext_power_changed(struct power_supply *psy)
{
	...
	jeita_old = info->pm830_jeita;
	info->pm830_jeita = info->chip->get_batt_jeita();
	chg_allowed = pm830_is_chg_allowed(info);
	...
	if (chg_allowed) {
		if (info->pm830_status != POWER_SUPPLY_STATUS_CHARGING) {
			...
			power_supply_changed(&info->ac);
			power_supply_changed(&info->usb);
		} else if (jeita_old != info->pm830_jeita) {
			dev_info(info->dev, "%s: battery temperature changed!!\n", __func__);
			/* temp changed, need to use the new voltage and current setting */
			pm830_stop_charging(info);
			pm830_start_charging(info);
			power_supply_changed(&info->ac);
			power_supply_changed(&info->usb);
...
static void pm830_chg_init(struct pm830_charger_info *info)
{
	 ...
	 * design team
	 */
	if (info->over_vol < info->fastchg_vol[JEITA_STR])
		info->over_vol = info->fastchg_vol[JEITA_STR] + 400;
	...
	/* disable MPPT when it is not needed */
	if (!info->mppt_en)
		regmap_update_bits(info->regmap, PM830_CHG_CTRL19, PM830_CHG_MPPT_EN, 0);

	/*
	 * set VBAT low threshold to 0V, and upp threshold to MAX.
	 * the upp threshold will be used by battery short detect and WA_TH dynamic setting
	 */
				       ...
				       unsigned long type, void *chg_event)
{
	...
	pm830_mppt_enable(info, get_limit_cur(info));
	info->allow_recharge = 1;
	...
	dev_info(info->dev, "usb inserted: ac = %d, usb = %d\n",
...
static irqreturn_t pm830_done_handler(int irq, void *data)
{
	...
	if (value & PM830_FAULT_CHG_REMOVAL) {
		 ...
		 */
		dev_info(info->dev, "usb plugged out case\n");
...
static irqreturn_t pm830_exception_handler(int irq, void *data)
{
	...
	/* report interrupt reason according to register 0x57, 0x58 */
			 ...
			 struct pm830_chg_pdata *pdata)
{
	...
	if (ret)
		return ret;
	ret = of_property_read_u32_array(np, "fastchg-voltage", pdata->fastchg_vol, 3);
	if (ret)
		return ret;
	ret = of_property_read_u32_array(np, "fastchg-current", pdata->fastchg_cur, 3);
	if (ret)
		return ret;
	...
	pdata->mppt_en = of_property_read_bool(np, "mppt-enable");
	if (pdata->mppt_en) {
...
static int pm830_charger_probe(struct platform_device *pdev)
{
	...
	info->fastchg_cur = pdata->fastchg_cur;
	...
	info->pm830_jeita = chip->get_batt_jeita();
